...
//
RTC_EXTERN void RTCLogReportEx(RTCLoggingSeverity severity, BOOL is_report, NSString* log_string);
...
#define RTCLogReportFormat(severity, is_report, format, ...)    \
  do {                                                          \
    NSString* log_string = RTCLogString(format, ##__VA_ARGS__); \
    RTCLogReportEx(severity, is_report, log_string);            \
  } while (false)

#define RTCLogReportVerbose(format, ...) \
  RTCLogReportFormat(RTCLoggingSeverityVerbose, true, format, ##__VA_ARGS__)

#define RTCLogReportInfo(format, ...) \
  RTCLogReportFormat(RTCLoggingSeverityInfo, true, format, ##__VA_ARGS__)

#define RTCLogReportWarning(format, ...) \
  RTCLogReportFormat(RTCLoggingSeverityWarning, true, format, ##__VA_ARGS__)

#define RTCLogReportError(format, ...) \
  RTCLogReportFormat(RTCLoggingSeverityError, true, format, ##__VA_ARGS__)

#if !defined(NDEBUG)
#define RTCLogReportDebug(format, ...) RTCLogReportInfo(format, ##__VA_ARGS__)
#else
#define RTCLogReportDebug(format, ...) \
  do {                                 \
  } while (false)
#endif

#define RTCLogReport(format, ...) RTCLogReportInfo(format, ##__VA_ARGS__)
