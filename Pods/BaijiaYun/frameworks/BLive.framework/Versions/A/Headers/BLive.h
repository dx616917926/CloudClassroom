...
#import "BLiveDef.h"
#import "BLiveDelegate.h"
#import "BLiveMixStreamParams.h"
#import "BLiveView.h"

NS_ASSUME_NONNULL_BEGIN

//! Project version number for BLive.
FOUNDATION_EXPORT NSString *BLiveName(void);

//! Project version string for BLive.
FOUNDATION_EXPORT NSString *BLiveVersion(void);

#pragma mark -

/**
 ### 直播场景管理类
 SDK 入口类，管理进出直播场景、切换用户角色、推拉流等功能
 */
@interface BLive: NSObject
...
/**
 直播事件回调代理
 */
@property (nonatomic, weak) id<BLiveDelegate> delegate;
...
/**
 进入直播场景
 @param params 进入时必须的参数，参考 `BLiveParams`
 */
- (nullable NSError *)enterRoom:(BLiveParams *)params;
...
/**
 切换角色
 主播：可以随时发布自己的音视频流
 观众：只能观看其他用户的音视频流，要发布音视频流，需要先通过 switchRole 切换成主播
 切换完成回调参考 `onSwitchLoginUserRole:`
 @param role 参考 `BLiveRoleType`
 */
- (nullable NSError *)switchRole:(BLiveRoleType)role;

#pragma mark - mix stream

/**
 开启合流
 设置合流参数，需要在 `startMixTranscode` 前设置，未设置将使用默认值
 合流开启后，所有用户可以收到 `onMixStreamAdded:` 后播放合流
 合流的设置以最后一个用户调用时设置的参数为准
 需要当前用户为 BLiveRoleType_anchor 身份
 */
- (nullable NSError *)startMixTranscode:(BLiveMixStreamParams *)params;
...
/**
 ...
 需要当前用户为 BLiveRoleType_anchor 身份
 */
- (nullable NSError *)stopMixTranscode:(NSString *)mixStreamID;

/**
 播放合流
 收到 `onMixStreamAdded:` 后即可播放合流
 禁止直接添加视图到返回的视图上
 */
- (nullable BLiveView *)playMixStream:(BLivePlayStreamInfo *)playInfo;

/**
 停止播放合流
 指定了 mixStreamID，将会停止正在播放的该 mixStreamID 的所有流
 指定了 urlString，将会停止指定播放的 urlString 的流
 如果是合流，建议调用当前接口，不直接调用 `BLiveView` 的 `clear` 方法
 */
...
/**
 部署环境
 */
+ (void)setDeployType:(BLiveDeployType)deployType;
 ...
 * @return NSError BLiveErrorCode_invalidCalling 需要主播身份
 */
- (nullable NSError *)muteLocalVideo:(BOOL)mute;
 ...
 * @return NSError BLiveErrorCode_invalidCalling 需要主播身份
 */
- (nullable NSError *)startRemoteView:(NSString *)userId streamType:(BRTCVideoStreamType)streamType view:(BRTCView *)view;
 ...
 * @return NSError BLiveErrorCode_invalidCalling 需要主播身份
 */
- (nullable NSError *)muteRemoteVideoStream:(NSString *)userId mute:(BOOL)mute;
 ...
 * @return NSError BLiveErrorCode_invalidCalling 需要主播身份
 */
- (nullable NSError *)muteAllRemoteVideoStreams:(BOOL)mute;
 ...
 * @return NSError BLiveErrorCode_invalidCalling 需要主播身份
 */
- (nullable NSError *)muteLocalAudio:(BOOL)mute;
 ...
 * @return NSError BLiveErrorCode_invalidCalling 需要主播身份
 */
- (nullable NSError *)muteRemoteAudio:(NSString *)userId mute:(BOOL)mute;
 ...
 * @return NSError BLiveErrorCode_invalidCalling 需要主播身份
 */
- (nullable NSError *)muteAllRemoteAudio:(BOOL)mute;
