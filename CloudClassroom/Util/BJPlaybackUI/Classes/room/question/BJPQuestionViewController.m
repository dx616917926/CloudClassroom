...
@property (nonatomic) NSMutableArray<BJVQuestion *> *questionList;
...
- (void)makeObserving {
    ...
    [self bjl_observe:BJLMakeMethod(self.room.roomVM, didUnpublishQuestionWithQuestionID:)
             observer:^BOOL(NSString *questionID) {
                 ...
                 [self updateQuestionListWithQuestionID:questionID questionState:BJVQuestionUnpublished];
                 if (!self || !self.isViewLoaded || !self.view.window || self.view.hidden) {
                     return YES;
                 }
                 [self.tableView reloadData];
...
- (void)updateQuestionListWithQuestion:(BJVQuestion *)question {
...
- (BOOL)replaceQuestionListWithQuestion:(BJVQuestion *)question {
    // !!! 更新问答
    BOOL existQuestion = NO;
    for (BJVQuestion *oldQuestion in [self.questionList copy]) {
...
- (void)updateQuestionListWithPublishQuestion:(BJVQuestion *)question {
...
- (void)updateQuestionListWithQuestionID:(NSString *)questionID questionState:(BJVQuestionState)state {
    // !!! 目前主要用于处理取消发布的问答，删除问答数据
    for (BJVQuestion *question in [self.questionList copy]) {
        if ([question.ID isEqualToString:questionID]) {
            // 改变状态
            question.state = state;
            if (question.state & BJVQuestionUnpublished) {
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    BJVQuestion *question = [self.questionList bjl_objectAtIndex:section];
    return question.replies.count + 1;
}

- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    BJVQuestion *question = [self.questionList bjl_objectAtIndex:indexPath.section];
    ...
    if (indexPath.row > 0) {
        ...
        [cell updateWithQuestion:nil questionReply:bjl_as([question.replies bjl_objectAtIndex:indexPath.row - 1], BJVQuestionReply)];
    }
    else {
...
- (CGFloat)tableView:(UITableView *)tableView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    BJVQuestion *question = [self.questionList bjl_objectAtIndex:indexPath.section];
    BJVQuestionReply *questionReply;
    NSString *key;
    NSString *identifier;
    if (indexPath.row > 0) {
        questionReply = bjl_as([question.replies bjl_objectAtIndex:indexPath.row - 1], BJVQuestionReply);
