...
@property (nonatomic) BJVAnswerSheet *answerSheet;

@property (nonatomic) NSArray<BJVAnswerSheetOption *> *selectedOptions;
...
- (instancetype)initWithAnswerSheet:(BJVAnswerSheet *)answerSheet {
    ...
    if (self) {
        ...
        self->_optionButtonWH = (answerSheet.answerType == BJVAnswerSheetType_Judgement) ? 64.0 : 40.0;
        self->_optionCountForRow = (answerSheet.answerType == BJVAnswerSheetType_Judgement) ? 2 : 4;
...
- (void)showCorrectAnswers {
    NSMutableArray<BJVAnswerSheetOption *> *optionsArray = [NSMutableArray new];

    NSString *answerString = @"";
    for (BJVAnswerSheetOption *option in self.answerSheet.options) {
    ...
    UILabel *titleLabel = [self labelWithTitle:BJLLocalizedString(@"答题结果") color:[UIColor whiteColor]];
    ...
    UILabel *tipLabel = [self labelWithTitle:BJLLocalizedString(@"我的答案：") color:[self grayTextColor]];
    ...
    if ([self.selectedOptions count]) {
        ...
        UILabel *label = [self labelWithTitle:BJLLocalizedString(@"您未参与答题~") color:[self grayTextColor]];
    ...
    UILabel *correctAnswerLabel = [self labelWithTitle:[NSString stringWithFormat:BJLLocalizedString(@"正确答案：%@"), answerString] color:[self grayTextColor]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    BJVAnswerSheetOption *option = [[self.selectedOptions bjl_objectAtIndex:indexPath.row] bjl_as:[BJVAnswerSheetOption class]];
...
- (UIView *)answerResultContentView {
    if (!_answerResultContentView) {
        ...
        _answerResultContentView.accessibilityIdentifier = BJLKeypath(self, answerResultContentView);
    }
