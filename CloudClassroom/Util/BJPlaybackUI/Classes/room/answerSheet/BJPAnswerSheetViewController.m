...
- (void)setUpTopBar {
    // title label
    UILabel *titleLabel = [self labelWithTitle:BJLLocalizedString(@"答题器") color:[UIColor whiteColor]];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    BJVAnswerSheetOption *option = [[self.answerSheet.options bjl_objectAtIndex:indexPath.row] bjl_as:[BJVAnswerSheetOption class]];
    ...
    [cell setOptionSelectedCallback:^(BOOL selected) {
        bjl_strongify(self);
        if (self.answerSheet.answerType == BJVAnswerSheetType_Judgement) {
            // 判断题只能选择一个答案
            for (BJVAnswerSheetOption *option in self.answerSheet.options) {
                option.selected = NO;
            }
...
- (UILabel *)commentsLabel {
    if (!_commentsLabel) {
        ...
        _commentsLabel.accessibilityIdentifier = BJLKeypath(self, commentsLabel);
...
- (UILabel *)answerDescriptLabel {
    if (!_answerDescriptLabel) {
        _answerDescriptLabel = [self labelWithTitle:BJLLocalizedString(@"已选") color:[self grayTextColor]];
    }
...
- (UIButton *)finishButton {
    if (!_finishButton) {
        _finishButton = ({
            UIButton *button = [self buttonWithTitle:BJLLocalizedString(@"确定")];
...
- (UIButton *)submitButton {
    if (!_submitButton) {
        _submitButton = [self buttonWithTitle:BJLLocalizedString(@"提交答案")];
