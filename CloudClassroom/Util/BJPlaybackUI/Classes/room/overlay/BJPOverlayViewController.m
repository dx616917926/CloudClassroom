...
@property (nonatomic) UIView *contentView, *backgroundView;
...
- (void)setupSubviews {
    ...
    self.backgroundView = ({
        ...
        view.accessibilityIdentifier = BJLKeypath(self, backgroundView);
...
- (void)showWithChildViewController:(UIViewController *)childViewController
    ...
    if (title.length) {
        [self.titleLabel bjl_updateConstraints:^(BJLConstraintMaker *_Nonnull make) {
            make.top.left.equalTo(self.contentView.bjl_safeAreaLayoutGuide ?: self.contentView).offset(BJPViewSpaceS);
            make.height.equalTo(@(30.0)).priorityHigh();
        }];
    }
        ...
        [self.titleLabel bjl_updateConstraints:^(BJLConstraintMaker *_Nonnull make) {
            make.top.left.equalTo(self.contentView.bjl_safeAreaLayoutGuide ?: self.contentView);
    ...
    if (self.childViewController) {
        [self.childViewController bjl_removeFromParentViewControllerAndSuperiew];
    }
