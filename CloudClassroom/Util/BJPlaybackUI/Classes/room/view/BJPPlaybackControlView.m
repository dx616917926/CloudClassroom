...
@property (nonatomic) BOOL showCatalogue;

@property (nonatomic) BOOL shouldHiddenControlsForinteractive;

@property (nonatomic) NSTimer *timer;
@property (nonatomic, nullable, strong) id<BJLObservation> timerObservation;
...
- (void)setupMediaControlView {
    ...
    // 目录按钮, 约束视横竖屏而定
    [controlView addSubview:self.catalogueButton];
}
...
- (void)addObservers {
    ...
    [self bjl_observe:BJLMakeMethod(self.room, roomDidEnterWithError:)
             observer:^BOOL(BJLError *error) {
                 bjl_strongify(self);
                 if (error) {
                     [self deallocTimerObservation];
                     return YES;
                 }

                 [self deallocTimerObservation];

                 self.timerObservation = [self bjl_kvo:BJLMakeProperty(self, controlsHidden)
...
- (void)deallocTimerObservation {
    if (self.timer.isValid) {
        [self.timer invalidate];
    }
    if (self.timerObservation) {
        [self.timerObservation stopObserving];
        self.timerObservation = nil;
    }
}
...
- (void)updateConstraintsForHorizontal:(BOOL)isHorizontal {
    ...
    if (isHorizontal) {
        ...
        [self.progressView bjl_remakeConstraints:^(BJLConstraintMaker *make) {
            ...
            make.right.equalTo(self.showCatalogue ? self.catalogueButton.bjl_left : self.definitionButton.bjl_left).offset(-20.0);
            make.height.centerY.equalTo(self.mediaControlView);
        }];

        [self.catalogueButton bjl_remakeConstraints:^(BJLConstraintMaker *_Nonnull make) {
            make.width.equalTo(@(self.showCatalogue ? 35.0 : 0.0));
            make.right.equalTo(self.definitionButton.bjl_left).offset(-20.0);
            make.height.centerY.equalTo(self.mediaControlView);
        }];
        if (self.reloadView.hidden == YES) {
            [self autoHideControls];
        }
    }
    else {
...
- (void)setCatalogueHidden:(BOOL)hidden {
    if (hidden != !self.showCatalogue) {
        self.showCatalogue = !hidden;
        [self updateConstraintsForHorizontal:self.isHorizontal];
    }
}
...
- (void)showCatalogueList {
    ...
    if (self.showCatalogueListCallback) {
        self.showCatalogueListCallback();
    }
}
...
- (UIButton *)catalogueButton {
    if (!_catalogueButton) {
        _catalogueButton = ({
            ...
            button.titleLabel.font = [UIFont systemFontOfSize:13.0];
            [button setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
            [button setTitle:BJLLocalizedString(@"目录") forState:UIControlStateNormal];
            [button addTarget:self action:@selector(showCatalogueList) forControlEvents:UIControlEventTouchUpInside];
            button;
        });
    }
