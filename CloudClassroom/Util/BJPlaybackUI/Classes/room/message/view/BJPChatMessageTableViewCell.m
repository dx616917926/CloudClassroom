...
- (void)setupConstraints {
    ...
    // else if BJVMessageImageIdentifier || BJVMessageUploadingImageIdentifier:
...
- (void)_updateLabelsWithMessage:(BJVMessage *)message
                        fromUser:(BJVUser *)fromUser
               shouldHiddenPhone:(BOOL)hidden {
    //    self.nameLabel.text = [fromUser.displayName ?: @"?" stringByAppendingString:@" "];
    NSString *teacherString = [NSString stringWithFormat:BJLLocalizedString(@"%@(老师):"), fromUser.displayName ?: @"?"];
    ...
    if (message.text) {
        self.messageLabel.attributedText = [message attributedEmoticonStringWithEmoticonSize:fontSize + 4.0
                                                                                  attributes:@{NSFontAttributeName: [UIFont systemFontOfSize:fontSize], NSForegroundColorAttributeName: (horizontal ? [UIColor whiteColor] : [UIColor bjp_darkGrayTextColor])}
                                                                                   hidePhone:hidden
                                                                                      cached:YES
                                                                                   cachedKey:(horizontal ? @"white" : @"gray")];
    }
...
- (void)updateWithMessage:(__kindof BJVMessage *)message
    ...
    if (message.type == BJVMessageType_emoticon) {
    ...
    else if (message.type == BJVMessageType_image) {
...
+ (NSString *)cellIdentifierForMessageType:(BJVMessageType)type {
    switch (type) {
        case BJVMessageType_emoticon:
            return BJPMessageEmoticonIdentifier;
        case BJVMessageType_image:
