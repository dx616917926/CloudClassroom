...
/// The attribute type
typedef NS_OPTIONS(NSInteger, BJLYYTextAttributeType) {
    BJLYYTextAttributeTypeNone = 0,
    BJLYYTextAttributeTypeUIKit = 1 << 0, ///< UIKit attributes, such as UILabel/UITextField/drawInRect.
    BJLYYTextAttributeTypeCoreText = 1 << 1, ///< CoreText attributes, used by CoreText.
    BJLYYTextAttributeTypeYYText = 1 << 2, ///< YYText attributes, used by YYText.
};

/// Get the attribute type from an attribute name.
extern BJLYYTextAttributeType BJLYYTextAttributeGetType(NSString *attributeName);

/**
 Line style in YYText (similar to NSUnderlineStyle).
 */
typedef NS_OPTIONS(NSInteger, BJLYYTextLineStyle) {
    // basic style (bitmask:0xFF)
    BJLYYTextLineStyleNone = 0x00, ///< (        ) Do not draw a line (Default).
    BJLYYTextLineStyleSingle = 0x01, ///< (──────) Draw a single line.
    BJLYYTextLineStyleThick = 0x02, ///< (━━━━━━━) Draw a thick line.
    BJLYYTextLineStyleDouble = 0x09, ///< (══════) Draw a double line.

    // style pattern (bitmask:0xF00)
    BJLYYTextLineStylePatternSolid = 0x000, ///< (────────) Draw a solid line (Default).
    BJLYYTextLineStylePatternDot = 0x100, ///< (‑ ‑ ‑ ‑ ‑ ‑) Draw a line of dots.
    BJLYYTextLineStylePatternDash = 0x200, ///< (— — — —) Draw a line of dashes.
    BJLYYTextLineStylePatternDashDot = 0x300, ///< (— ‑ — ‑ — ‑) Draw a line of alternating dashes and dots.
    BJLYYTextLineStylePatternDashDotDot = 0x400, ///< (— ‑ ‑ — ‑ ‑) Draw a line of alternating dashes and two dots.
    BJLYYTextLineStylePatternCircleDot = 0x900, ///< (••••••••••••) Draw a line of small circle dots.
};

/**
 Text vertical alignment.
 */
typedef NS_ENUM(NSInteger, BJLYYTextVerticalAlignment) {
    BJLYYTextVerticalAlignmentTop = 0, ///< Top alignment.
    BJLYYTextVerticalAlignmentCenter = 1, ///< Center alignment.
    BJLYYTextVerticalAlignmentBottom = 2, ///< Bottom alignment.
};

/**
 The direction define in YYText.
 */
typedef NS_OPTIONS(NSUInteger, BJLYYTextDirection) {
    BJLYYTextDirectionNone = 0,
    BJLYYTextDirectionTop = 1 << 0,
    BJLYYTextDirectionRight = 1 << 1,
    BJLYYTextDirectionBottom = 1 << 2,
    BJLYYTextDirectionLeft = 1 << 3,
};

/**
 The trunction type, tells the truncation engine which type of truncation is being requested.
 */
typedef NS_ENUM(NSUInteger, BJLYYTextTruncationType) {
    /// No truncate.
    BJLYYTextTruncationTypeNone = 0,

    /// Truncate at the beginning of the line, leaving the end portion visible.
    BJLYYTextTruncationTypeStart = 1,

    /// Truncate at the end of the line, leaving the start portion visible.
    BJLYYTextTruncationTypeEnd = 2,

    /// Truncate in the middle of the line, leaving both the start and the end portions visible.
    BJLYYTextTruncationTypeMiddle = 3,
};
...
UIKIT_EXTERN NSString *const BJLYYTextBackedStringAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextBindingAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextShadowAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextInnerShadowAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextUnderlineAttributeName;

/// The value of this attribute is a `YYTextDecoration` object.
/// Use this attribute to add strikethrough (delete line) to a range of text.
/// The strikethrough will be drawn above text glyphs.
UIKIT_EXTERN NSString *const BJLYYTextStrikethroughAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextBorderAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextBackgroundBorderAttributeName;

/// The value of this attribute is a `YYTextBorder` object.
/// Use this attribute to add a code block border to one or more line of text.
/// The border will be drawn below the text glyphs.
UIKIT_EXTERN NSString *const BJLYYTextBlockBorderAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextAttachmentAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextHighlightAttributeName;
...
UIKIT_EXTERN NSString *const BJLYYTextGlyphTransformAttributeName;

#pragma mark - String Token Define

UIKIT_EXTERN NSString *const BJLYYTextAttachmentToken; ///< Object replacement character (U+FFFC), used for text attachment.
UIKIT_EXTERN NSString *const BJLYYTextTruncationToken; ///< Horizontal ellipsis (U+2026), used for text truncation  "…".
 ...
 */
typedef void (^BJLYYTextAction)(UIView *containerView, NSAttributedString *text, NSRange range, CGRect rect);
...
/**
 ...
 string under the key named BJLYYTextBackedStringAttributeName).
 
 It may used for copy/paste plain text from attributed string.
 Example: If :) is replace by a custom emoji (such as😊), the backed string can be set to @":)".
 */
@interface BJLYYTextBackedString: NSObject <NSCoding, NSCopying>
...
/**
 ...
 the key named BJLYYTextBindingAttributeName).
 ...
 selection and edit.
 */
@interface BJLYYTextBinding: NSObject <NSCoding, NSCopying>
...
/**
 ...
 as the values for shadow attributes (stored in the attributed string under
 the key named BJLYYTextShadowAttributeName or BJLYYTextInnerShadowAttributeName).
 
 It's similar to `NSShadow`, but offers more options.
 */
@interface BJLYYTextShadow: NSObject <NSCoding, NSCopying>
...
@property (nullable, nonatomic, strong) BJLYYTextShadow *subShadow; ///< a sub shadow which will be added above the parent shadow
...
/**
 ...
 the key named BJLYYTextUnderlineAttributeName or BJLYYTextStrikethroughAttributeName).
 ...
 */
@interface BJLYYTextDecoration: NSObject <NSCoding, NSCopying>
+ (instancetype)decorationWithStyle:(BJLYYTextLineStyle)style;
+ (instancetype)decorationWithStyle:(BJLYYTextLineStyle)style width:(nullable NSNumber *)width color:(nullable UIColor *)color;
@property (nonatomic) BJLYYTextLineStyle style; ///< line style
...
@property (nullable, nonatomic, strong) BJLYYTextShadow *shadow; ///< line shadow
@end
...
/**
 ...
 the key named BJLYYTextBorderAttributeName or BJLYYTextBackgroundBorderAttributeName).
 ...
 Example:
    ...
    ╰──────╯
 */
@interface BJLYYTextBorder: NSObject <NSCoding, NSCopying>
+ (instancetype)borderWithLineStyle:(BJLYYTextLineStyle)lineStyle lineWidth:(CGFloat)width strokeColor:(nullable UIColor *)color;
+ (instancetype)borderWithFillColor:(nullable UIColor *)color cornerRadius:(CGFloat)cornerRadius;
@property (nonatomic) BJLYYTextLineStyle lineStyle; ///< border line style
...
@property (nullable, nonatomic, strong) BJLYYTextShadow *shadow; ///< border shadow
...
/**
 ...
 the key named BJLYYTextAttachmentAttributeName).
 ...
 */
@interface BJLYYTextAttachment: NSObject <NSCoding, NSCopying>
...
/**
 ...
 under the key named BJLYYTextHighlightAttributeName).
 ...
 */
@interface BJLYYTextHighlight: NSObject <NSCoding, NSCopying>
...
- (void)setShadow:(nullable BJLYYTextShadow *)shadow;
- (void)setInnerShadow:(nullable BJLYYTextShadow *)shadow;
- (void)setUnderline:(nullable BJLYYTextDecoration *)underline;
- (void)setStrikethrough:(nullable BJLYYTextDecoration *)strikethrough;
- (void)setBackgroundBorder:(nullable BJLYYTextBorder *)border;
- (void)setBorder:(nullable BJLYYTextBorder *)border;
- (void)setAttachment:(nullable BJLYYTextAttachment *)attachment;
 ...
 */
@property (nullable, nonatomic, copy) BJLYYTextAction tapAction;

/**
 Long press action when user long press the highlight, default is nil.
 If the value is nil, YYTextView or YYLabel will ask it's delegate to handle the long press action.
 */
@property (nullable, nonatomic, copy) BJLYYTextAction longPressAction;
