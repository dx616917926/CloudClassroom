...
#import "BJLYYTextRubyAnnotation.h"
 ...
 */
static CFTypeID BJLCTRunDelegateTypeID() {
...
static CFTypeID BJLCTRubyAnnotationTypeID() {
    static CFTypeID typeID;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
 ...
 */
@interface _BJLYYCGColor: NSObject <NSCopying, NSCoding>
@property (nonatomic, assign) CGColorRef CGColor;
+ (instancetype)colorWithCGColor:(CGColorRef)CGColor;
@end

@implementation _BJLYYCGColor

+ (instancetype)colorWithCGColor:(CGColorRef)CGColor {
    _BJLYYCGColor *color = [self new];
...
- (id)copyWithZone:(NSZone *)zone {
    _BJLYYCGColor *color = [self.class new];
 ...
 */
@interface _BJLYYCGImage: NSObject <NSCoding, NSCopying>
@property (nonatomic, assign) CGImageRef CGImage;
+ (instancetype)imageWithCGImage:(CGImageRef)CGImage;
@end

@implementation _BJLYYCGImage

+ (instancetype)imageWithCGImage:(CGImageRef)CGImage {
    _BJLYYCGImage *image = [self new];
...
- (id)copyWithZone:(NSZone *)zone {
    _BJLYYCGImage *image = [self.class new];
...
@implementation BJLYYTextArchiver

+ (NSData *)archivedDataWithRootObject:(id)rootObject {
    if (!rootObject) return nil;
    NSMutableData *data = [NSMutableData data];
    BJLYYTextArchiver *archiver = [[[self class] alloc] initForWritingWithMutableData:data];
...
- (id)archiver:(NSKeyedArchiver *)archiver willEncodeObject:(id)object {
    ...
    if (typeID == BJLCTRunDelegateTypeID()) {
        CTRunDelegateRef runDelegate = (__bridge CFTypeRef)(object);
        id ref = CTRunDelegateGetRefCon(runDelegate);
        if (ref) return ref;
    }
    else if (typeID == BJLCTRubyAnnotationTypeID()) {
        CTRubyAnnotationRef ctRuby = (__bridge CFTypeRef)(object);
        BJLYYTextRubyAnnotation *ruby = [BJLYYTextRubyAnnotation rubyWithCTRubyRef:ctRuby];
        if (ruby) return ruby;
    }
    else if (typeID == CGColorGetTypeID()) {
        return [_BJLYYCGColor colorWithCGColor:(CGColorRef)object];
    }
    else if (typeID == CGImageGetTypeID()) {
        return [_BJLYYCGImage imageWithCGImage:(CGImageRef)object];
    }
    return object;
}
...
@implementation BJLYYTextUnarchiver

+ (id)unarchiveObjectWithData:(NSData *)data {
    if (data.length == 0) return nil;
    BJLYYTextUnarchiver *unarchiver = [[self alloc] initForReadingWithData:data];
...
- (id)unarchiver:(NSKeyedUnarchiver *)unarchiver didDecodeObject:(id)NS_RELEASES_ARGUMENT object NS_RETURNS_RETAINED {
    if ([object class] == [BJLYYTextRunDelegate class]) {
        BJLYYTextRunDelegate *runDelegate = object;
        CTRunDelegateRef ct = runDelegate.CTRunDelegate;
        id ctObj = (__bridge id)ct;
        if (ct) CFRelease(ct);
        return ctObj;
    }
    else if ([object class] == [BJLYYTextRubyAnnotation class]) {
        BJLYYTextRubyAnnotation *ruby = object;
    ...
    else if ([object class] == [_BJLYYCGColor class]) {
        _BJLYYCGColor *color = object;
        return (id)color.CGColor;
    }
    else if ([object class] == [_BJLYYCGImage class]) {
        _BJLYYCGImage *image = object;
