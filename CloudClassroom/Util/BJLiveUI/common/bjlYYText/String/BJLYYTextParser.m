...
@implementation BJLYYTextSimpleMarkdownParser {
    UIFont *_font;
...
- (void)_updateFonts {
    ...
    _boldFont = BJLYYTextFontWithBold(_font);
    _italicFont = BJLYYTextFontWithItalic(_font);
    _boldItalicFont = BJLYYTextFontWithBoldItalic(_font);
...
- (void)setColorWithBrightTheme {
    ...
    _border = [BJLYYTextBorder new];
    _border.lineStyle = BJLYYTextLineStyleSingle;
    ...
    _border.strokeWidth = BJLYYTextCGFloatFromPixel(1);
}
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    [text bjlyy_removeAttributesInRange:NSMakeRange(0, text.length)];
    text.bjlyy_font = _font;
    text.bjlyy_color = _textColor;
    ...
    [_regexHeader enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location, whiteLen + sharpLen)];
        [text bjlyy_setColor:_headerTextColor range:NSMakeRange(r.location + whiteLen + sharpLen, r.length - whiteLen - sharpLen)];
        [text bjlyy_setFont:_headerFonts[sharpLen - 1] range:result.range];
    }];
    ...
    [_regexH1 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text bjlyy_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text bjlyy_setFont:_headerFonts[0] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text bjlyy_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexH2 enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (linebreak.location != NSNotFound) {
            [text bjlyy_setColor:_headerTextColor range:NSMakeRange(r.location, linebreak.location - r.location)];
            [text bjlyy_setFont:_headerFonts[1] range:NSMakeRange(r.location, linebreak.location - r.location + 1)];
            [text bjlyy_setColor:_controlTextColor range:NSMakeRange(linebreak.location + linebreak.length, r.location + r.length - linebreak.location - linebreak.length)];
        }
    }];
    ...
    [_regexBreakline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        [text bjlyy_setColor:_controlTextColor range:result.range];
    }];
    ...
    [_regexEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location, 1)];
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 1, 1)];
        [text bjlyy_setFont:_italicFont range:NSMakeRange(r.location + 1, r.length - 2)];
    }];
    ...
    [_regexStrong enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text bjlyy_setFont:_boldFont range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrongEmphasis enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location, 3)];
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 3, 3)];
        [text bjlyy_setFont:_boldItalicFont range:NSMakeRange(r.location + 3, r.length - 6)];
    }];
    ...
    [_regexUnderline enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text bjlyy_setTextUnderline:[BJLYYTextDecoration decorationWithStyle:BJLYYTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexStrikethrough enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location, 2)];
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - 2, 2)];
        [text bjlyy_setTextStrikethrough:[BJLYYTextDecoration decorationWithStyle:BJLYYTextLineStyleSingle width:@1 color:nil] range:NSMakeRange(r.location + 2, r.length - 4)];
    }];
    ...
    [_regexInlineCode enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location, len)];
        [text bjlyy_setColor:_controlTextColor range:NSMakeRange(r.location + r.length - len, len)];
        [text bjlyy_setColor:_inlineTextColor range:NSMakeRange(r.location + len, r.length - 2 * len)];
        [text bjlyy_setFont:_monospaceFont range:r];
        [text bjlyy_setTextBorder:_border.copy range:r];
    }];
    ...
    [_regexLink enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text bjlyy_setColor:_linkTextColor range:r];
    }];
    ...
    [_regexLinkRefer enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        NSRange r = result.range;
        [text bjlyy_setColor:_controlTextColor range:r];
    }];
    ...
    [_regexCodeBlock enumerateMatchesInString:str options:0 range:NSMakeRange(0, str.length) usingBlock:^(NSTextCheckingResult *result, NSMatchingFlags flags, BOOL *stop) {
        ...
        if (lenStart + lenEnd < r.length) {
            ...
            [text bjlyy_setColor:_codeTextColor range:codeR];
            [text bjlyy_setFont:_monospaceFont range:codeR];
            BJLYYTextBorder *border = [BJLYYTextBorder new];
            border.lineStyle = BJLYYTextLineStyleSingle;
            ...
            border.strokeWidth = BJLYYTextCGFloatFromPixel(2);
            [text bjlyy_setTextBlockBorder:_border.copy range:codeR];
        }
    }];
    return YES;
}
...
@implementation BJLYYTextSimpleEmoticonParser {
...
- (BOOL)parseText:(NSMutableAttributedString *)text selectedRange:(NSRangePointer)range {
    ...
    for (NSUInteger i = 0, max = matches.count; i < max; i++) {
        ...
        CTFontRef font = (__bridge CTFontRef)([text bjlyy_attribute:NSFontAttributeName atIndex:oneRange.location]);
        if (font) fontSize = CTFontGetSize(font);
        NSMutableAttributedString *atr = [NSAttributedString bjlyy_attachmentStringWithEmojiImage:emoticon fontSize:fontSize];
        [atr bjlyy_setTextBackedString:[BJLYYTextBackedString stringWithString:subStr] range:NSMakeRange(0, atr.length)];
        [text replaceCharactersInRange:oneRange withString:atr.string];
        [text bjlyy_removeDiscontinuousAttributesInRange:NSMakeRange(oneRange.location, atr.length)];
        [text addAttributes:atr.bjlyy_attributes range:NSMakeRange(oneRange.location, atr.length)];
