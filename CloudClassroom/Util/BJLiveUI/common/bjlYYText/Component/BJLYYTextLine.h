...
@class BJLYYTextRunGlyphRange;

NS_ASSUME_NONNULL_BEGIN

/**
 A text line object wrapped `CTLineRef`, see `BJLYYTextLayout` for more.
 */
@interface BJLYYTextLine: NSObject

+ (instancetype)lineWithCTLine:(CTLineRef)CTLine position:(CGPoint)position vertical:(BOOL)isVertical;

@property (nonatomic) NSUInteger index; ///< line index
@property (nonatomic) NSUInteger row; ///< line row
@property (nullable, nonatomic, strong) NSArray<NSArray<BJLYYTextRunGlyphRange *> *> *verticalRotateRange; ///< Run rotate range
...
@property (nullable, nonatomic, readonly) NSArray<BJLYYTextAttachment *> *attachments; ///< YYTextAttachment
...
typedef NS_ENUM(NSUInteger, BJLYYTextRunGlyphDrawMode) {
    /// No rotate.
    BJLYYTextRunGlyphDrawModeHorizontal = 0,

    /// Rotate vertical for single glyph.
    BJLYYTextRunGlyphDrawModeVerticalRotate = 1,

    /// Rotate vertical for single glyph, and move the glyph to a better position,
    /// such as fullwidth punctuation.
    BJLYYTextRunGlyphDrawModeVerticalRotateMove = 2,
};

/**
 A range in CTRun, used for vertical form.
 */
@interface BJLYYTextRunGlyphRange: NSObject
@property (nonatomic) NSRange glyphRangeInRun;
@property (nonatomic) BJLYYTextRunGlyphDrawMode drawMode;
+ (instancetype)rangeWithRange:(NSRange)range drawMode:(BJLYYTextRunGlyphDrawMode)mode;
@end
