...
static inline CGSize YYTextClipCGSize(CGSize size) {
    if (size.width > BJLYYTextContainerMaxSize.width) size.width = BJLYYTextContainerMaxSize.width;
    if (size.height > BJLYYTextContainerMaxSize.height) size.height = BJLYYTextContainerMaxSize.height;
    return size;
}
...
@implementation BJLYYTextLinePositionSimpleModifier
- (void)modifyLines:(NSArray *)lines fromText:(NSAttributedString *)text inContainer:(BJLYYTextContainer *)container {
    if (container.verticalForm) {
        for (NSUInteger i = 0, max = lines.count; i < max; i++) {
            BJLYYTextLine *line = lines[i];
...
- (id)copyWithZone:(NSZone *)zone {
    BJLYYTextLinePositionSimpleModifier *one = [self.class new];
    one.fixedLineHeight = _fixedLineHeight;
    return one;
}
@end

@implementation BJLYYTextContainer {
@package
    BOOL _readonly; ///< used only in BJLYYTextLayout.implementation
    ...
    BJLYYTextTruncationType _truncationType;
    NSAttributedString *_truncationToken;
    id<BJLYYTextLinePositionModifier> _linePositionModifier;
}
...
+ (instancetype)containerWithSize:(CGSize)size insets:(UIEdgeInsets)insets {
    BJLYYTextContainer *one = [self new];
...
- (id)copyWithZone:(NSZone *)zone {
    BJLYYTextContainer *one = [self.class new];
...
#define Setter(...)                                                                                                  \
    if (_readonly) {                                                                                                 \
        @throw [NSException exceptionWithName:NSInternalInconsistencyException                                       \
                                       reason:@"Cannot change the property of the 'container' in 'BJLYYTextLayout'." \
                                     userInfo:nil];                                                                  \
        return;                                                                                                      \
    }                                                                                                                \
    dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);                                                           \
...
- (BJLYYTextTruncationType)truncationType {
    Getter(BJLYYTextTruncationType type = _truncationType) return type;
}

- (void)setTruncationType:(BJLYYTextTruncationType)truncationType {
...
- (void)setLinePositionModifier:(id<BJLYYTextLinePositionModifier>)linePositionModifier {
    Setter(_linePositionModifier = [(NSObject *)linePositionModifier copy]);
}

- (id<BJLYYTextLinePositionModifier>)linePositionModifier {
    Getter(id<BJLYYTextLinePositionModifier> m = _linePositionModifier) return m;
}
...
@interface BJLYYTextLayout ()

@property (nonatomic, readwrite) BJLYYTextContainer *container;
...
@property (nonatomic, readwrite) BJLYYTextLine *truncatedLine;
...
@implementation BJLYYTextLayout
...
+ (BJLYYTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text {
    BJLYYTextContainer *container = [BJLYYTextContainer containerWithSize:size];
    return [self layoutWithContainer:container text:text];
}

+ (BJLYYTextLayout *)layoutWithContainer:(BJLYYTextContainer *)container text:(NSAttributedString *)text {
    return [self layoutWithContainer:container text:text range:NSMakeRange(0, text.length)];
}

+ (BJLYYTextLayout *)layoutWithContainer:(BJLYYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range {
    BJLYYTextLayout *layout = NULL;
    ...
    BJLYYTextLine *truncatedLine = nil;
    ...
    if (needFixJoinedEmojiBug) {
        [((NSMutableAttributedString *)text) bjlyy_setClearColorToJoinedEmoji];
    }

    layout = [[BJLYYTextLayout alloc] _init];
    layout.text = text;
    ...
    if (container.path == nil && container.exclusionPaths.count == 0) {
        ...
        if (needFixLayoutSizeBug) {
            ...
            if (container.isVerticalForm) {
                rect.size.width = BJLYYTextContainerMaxSize.width;
            }
            else {
                rect.size.height = BJLYYTextContainerMaxSize.height;
            }
        }
    ...
    ctFrame = CTFramesetterCreateFrame(ctSetter, BJLYYTextCFRangeFromNSRange(range), cgPath, (CFTypeRef)frameAttrs);
    ...
    for (NSUInteger i = 0; i < lineCount; i++) {
        ...
        BJLYYTextLine *line = [BJLYYTextLine lineWithCTLine:ctLine position:position vertical:isVerticalForm];
        CGRect rect = line.bounds;

        if (constraintSizeIsExtended) {
            if (isVerticalForm) {
                if (rect.origin.x + rect.size.width > constraintRectBeforeExtended.origin.x + constraintRectBeforeExtended.size.width) break;
            }
            else {
    ...
    if (rowCount > 0) {
        if (maximumNumberOfRows > 0) {
            if (rowCount > maximumNumberOfRows) {
                ...
                do {
                    BJLYYTextLine *line = lines.lastObject;
                    if (!line) break;
        ...
        BJLYYTextLine *lastLine = lines.lastObject;
    ...
    { // calculate bounding size
        ...
        if (container.path) {
            ...
            rect = UIEdgeInsetsInsetRect(rect, BJLYYTextUIEdgeInsetsInvert(container.insets));
        }
        rect = CGRectStandardize(rect);
    ...
    visibleRange = BJLYYTextNSRangeFromCFRange(CTFrameGetVisibleStringRange(ctFrame));
    if (needTruncation) {
        BJLYYTextLine *lastLine = lines.lastObject;
        ...
        if (container.truncationType != BJLYYTextTruncationTypeNone) {
            ...
            if (container.truncationToken) {
                ...
                if (runCount > 0) {
                    ...
                    [attrs removeObjectsForKeys:[NSMutableAttributedString bjlyy_allDiscontinuousAttributeKeys]];
                ...
                truncationToken = [[NSAttributedString alloc] initWithString:BJLYYTextTruncationToken attributes:attrs];
                truncationTokenLine = CTLineCreateWithAttributedString((CFAttributedStringRef)truncationToken);
            }
            if (truncationTokenLine) {
                CTLineTruncationType type = kCTLineTruncationEnd;
                if (container.truncationType == BJLYYTextTruncationTypeStart) {
                    type = kCTLineTruncationStart;
                }
                else if (container.truncationType == BJLYYTextTruncationTypeMiddle) {
                ...
                if (ctLastLineExtend) {
                    ...
                    if (ctTruncatedLine) {
                        truncatedLine = [BJLYYTextLine lineWithCTLine:ctTruncatedLine position:lastLine.position vertical:isVerticalForm];
    ...
    if (isVerticalForm) {
        NSCharacterSet *rotateCharset = BJLYYTextVerticalFormRotateCharacterSet();
        NSCharacterSet *rotateMoveCharset = BJLYYTextVerticalFormRotateAndMoveCharacterSet();

        void (^lineBlock)(BJLYYTextLine *) = ^(BJLYYTextLine *line) {
            ...
            for (NSUInteger r = 0; r < runCount; r++) {
                ...
                BOOL isColorGlyph = BJLYYTextCTFontContainsColorBitmapGlyphs(font);

                NSUInteger prevIdx = 0;
                BJLYYTextRunGlyphDrawMode prevMode = BJLYYTextRunGlyphDrawModeHorizontal;
                ...
                for (NSUInteger g = 0; g < glyphCount; g++) {
                    ...
                    BJLYYTextRunGlyphDrawMode mode = glyphRotateMove ? BJLYYTextRunGlyphDrawModeVerticalRotateMove : (glyphRotate ? BJLYYTextRunGlyphDrawModeVerticalRotate : BJLYYTextRunGlyphDrawModeHorizontal);
                    if (g == 0) {
                        prevMode = mode;
                    }
                    else if (mode != prevMode) {
                        BJLYYTextRunGlyphRange *aRange = [BJLYYTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, g - prevIdx) drawMode:prevMode];
                ...
                if (prevIdx < glyphCount) {
                    BJLYYTextRunGlyphRange *aRange = [BJLYYTextRunGlyphRange rangeWithRange:NSMakeRange(prevIdx, glyphCount - prevIdx) drawMode:prevMode];
                    [runRanges addObject:aRange];
                }
            }
        };
        for (BJLYYTextLine *line in lines) {
            lineBlock(line);
        }
    ...
    if (visibleRange.length > 0) {
        ...
        void (^block)(NSDictionary *attrs, NSRange range, BOOL *stop) = ^(NSDictionary *attrs, NSRange range, BOOL *stop) {
            if (attrs[BJLYYTextHighlightAttributeName]) layout.containsHighlight = YES;
            if (attrs[BJLYYTextBlockBorderAttributeName]) layout.needDrawBlockBorder = YES;
            if (attrs[BJLYYTextBackgroundBorderAttributeName]) layout.needDrawBackgroundBorder = YES;
            if (attrs[BJLYYTextShadowAttributeName] || attrs[NSShadowAttributeName]) layout.needDrawShadow = YES;
            if (attrs[BJLYYTextUnderlineAttributeName]) layout.needDrawUnderline = YES;
            if (attrs[BJLYYTextAttachmentAttributeName]) layout.needDrawAttachment = YES;
            if (attrs[BJLYYTextInnerShadowAttributeName]) layout.needDrawInnerShadow = YES;
            if (attrs[BJLYYTextStrikethroughAttributeName]) layout.needDrawStrikethrough = YES;
            if (attrs[BJLYYTextBorderAttributeName]) layout.needDrawBorder = YES;
        };
    ...
    for (NSUInteger i = 0, max = lines.count; i < max; i++) {
        ...
        if (line.attachments.count > 0) {
            ...
            for (BJLYYTextAttachment *attachment in line.attachments) {
...
+ (NSArray *)layoutWithContainers:(NSArray *)containers text:(NSAttributedString *)text range:(NSRange)range {
    ...
    for (NSUInteger i = 0, max = containers.count; i < max; i++) {
        BJLYYTextContainer *container = containers[i];
        BJLYYTextLayout *layout = [self layoutWithContainer:container text:text range:range];
...
- (void)encodeWithCoder:(NSCoder *)aCoder {
    NSData *textData = [BJLYYTextArchiver archivedDataWithRootObject:_text];
...
- (id)initWithCoder:(NSCoder *)aDecoder {
    ...
    NSAttributedString *text = [BJLYYTextUnarchiver unarchiveObjectWithData:textData];
    BJLYYTextContainer *container = [aDecoder decodeObjectForKey:@"container"];
 ...
 */
- (CTRunRef)_runForLine:(BJLYYTextLine *)line position:(BJLYYTextPosition *)position {
    if (!line || !position) return NULL;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger i = 0, max = CFArrayGetCount(runs); i < max; i++) {
        CTRunRef run = CFArrayGetValueAtIndex(runs, i);
        CFRange range = CTRunGetStringRange(run);
        if (position.affinity == BJLYYTextAffinityBackward) {
...
/**
 ...
 @param block    The block to be executed before returns YES.
            ...
            next:  right position
 */
- (BOOL)_insideComposedCharacterSequences:(BJLYYTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
 ...
 */
- (BOOL)_insideEmoji:(BJLYYTextLine *)line position:(NSUInteger)position block:(void (^)(CGFloat left, CGFloat right, NSUInteger prev, NSUInteger next))block {
    if (!line) return NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
    for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
        ...
        if (!BJLYYTextCTFontContainsColorBitmapGlyphs(font)) continue;
...
/**
 ...
 @return YES if RTL.
 */
- (BOOL)_isRightToLeftInLine:(BJLYYTextLine *)line atPoint:(CGPoint)point {
    if (!line) return NO;
    // get write direction
    BOOL RTL = NO;
    CFArrayRef runs = CTLineGetGlyphRuns(line.CTLine);
 ...
 */
- (BJLYYTextRange *)_correctedRangeWithEdge:(BJLYYTextRange *)range {
    NSRange visibleRange = self.visibleRange;
    BJLYYTextPosition *start = range.start;
    BJLYYTextPosition *end = range.end;

    if (start.offset == visibleRange.location && start.affinity == BJLYYTextAffinityBackward) {
        start = [BJLYYTextPosition positionWithOffset:start.offset affinity:BJLYYTextAffinityForward];
    }

    if (end.offset == visibleRange.location + visibleRange.length && start.affinity == BJLYYTextAffinityForward) {
        end = [BJLYYTextPosition positionWithOffset:end.offset affinity:BJLYYTextAffinityBackward];
    }

    if (start != range.start || end != range.end) {
        range = [BJLYYTextRange rangeWithStart:start end:end];
    }
    return range;
}
...
- (NSUInteger)rowIndexForLine:(NSUInteger)line {
    ...
    return ((BJLYYTextLine *)_lines[line]).row;
}
...
- (NSUInteger)lineIndexForPoint:(CGPoint)point {
    ...
    for (NSUInteger i = lineIdx0; i <= lineIdx1; i++) {
        CGRect bounds = ((BJLYYTextLine *)_lines[i]).bounds;
...
- (CGFloat)offsetForTextPosition:(NSUInteger)position lineIndex:(NSUInteger)lineIndex {
    ...
    BJLYYTextLine *line = _lines[lineIndex];
...
- (NSUInteger)textPositionForPoint:(CGPoint)point lineIndex:(NSUInteger)lineIndex {
    ...
    for (NSUInteger r = 0, max = CFArrayGetCount(runs); r < max; r++) {
        ...
        if (range.location <= idx && idx < range.location + range.length) {
            ...
            CFDictionaryRef attrs = CTRunGetAttributes(run);
            CTFontRef font = CFDictionaryGetValue(attrs, kCTFontAttributeName);
            if (!BJLYYTextCTFontContainsColorBitmapGlyphs(font)) break;
...
- (BJLYYTextPosition *)closestPositionToPoint:(CGPoint)point {
    BOOL isVertical = _container.verticalForm;
    ...
    if (position <= _visibleRange.location) {
        return [BJLYYTextPosition positionWithOffset:_visibleRange.location affinity:BJLYYTextAffinityForward];
    }
    else if (position >= _visibleRange.location + _visibleRange.length) {
        return [BJLYYTextPosition positionWithOffset:_visibleRange.location + _visibleRange.length affinity:BJLYYTextAffinityBackward];
    }

    BJLYYTextAffinity finalAffinity = BJLYYTextAffinityForward;
    BOOL finalAffinityDetected = NO;

    // binding range
    NSRange bindingRange;
    BJLYYTextBinding *binding = [_text attribute:BJLYYTextBindingAttributeName atIndex:position longestEffectiveRange:&bindingRange inRange:NSMakeRange(0, _text.length)];
    if (binding && bindingRange.length > 0) {
        NSUInteger headLineIdx = [self lineIndexForPosition:[BJLYYTextPosition positionWithOffset:bindingRange.location]];
        NSUInteger tailLineIdx = [self lineIndexForPosition:[BJLYYTextPosition positionWithOffset:bindingRange.location + bindingRange.length affinity:BJLYYTextAffinityBackward]];
            ...
            if (left != CGFLOAT_MAX && right != CGFLOAT_MAX) {
                if (_container.isVerticalForm) {
                    if (fabs(point.y - left) < fabs(point.y - right)) {
                        ...
                        finalAffinity = BJLYYTextAffinityForward;
                    }
                    else {
                        position = bindingRange.location + bindingRange.length;
                        finalAffinity = BJLYYTextAffinityBackward;
                    }
                }
                else {
    ...
    if (line.range.length == 0) {
        ...
        return [BJLYYTextPosition positionWithOffset:line.range.location affinity:behind ? BJLYYTextAffinityBackward : BJLYYTextAffinityForward];
    }
    ...
    if (line.range.length <= 2) {
        ...
        if (BJLYYTextIsLinebreakString(str)) { // an empty line ("\r", "\n", "\r\n")
            return [BJLYYTextPosition positionWithOffset:line.range.location];
        }
    }
    ...
    if (position >= line.range.location + line.range.length - 1) {
        if (position > line.range.location) {
            ...
            if (BJLYYTextIsLinebreakChar(c1)) {
                position--;
                if (position > line.range.location) {
                    unichar c0 = [_text.string characterAtIndex:position - 1];
                    if (BJLYYTextIsLinebreakChar(c0)) {
                        position--;
                    }
                }
            }
    ...
    if (position == line.range.location) {
        return [BJLYYTextPosition positionWithOffset:position];
    }
    if (position == line.range.location + line.range.length) {
        return [BJLYYTextPosition positionWithOffset:position affinity:BJLYYTextAffinityBackward];
    }
    ...
    if (!finalAffinityDetected) {
        ...
        if (ofs != CGFLOAT_MAX) {
            ...
            if (position >= line.range.location + line.range.length) {
                finalAffinity = RTL ? BJLYYTextAffinityForward : BJLYYTextAffinityBackward;
            }
            else if (position <= line.range.location) {
                finalAffinity = RTL ? BJLYYTextAffinityBackward : BJLYYTextAffinityForward;
            }
            else {
                finalAffinity = (ofs < (isVertical ? point.y : point.x) && !RTL) ? BJLYYTextAffinityForward : BJLYYTextAffinityBackward;
            }
        }
    }

    return [BJLYYTextPosition positionWithOffset:position affinity:finalAffinity];
}

- (BJLYYTextPosition *)positionForPoint:(CGPoint)point
                            oldPosition:(BJLYYTextPosition *)oldPosition
                          otherPosition:(BJLYYTextPosition *)otherPosition {
    if (!oldPosition || !otherPosition) {
        return oldPosition;
    }
    BJLYYTextPosition *newPos = [self closestPositionToPoint:point];
    ...
    if (_container.isVerticalForm) {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            BJLYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionUp offset:1];
            if (range) return range.start;
        }
        else { // search forward
            BJLYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionDown offset:1];
            if (range) return range.end;
        }
    }
    else {
        if ([oldPosition compare:otherPosition] == NSOrderedAscending) { // search backward
            BJLYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionLeft offset:1];
            if (range) return range.start;
        }
        else { // search forward
            BJLYYTextRange *range = [self textRangeByExtendingPosition:otherPosition inDirection:UITextLayoutDirectionRight offset:1];
            if (range) return range.end;
        }
    }
...
- (BJLYYTextRange *)textRangeAtPoint:(CGPoint)point {
    ...
    BJLYYTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    if (_container.verticalForm) {
        BJLYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.y >= point.y && !RTL) ? UITextLayoutDirectionUp : UITextLayoutDirectionDown offset:1];
        return range;
    }
    else {
        BJLYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:(rect.origin.x >= point.x && !RTL) ? UITextLayoutDirectionLeft : UITextLayoutDirectionRight offset:1];
        return range;
    }
}

- (BJLYYTextRange *)closestTextRangeAtPoint:(CGPoint)point {
    BJLYYTextPosition *pos = [self closestPositionToPoint:point];
    if (!pos) return nil;
    ...
    BJLYYTextRange *range = [self textRangeByExtendingPosition:pos inDirection:direction offset:1];
    return range;
}

- (BJLYYTextRange *)textRangeByExtendingPosition:(BJLYYTextPosition *)position {
    ...
    if (position.offset == visibleStart) {
        return [BJLYYTextRange rangeWithRange:NSMakeRange(position.offset, 0)];
    }
    else if (position.offset == visibleEnd) {
        return [BJLYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:BJLYYTextAffinityBackward];
    }

    // binding range
    NSRange tRange;
    BJLYYTextBinding *binding = [_text attribute:BJLYYTextBindingAttributeName atIndex:position.offset longestEffectiveRange:&tRange inRange:_visibleRange];
    if (binding && tRange.length > 0 && tRange.location < position.offset) {
        return [BJLYYTextRange rangeWithRange:tRange];
    }
    ...
    if (lineIndex != NSNotFound) {
        ...
        if (emoji || seq) {
            return [BJLYYTextRange rangeWithRange:NSMakeRange(_prev, _next - _prev)];
        }
    }
    ...
    if (position.offset > visibleStart && position.offset < visibleEnd) {
        ...
        if ((c0 == '\r') && position.offset < visibleEnd) {
            ...
            if (c1 == '\n') {
                return [BJLYYTextRange rangeWithStart:[BJLYYTextPosition positionWithOffset:position.offset - 1] end:[BJLYYTextPosition positionWithOffset:position.offset + 1]];
            }
        }
        if (BJLYYTextIsLinebreakChar(c0) && position.affinity == BJLYYTextAffinityBackward) {
            NSString *str = [_text.string substringToIndex:position.offset];
            NSUInteger len = BJLYYTextLinebreakTailLength(str);
            return [BJLYYTextRange rangeWithStart:[BJLYYTextPosition positionWithOffset:position.offset - len] end:[BJLYYTextPosition positionWithOffset:position.offset]];
        }
    }

    return [BJLYYTextRange rangeWithRange:NSMakeRange(position.offset, 0) affinity:position.affinity];
}
...
- (BJLYYTextRange *)textRangeByExtendingPosition:(BJLYYTextPosition *)position
    ...
    if (!forwardMove && position.offset == visibleStart) {
        return [BJLYYTextRange rangeWithRange:NSMakeRange(_visibleRange.location, 0)];
    }
    ...
    BJLYYTextRange *fromRange = [self textRangeByExtendingPosition:position];
    if (!fromRange) return nil;
    BJLYYTextRange *allForward = [BJLYYTextRange rangeWithStart:fromRange.start end:[BJLYYTextPosition positionWithOffset:visibleEnd]];
    BJLYYTextRange *allBackward = [BJLYYTextRange rangeWithStart:[BJLYYTextPosition positionWithOffset:visibleStart] end:fromRange.end];
    ...
    if (verticalMove) { // up/down in text layout
        ...
        BJLYYTextLine *mostLeftLine = nil, *mostRightLine = nil;
        ...
        BJLYYTextLine *insideLine = _lines[insideIndex];
        NSUInteger pos;
        if (isVerticalForm) {
        ...
        BJLYYTextPosition *extPos;
        if (afinityEdge) {
            if (pos == insideLine.range.location + insideLine.range.length) {
                NSString *subStr = [_text.string substringWithRange:insideLine.range];
                NSUInteger lineBreakLen = BJLYYTextLinebreakTailLength(subStr);
                extPos = [BJLYYTextPosition positionWithOffset:pos - lineBreakLen];
            }
            else {
                extPos = [BJLYYTextPosition positionWithOffset:pos];
            }
        }
        else {
            extPos = [BJLYYTextPosition positionWithOffset:pos];
        }
        BJLYYTextRange *ext = [self textRangeByExtendingPosition:extPos];
        if (!ext) return nil;
        if (forwardMove) {
            return [BJLYYTextRange rangeWithStart:fromRange.start end:ext.end];
        }
        else {
            return [BJLYYTextRange rangeWithStart:ext.start end:fromRange.end];
        }
    }
    else { // left/right in text layout
        BJLYYTextPosition *toPosition = [BJLYYTextPosition positionWithOffset:position.offset + (forwardMove ? offset : -offset)];
        if (toPosition.offset <= visibleStart)
            return allBackward;
        else if (toPosition.offset >= visibleEnd)
            return allForward;

        BJLYYTextRange *toRange = [self textRangeByExtendingPosition:toPosition];
        ...
        return [BJLYYTextRange rangeWithRange:NSMakeRange(start, end - start)];
    }
}

- (NSUInteger)lineIndexForPosition:(BJLYYTextPosition *)position {
...
- (CGPoint)linePositionForPosition:(BJLYYTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)caretRectForPosition:(BJLYYTextPosition *)position {
    NSUInteger lineIndex = [self lineIndexForPosition:position];
...
- (CGRect)firstRectForRange:(BJLYYTextRange *)range {
    ...
    BJLYYTextLine *startLine = _lines[startLineIndex];
    BJLYYTextLine *endLine = _lines[endLineIndex];
    NSMutableArray *lines = [NSMutableArray new];
    for (NSUInteger i = startLineIndex; i <= startLineIndex; i++) {
        BJLYYTextLine *line = _lines[i];
...
- (CGRect)rectForRange:(BJLYYTextRange *)range {
    NSArray *rects = [self selectionRectsForRange:range];
    if (rects.count == 0) return CGRectNull;
    CGRect rectUnion = ((BJLYYTextSelectionRect *)rects.firstObject).rect;
    for (NSUInteger i = 1; i < rects.count; i++) {
        BJLYYTextSelectionRect *rect = rects[i];
        rectUnion = CGRectUnion(rectUnion, rect.rect);
    }
    return rectUnion;
}

- (NSArray *)selectionRectsForRange:(BJLYYTextRange *)range {
    range = [self _correctedRangeWithEdge:range];
    ...
    BJLYYTextSelectionRect *start = [BJLYYTextSelectionRect new];
    if (isVertical) {
    ...
    BJLYYTextSelectionRect *end = [BJLYYTextSelectionRect new];
    if (isVertical) {
    ...
    if (startLine.row == endLine.row) { // same row
        ...
        BJLYYTextSelectionRect *rect = [BJLYYTextSelectionRect new];
        if (isVertical) {
        ...
        BJLYYTextSelectionRect *topRect = [BJLYYTextSelectionRect new];
        ...
        BJLYYTextSelectionRect *bottomRect = [BJLYYTextSelectionRect new];
...
- (NSArray *)selectionRectsWithoutStartAndEndForRange:(BJLYYTextRange *)range {
...
- (NSArray *)selectionRectsWithOnlyStartAndEndForRange:(BJLYYTextRange *)range {
    NSMutableArray *rects = [self selectionRectsForRange:range].mutableCopy;
    for (NSInteger i = 0, max = rects.count; i < max; i++) {
        BJLYYTextSelectionRect *rect = rects[i];
        if (!rect.containsStart && !rect.containsEnd) {
...
typedef NS_OPTIONS(NSUInteger, BJLYYTextDecorationType) {
    BJLYYTextDecorationTypeUnderline = 1 << 0,
    BJLYYTextDecorationTypeStrikethrough = 1 << 1,
};

typedef NS_OPTIONS(NSUInteger, BJLYYTextBorderType) {
    BJLYYTextBorderTypeBackgound = 1 << 0,
    BJLYYTextBorderTypeNormal = 1 << 1,
};
...
static void YYTextDrawRun(BJLYYTextLine *line, CTRunRef run, CGContextRef context, CGSize size, BOOL isVertical, NSArray *runRanges, CGFloat verticalOffset) {
    CGAffineTransform runTextMatrix = CTRunGetTextMatrix(run);
    BOOL runTextMatrixIsID = CGAffineTransformIsIdentity(runTextMatrix);

    CFDictionaryRef runAttrs = CTRunGetAttributes(run);
    NSValue *glyphTransformValue = CFDictionaryGetValue(runAttrs, (__bridge const void *)(BJLYYTextGlyphTransformAttributeName));
    if (!isVertical && !glyphTransformValue) { // draw run
        ...
        CGContextSaveGState(context);
        {
            ...
            if (isVertical) {
                ...
                for (BJLYYTextRunGlyphRange *oneRange in runRanges) {
                    NSRange range = oneRange.glyphRangeInRun;
                    NSUInteger rangeMax = range.location + range.length;
                    BJLYYTextRunGlyphDrawMode mode = oneRange.drawMode;
                    ...
                    for (NSUInteger g = range.location; g < rangeMax; g++) {
                        CGContextSaveGState(context);
                        {
                            ...
                            if (mode) { // CJK glyph, need rotated
                                ...
                                if (mode == BJLYYTextRunGlyphDrawModeVerticalRotateMove) {
                                    x += w;
                                    y += w;
                                }
                                ...
                                CGContextRotateCTM(context, BJLYYTextDegreesToRadians(-90));
                                CGContextSetTextPosition(context,
                                    line.position.y - size.height + glyphPositions[g].x,
                                    line.position.x + verticalOffset + glyphPositions[g].y);
                            }

                            if (BJLYYTextCTFontContainsColorBitmapGlyphs(runFont)) {
...
static void YYTextSetLinePatternInContext(BJLYYTextLineStyle style, CGFloat width, CGFloat phase, CGContextRef context) {
    ...
    else if (pattern == BJLYYTextLineStylePatternCircleDot) {
...
static void YYTextDrawBorderRects(CGContextRef context, CGSize size, BJLYYTextBorder *border, NSArray *rects, BOOL isVertical) {
    if (rects.count == 0) return;

    BJLYYTextShadow *shadow = border.shadow;
    if (shadow.color) {
        CGContextSaveGState(context);
    ...
    for (NSValue *value in rects) {
        ...
        rect = BJLYYTextCGRectPixelRound(rect);
    ...
    if (border.strokeColor && border.lineStyle > 0 && border.strokeWidth > 0) {
        ...
        if ((border.lineStyle & 0xFF) == BJLYYTextLineStyleThick) {
            inset *= 2;
        ...
        //------------------------- second line ------------------------------//
        if ((border.lineStyle & 0xFF) == BJLYYTextLineStyleDouble) {
            CGContextSaveGState(context);
...
static void YYTextDrawLineStyle(CGContextRef context, CGFloat length, CGFloat lineWidth, BJLYYTextLineStyle style, CGPoint position, CGColorRef color, BOOL isVertical) {
    ...
    CGContextSaveGState(context);
    {
        if (isVertical) {
            CGFloat x, y1, y2, w;
            y1 = BJLYYTextCGFloatPixelRound(position.y);
            y2 = BJLYYTextCGFloatPixelRound(position.y + length);
            w = (styleBase == BJLYYTextLineStyleThick ? lineWidth * 2 : lineWidth);

            CGFloat linePixel = BJLYYTextCGFloatToPixel(w);
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    x = BJLYYTextCGFloatPixelHalf(position.x);
                }
                else {
                    x = BJLYYTextCGFloatPixelFloor(position.x);
                }
            }
            else {
                x = position.x;
            ...
            if (styleBase == BJLYYTextLineStyleSingle) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            }
            else if (styleBase == BJLYYTextLineStyleThick) {
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            }
            else if (styleBase == BJLYYTextLineStyleDouble) {
            ...
            CGFloat x1, x2, y, w;
            x1 = BJLYYTextCGFloatPixelRound(position.x);
            x2 = BJLYYTextCGFloatPixelRound(position.x + length);
            w = (styleBase == BJLYYTextLineStyleThick ? lineWidth * 2 : lineWidth);
            ...
            if (fabs(linePixel - floor(linePixel)) < 0.1) {
                int iPixel = linePixel;
                if (iPixel == 0 || (iPixel % 2)) { // odd line pixel
                    y = BJLYYTextCGFloatPixelHalf(position.y);
                }
                else {
                    y = BJLYYTextCGFloatPixelFloor(position.y);
                }
            }
            else {
                y = position.y;
...
static void YYTextDrawText(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    {
...
static void YYTextDrawBlockBorder(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            BJLYYTextBorder *border = attrs[BJLYYTextBlockBorderAttributeName];
            if (!border) continue;

            NSUInteger lineStartIndex = line.index;
            while (lineStartIndex > 0) {
                if (((BJLYYTextLine *)lines[lineStartIndex - 1]).row == line.row)
                    lineStartIndex--;
                else
                    break;
            }
            ...
            NSUInteger lineStartRow = ((BJLYYTextLine *)lines[lineStartIndex]).row;
            NSUInteger lineContinueIndex = lineStartIndex;
            NSUInteger lineContinueRow = lineStartRow;
            do {
                BJLYYTextLine *one = lines[lineContinueIndex];
                ...
                BJLYYTextLine *next = lines[lineContinueIndex + 1];
                if (next.row != lineContinueRow) {
                    BJLYYTextBorder *nextBorder = [layout.text bjlyy_attribute:BJLYYTextBlockBorderAttributeName atIndex:next.range.location];
...
static void YYTextDrawBorder(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BJLYYTextBorderType type, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    ...
    NSArray *lines = layout.lines;
    NSString *borderKey = (type == BJLYYTextBorderTypeNormal ? BJLYYTextBorderAttributeName : BJLYYTextBackgroundBorderAttributeName);
    ...
    for (NSInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            BJLYYTextBorder *border = attrs[borderKey];
            if (!border) continue;
            ...
            for (NSInteger ll = l; ll < lMax; ll++) {
                ...
                for (NSInteger rr = (ll == l) ? r : 0, rrMax = CFArrayGetCount(iRuns); rr < rrMax; rr++) {
                    ...
                    BJLYYTextBorder *iBorder = iAttrs[borderKey];
...
static void YYTextDrawDecoration(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BJLYYTextDecorationType type, BOOL (^cancel)(void)) {
    NSArray *lines = layout.lines;
    ...
    for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            BJLYYTextDecoration *underline = attrs[BJLYYTextUnderlineAttributeName];
            BJLYYTextDecoration *strikethrough = attrs[BJLYYTextStrikethroughAttributeName];

            BOOL needDrawUnderline = NO, needDrawStrikethrough = NO;
            if ((type & BJLYYTextDecorationTypeUnderline) && underline.style > 0) {
                needDrawUnderline = YES;
            }
            if ((type & BJLYYTextDecorationTypeStrikethrough) && strikethrough.style > 0) {
            ...
            if (BJLYYTextIsLinebreakString(runStr)) continue; // may need more checks...
            ...
            if (needDrawUnderline) {
                ...
                BJLYYTextShadow *shadow = underline.shadow;
                while (shadow) {
                    if (!shadow.color) {
...
static void YYTextDrawAttachment(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, UIView *targetView, CALayer *targetLayer, BOOL (^cancel)(void)) {
    BOOL isVertical = layout.container.verticalForm;
    CGFloat verticalOffset = isVertical ? (size.width - layout.container.size.width) : 0;

    for (NSUInteger i = 0, max = layout.attachments.count; i < max; i++) {
        BJLYYTextAttachment *a = layout.attachments[i];
        ...
        CGSize asize = image ? image.size : view ? view.frame.size
                                                 : layer.frame.size;
        ...
        rect = BJLYYTextCGRectFitWithContentMode(rect, asize, a.contentMode);
        rect = BJLYYTextCGRectPixelRound(rect);
        rect = CGRectStandardize(rect);
...
static void YYTextDrawShadow(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    ...
    CGContextSaveGState(context);
    {
        ...
        for (NSUInteger l = 0, lMax = layout.lines.count; l < lMax; l++) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                NSDictionary *attrs = (id)CTRunGetAttributes(run);
                BJLYYTextShadow *shadow = attrs[BJLYYTextShadowAttributeName];
                BJLYYTextShadow *nsShadow = [BJLYYTextShadow shadowWithNSShadow:attrs[NSShadowAttributeName]]; // NSShadow compatible
                if (nsShadow) {
...
static void YYTextDrawInnerShadow(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BOOL (^cancel)(void)) {
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextTranslateCTM(context, 0, size.height);
    CGContextScaleCTM(context, 1, -1);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
            ...
            CGContextSetTextPosition(context, linePosX, linePosY);
            NSDictionary *attrs = (id)CTRunGetAttributes(run);
            BJLYYTextShadow *shadow = attrs[BJLYYTextInnerShadowAttributeName];
            while (shadow) {
                if (!shadow.color) {
                    shadow = shadow.subShadow;
                    continue;
...
static void YYTextDrawDebug(BJLYYTextLayout *layout, CGContextRef context, CGSize size, CGPoint point, BJLYYTextDebugOption *op) {
    UIGraphicsPushContext(context);
    CGContextSaveGState(context);
    CGContextTranslateCTM(context, point.x, point.y);
    CGContextSetLineWidth(context, 1.0 / BJLYYTextScreenScale());
    ...
    if (op.CTFrameBorderColor || op.CTFrameFillColor) {
        ...
        if (!path) {
            ...
            if (op.CTFrameBorderColor)
                rect = BJLYYTextCGRectPixelHalf(rect);
            else
                rect = BJLYYTextCGRectPixelRound(rect);
    ...
    for (NSUInteger l = 0, lMax = lines.count; l < lMax; l++) {
        ...
        if (op.CTLineFillColor) {
            ...
            CGContextAddRect(context, BJLYYTextCGRectPixelRound(lineBounds));
            CGContextFillPath(context);
        }
        if (op.CTLineBorderColor) {
            [op.CTLineBorderColor setStroke];
            CGContextAddRect(context, BJLYYTextCGRectPixelHalf(lineBounds));
            CGContextStrokePath(context);
        }
        if (op.baselineColor) {
            [op.baselineColor setStroke];
            if (isVertical) {
                CGFloat x = BJLYYTextCGFloatPixelHalf(line.position.x);
                CGFloat y1 = BJLYYTextCGFloatPixelHalf(line.top);
                CGFloat y2 = BJLYYTextCGFloatPixelHalf(line.bottom);
                CGContextMoveToPoint(context, x, y1);
                CGContextAddLineToPoint(context, x, y2);
                CGContextStrokePath(context);
            }
            else {
                CGFloat x1 = BJLYYTextCGFloatPixelHalf(lineBounds.origin.x);
                CGFloat x2 = BJLYYTextCGFloatPixelHalf(lineBounds.origin.x + lineBounds.size.width);
                CGFloat y = BJLYYTextCGFloatPixelHalf(line.position.y);
                CGContextMoveToPoint(context, x1, y);
                CGContextAddLineToPoint(context, x2, y);
                CGContextStrokePath(context);
            }
        ...
        if (op.CTLineNumberColor) {
            ...
            num.bjlyy_color = op.CTLineNumberColor;
            num.bjlyy_font = [UIFont systemFontOfSize:6];
        ...
        if (op.CTRunFillColor || op.CTRunBorderColor || op.CTRunNumberColor || op.CGGlyphFillColor || op.CGGlyphBorderColor) {
            ...
            for (NSUInteger r = 0, rMax = CFArrayGetCount(runs); r < rMax; r++) {
                ...
                if (op.CTRunFillColor) {
                    ...
                    CGContextAddRect(context, BJLYYTextCGRectPixelRound(runTypoBounds));
                    CGContextFillPath(context);
                }
                if (op.CTRunBorderColor) {
                    [op.CTRunBorderColor setStroke];
                    CGContextAddRect(context, BJLYYTextCGRectPixelHalf(runTypoBounds));
                    CGContextStrokePath(context);
                }
                if (op.CTRunNumberColor) {
                    ...
                    num.bjlyy_color = op.CTRunNumberColor;
                    num.bjlyy_font = [UIFont systemFontOfSize:6];
                ...
                if (op.CGGlyphBorderColor || op.CGGlyphFillColor) {
                    for (NSUInteger g = 0; g < glyphCount; g++) {
                        ...
                        if (op.CGGlyphFillColor) {
                            ...
                            CGContextAddRect(context, BJLYYTextCGRectPixelRound(rect));
                            CGContextFillPath(context);
                        }
                        if (op.CGGlyphBorderColor) {
                            [op.CGGlyphBorderColor setStroke];
                            CGContextAddRect(context, BJLYYTextCGRectPixelHalf(rect));
                            CGContextStrokePath(context);
                        }
                    }
                }
...
- (void)drawInContext:(CGContextRef)context
                ...
                point:(CGPoint)point
                 view:(UIView *)view
                layer:(CALayer *)layer
                debug:(BJLYYTextDebugOption *)debug
    ...
    @autoreleasepool {
        ...
        if (self.needDrawBackgroundBorder && context) {
            if (cancel && cancel()) return;
            YYTextDrawBorder(self, context, size, point, BJLYYTextBorderTypeBackgound, cancel);
        }
        ...
        if (self.needDrawUnderline && context) {
            if (cancel && cancel()) return;
            YYTextDrawDecoration(self, context, size, point, BJLYYTextDecorationTypeUnderline, cancel);
        }
        ...
        if (self.needDrawStrikethrough && context) {
            if (cancel && cancel()) return;
            YYTextDrawDecoration(self, context, size, point, BJLYYTextDecorationTypeStrikethrough, cancel);
        }
        if (self.needDrawBorder && context) {
            if (cancel && cancel()) return;
            YYTextDrawBorder(self, context, size, point, BJLYYTextBorderTypeNormal, cancel);
        }
...
- (void)removeAttachmentFromViewAndLayer {
    NSAssert([NSThread isMainThread], @"This method must be called on the main thread");
    for (BJLYYTextAttachment *a in self.attachments) {
