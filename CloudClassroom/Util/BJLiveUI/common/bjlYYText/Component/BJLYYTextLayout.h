...
#import "BJLYYTextLine.h"
#import "BJLYYTextInput.h"

@protocol BJLYYTextLinePositionModifier;
 ...
 */
extern const CGSize BJLYYTextContainerMaxSize;

/**
 The BJLYYTextContainer class defines a region in which text is laid out.
 BJLYYTextLayout class uses one or more BJLYYTextContainer objects to generate layouts.
 
 A BJLYYTextContainer defines rectangular regions (`size` and `insets`) or 
 ...
 Example:
     ...
     └─────────────────────────────┘
 */
@interface BJLYYTextContainer: NSObject <NSCoding, NSCopying>
...
/// Creates a container with the specified path. @param path size The path.
+ (instancetype)containerWithPath:(nullable UIBezierPath *)path;

/// The constrained size. (if the size is larger than BJLYYTextContainerMaxSize, it will be clipped)
...
@property BJLYYTextTruncationType truncationType;
...
@property (nullable, copy) id<BJLYYTextLinePositionModifier> linePositionModifier;
@end

/**
 The BJLYYTextLinePositionModifier protocol declares the required method to modify
 the line position in text layout progress. See `BJLYYTextLinePositionSimpleModifier` for example.
 */
@protocol BJLYYTextLinePositionModifier <NSObject, NSCopying>
@required
/**
 ...
 */
- (void)modifyLines:(NSArray<BJLYYTextLine *> *)lines fromText:(NSAttributedString *)text inContainer:(BJLYYTextContainer *)container;
@end

/**
 A simple implementation of `BJLYYTextLinePositionModifier`. It can fix each line's position
 to a specified value, lets each line of height be the same.
 */
@interface BJLYYTextLinePositionSimpleModifier: NSObject <BJLYYTextLinePositionModifier>
@property (assign) CGFloat fixedLineHeight; ///< The fixed line height (distance between two baseline).
@end

/**
 BJLYYTextLayout class is a readonly class stores text layout result.
 ...
 example: (layout with a circle exclusion path)
     ...
     └──────────────────────────┘
 */
@interface BJLYYTextLayout: NSObject <NSCoding>
...
*/
+ (nullable BJLYYTextLayout *)layoutWithContainerSize:(CGSize)size text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable BJLYYTextLayout *)layoutWithContainer:(BJLYYTextContainer *)container text:(NSAttributedString *)text;
...
/**
 ...
 @return A new layout, or nil when an error occurs.
 */
+ (nullable BJLYYTextLayout *)layoutWithContainer:(BJLYYTextContainer *)container text:(NSAttributedString *)text range:(NSRange)range;

/**
 Generate layouts with the given containers and text.
 
 @param containers An array of BJLYYTextContainer object (if nil, returns nil).
 @param text       The text (if nil, returns nil).
 @return An array of BJLYYTextLayout object (the count is same as containers),
    or nil when an error occurs.
 */
+ (nullable NSArray<BJLYYTextLayout *> *)layoutWithContainers:(NSArray<BJLYYTextContainer *> *)containers
...
///< The text container
@property (nonatomic, strong, readonly) BJLYYTextContainer *container;
///< The full text
...
@property (nonatomic, strong, readonly) NSArray<BJLYYTextLine *> *lines;
///< YYTextLine with truncated token, or nil
@property (nullable, nonatomic, strong, readonly) BJLYYTextLine *truncatedLine;
///< Array of `YYTextAttachment`
@property (nullable, nonatomic, strong, readonly) NSArray<BJLYYTextAttachment *> *attachments;
 ...
 */
- (nullable BJLYYTextPosition *)closestPositionToPoint:(CGPoint)point;
...
/**
 ...
 @return A text position, or nil if not found.
 */
- (nullable BJLYYTextPosition *)positionForPoint:(CGPoint)point
 ...
 */
- (nullable BJLYYTextRange *)textRangeAtPoint:(CGPoint)point;
...
/**
 ...
 at point. Or nil if not found.
 */
- (nullable BJLYYTextRange *)closestTextRangeAtPoint:(CGPoint)point;
 ...
 */
- (nullable BJLYYTextRange *)textRangeByExtendingPosition:(BJLYYTextPosition *)position;
...
/**
 ...
 @return An array of `BJLYYTextSelectionRect` objects that encompass the selection.
 If not found, the array is empty.
 */
- (NSArray<BJLYYTextSelectionRect *> *)selectionRectsForRange:(BJLYYTextRange *)range;
...
/**
 ...
 If not found, the array is empty.
 */
- (NSArray<BJLYYTextSelectionRect *> *)selectionRectsWithoutStartAndEndForRange:(BJLYYTextRange *)range;
...
/**
 ...
 @param range An object representing a range in text.
 @return An array of `BJLYYTextSelectionRect` objects contains the start and end to
 the selection. If not found, the array is empty.
 */
- (NSArray<BJLYYTextSelectionRect *> *)selectionRectsWithOnlyStartAndEndForRange:(BJLYYTextRange *)range;
...
- (void)drawInContext:(nullable CGContextRef)context
                ...
                debug:(nullable BJLYYTextDebugOption *)debug
