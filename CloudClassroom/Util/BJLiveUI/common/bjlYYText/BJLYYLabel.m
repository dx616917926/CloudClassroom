...
@interface BJLYYLabel () <BJLYYTextDebugTarget, BJLYYTextAsyncLayerDelegate> {
    NSMutableAttributedString *_innerText; ///< nonnull
    BJLYYTextLayout *_innerLayout;
    BJLYYTextContainer *_innerContainer; ///< nonnull

    NSMutableArray *_attachmentViews;
    NSMutableArray *_attachmentLayers;

    NSRange _highlightRange; ///< current highlight range
    BJLYYTextHighlight *_highlight; ///< highlight attribute in `_highlightRange`
    BJLYYTextLayout *_highlightLayout; ///< when _state.showingHighlight=YES, this layout should be displayed

    BJLYYTextLayout *_shrinkInnerLayout;
    BJLYYTextLayout *_shrinkHighlightLayout;
...
- (void)_updateLayout {
    _innerLayout = [BJLYYTextLayout layoutWithContainer:_innerContainer text:_innerText];
    _shrinkInnerLayout = [BJLYYLabel _shrinkLayoutWithLayout:_innerLayout];
}
...
- (void)_clearInnerLayout {
    ...
    BJLYYTextLayout *layout = _innerLayout;
    _innerLayout = nil;
...
- (BJLYYTextLayout *)_highlightLayout {
    return _shrinkHighlightLayout ? _shrinkHighlightLayout : _highlightLayout;
}

+ (BJLYYTextLayout *)_shrinkLayoutWithLayout:(BJLYYTextLayout *)layout {
    if (layout.text.length && layout.lines.count == 0) {
        BJLYYTextContainer *container = layout.container.copy;
        container.maximumNumberOfRows = 1;
        CGSize containerSize = container.size;
        if (!container.verticalForm) {
            containerSize.height = BJLYYTextContainerMaxSize.height;
        }
        else {
            containerSize.width = BJLYYTextContainerMaxSize.width;
        }
        container.size = containerSize;
        return [BJLYYTextLayout layoutWithContainer:container text:layout.text];
    }
    else {
        return nil;
    }
...
- (void)_startLongPressTimer {
    ...
    _longPressTimer = [NSTimer timerWithTimeInterval:kLongPressMinimumDuration
                                              target:[BJLYYTextWeakProxy proxyWithTarget:self]
...
- (void)_trackDidLongPress {
    ...
    if (_state.hasLongPressAction && _textLongPressAction) {
        ...
        BJLYYTextRange *textRange = [self._innerLayout textRangeAtPoint:point];
    ...
    if (_highlight) {
        BJLYYTextAction longPressAction = _highlight.longPressAction ? _highlight.longPressAction : _highlightLongPressAction;
        if (longPressAction) {
            BJLYYTextPosition *start = [BJLYYTextPosition positionWithOffset:_highlightRange.location];
            BJLYYTextPosition *end = [BJLYYTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:BJLYYTextAffinityBackward];
            BJLYYTextRange *range = [BJLYYTextRange rangeWithStart:start end:end];
...
- (BJLYYTextHighlight *)_getHighlightAtPoint:(CGPoint)point range:(NSRangePointer)range {
    ...
    BJLYYTextHighlight *highlight = [_innerText attribute:BJLYYTextHighlightAttributeName
                                                  atIndex:startIndex
...
- (void)_showHighlightAnimated:(BOOL)animated {
    ...
    if (!_highlightLayout) {
        ...
        [newAttrs enumerateKeysAndObjectsUsingBlock:^(NSString *key, id value, BOOL *stop) {
            [hiText bjlyy_setAttribute:key value:value range:_highlightRange];
        }];
        _highlightLayout = [BJLYYTextLayout layoutWithContainer:_innerContainer text:hiText];
        _shrinkHighlightLayout = [BJLYYLabel _shrinkLayoutWithLayout:_highlightLayout];
...
- (CGPoint)_convertPointToLayout:(CGPoint)point {
    ...
    if (self._innerLayout.container.isVerticalForm) {
        ...
        if (_textVerticalAlignment == BJLYYTextVerticalAlignmentCenter) {
            point.x += (self.bounds.size.width - boundingSize.width) * 0.5;
        }
        else if (_textVerticalAlignment == BJLYYTextVerticalAlignmentBottom) {
...
- (void)_updateOuterLineBreakMode {
    if (_innerContainer.truncationType) {
        switch (_innerContainer.truncationType) {
            case BJLYYTextTruncationTypeStart: {
                _lineBreakMode = NSLineBreakByTruncatingHead;
            } break;
            case BJLYYTextTruncationTypeEnd: {
                _lineBreakMode = NSLineBreakByTruncatingTail;
            } break;
            case BJLYYTextTruncationTypeMiddle: {
        ...
        _lineBreakMode = _innerText.bjlyy_lineBreakMode;
    }
}

- (void)_updateOuterTextProperties {
    _text = [_innerText bjlyy_plainTextForRange:NSMakeRange(0, _innerText.length)];
    _font = _innerText.bjlyy_font;
    if (!_font) _font = [self _defaultFont];
    _textColor = _innerText.bjlyy_color;
    if (!_textColor) _textColor = [UIColor blackColor];
    _textAlignment = _innerText.bjlyy_alignment;
    _lineBreakMode = _innerText.bjlyy_lineBreakMode;
    NSShadow *shadow = _innerText.bjlyy_shadow;
...
- (void)_initLabel {
    ((BJLYYTextAsyncLayer *)self.layer).displaysAsynchronously = NO;
    ...
    _debugOption = [BJLYYTextDebugOption sharedDebugOption];
    [BJLYYTextDebugOption addDebugTarget:self];

    _font = [self _defaultFont];
    _textColor = [UIColor blackColor];
    _textVerticalAlignment = BJLYYTextVerticalAlignmentCenter;
    _numberOfLines = 1;
    _textAlignment = NSTextAlignmentNatural;
    _lineBreakMode = NSLineBreakByTruncatingTail;
    _innerText = [NSMutableAttributedString new];
    _innerContainer = [BJLYYTextContainer new];
    _innerContainer.truncationType = BJLYYTextTruncationTypeEnd;
...
- (void)dealloc {
    [BJLYYTextDebugOption removeDebugTarget:self];
    [_longPressTimer invalidate];
}

+ (Class)layerClass {
    return [BJLYYTextAsyncLayer class];
}
...
- (CGSize)sizeThatFits:(CGSize)size {
    ...
    if (!_verticalForm && size.width <= 0) size.width = BJLYYTextContainerMaxSize.width;
    if (_verticalForm && size.height <= 0) size.height = BJLYYTextContainerMaxSize.height;

    if ((!_verticalForm && size.width == self.bounds.size.width) || (_verticalForm && size.height == self.bounds.size.height)) {
        [self _updateIfNeeded];
        BJLYYTextLayout *layout = self._innerLayout;
        BOOL contains = NO;
    ...
    if (!_verticalForm) {
        size.height = BJLYYTextContainerMaxSize.height;
    }
    else {
        size.width = BJLYYTextContainerMaxSize.width;
    }

    BJLYYTextContainer *container = [_innerContainer copy];
    container.size = size;

    BJLYYTextLayout *layout = [BJLYYTextLayout layoutWithContainer:container text:_innerText];
    return layout.textBoundingSize;
}

- (NSString *)accessibilityLabel {
    return [_innerLayout.text bjlyy_plainTextForRange:_innerLayout.text.bjlyy_rangeOfAll];
}
...
- (instancetype)initWithCoder:(NSCoder *)aDecoder {
    ...
    [self _initLabel];
    BJLYYTextContainer *innerContainer = [aDecoder decodeObjectForKey:@"innerContainer"];
    if (innerContainer) {
...
- (void)touchesMoved:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (_state.trackingTouch) {
        ...
        if (_state.touchMoved && _highlight) {
            BJLYYTextHighlight *highlight = [self _getHighlightAtPoint:point range:NULL];
    ...
    if (_state.trackingTouch) {
        ...
        if (_highlight) {
            if (!_state.touchMoved || [self _getHighlightAtPoint:point range:NULL] == _highlight) {
                BJLYYTextAction tapAction = _highlight.tapAction ? _highlight.tapAction : _highlightTapAction;
                if (tapAction) {
                    BJLYYTextPosition *start = [BJLYYTextPosition positionWithOffset:_highlightRange.location];
                    BJLYYTextPosition *end = [BJLYYTextPosition positionWithOffset:_highlightRange.location + _highlightRange.length affinity:BJLYYTextAffinityBackward];
                    BJLYYTextRange *range = [BJLYYTextRange rangeWithStart:start end:end];
...
- (void)setText:(NSString *)text {
    ...
    [_innerText bjlyy_removeDiscontinuousAttributesInRange:NSMakeRange(0, _innerText.length)];
    if (needAddAttributes) {
        _innerText.bjlyy_font = _font;
        _innerText.bjlyy_color = _textColor;
        _innerText.bjlyy_shadow = [self _shadowFromProperties];
        _innerText.bjlyy_alignment = _textAlignment;
        switch (_lineBreakMode) {
            case NSLineBreakByWordWrapping:
            case NSLineBreakByCharWrapping:
            case NSLineBreakByClipping: {
                _innerText.bjlyy_lineBreakMode = _lineBreakMode;
            } break;
            case NSLineBreakByTruncatingHead:
            case NSLineBreakByTruncatingTail:
            case NSLineBreakByTruncatingMiddle: {
                _innerText.bjlyy_lineBreakMode = NSLineBreakByWordWrapping;
            } break;
            default:
                break;
        }
...
- (void)setTextColor:(UIColor *)textColor {
    ...
    _innerText.bjlyy_color = textColor;
    if (_innerText.length && !_ignoreCommonProperties) {
        if (_displaysAsynchronously && _clearContentsBeforeAsynchronouslyDisplay) {
            [self _clearContents];
        }
...
- (void)setTextAlignment:(NSTextAlignment)textAlignment {
    ...
    _innerText.bjlyy_alignment = textAlignment;
    if (_innerText.length && !_ignoreCommonProperties) {
        if (_displaysAsynchronously && _clearContentsBeforeAsynchronouslyDisplay) {
            [self _clearContents];
        }
...
- (void)setLineBreakMode:(NSLineBreakMode)lineBreakMode {
    ...
    _innerText.bjlyy_lineBreakMode = lineBreakMode;
    // allow multi-line break
    switch (lineBreakMode) {
        case NSLineBreakByWordWrapping:
        case NSLineBreakByCharWrapping:
        case NSLineBreakByClipping: {
            _innerContainer.truncationType = BJLYYTextTruncationTypeNone;
            _innerText.bjlyy_lineBreakMode = lineBreakMode;
        } break;
        case NSLineBreakByTruncatingHead: {
            _innerContainer.truncationType = BJLYYTextTruncationTypeStart;
            _innerText.bjlyy_lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        case NSLineBreakByTruncatingTail: {
            _innerContainer.truncationType = BJLYYTextTruncationTypeEnd;
        ...
        case NSLineBreakByTruncatingMiddle: {
            _innerContainer.truncationType = BJLYYTextTruncationTypeMiddle;
            _innerText.bjlyy_lineBreakMode = NSLineBreakByWordWrapping;
        } break;
        default:
            break;
...
- (void)setTextVerticalAlignment:(BJLYYTextVerticalAlignment)textVerticalAlignment {
...
- (void)setTextParser:(id<BJLYYTextParser>)textParser {
...
- (void)setTextLayout:(BJLYYTextLayout *)textLayout {
...
- (void)setDisplaysAsynchronously:(BOOL)displaysAsynchronously {
    ...
    ((BJLYYTextAsyncLayer *)self.layer).displaysAsynchronously = displaysAsynchronously;
}
...
- (CGSize)intrinsicContentSize {
    if (_preferredMaxLayoutWidth == 0) {
        BJLYYTextContainer *container = [_innerContainer copy];
        container.size = BJLYYTextContainerMaxSize;
...
#pragma mark - BJLYYTextDebugTarget

- (void)setDebugOption:(BJLYYTextDebugOption *)debugOption {
...
#pragma mark - BJLYYTextAsyncLayerDelegate

- (BJLYYTextAsyncLayerDisplayTask *)newAsyncDisplayTask {
    // capture current context
    BOOL contentsNeedFade = _state.contentsNeedFade;
    NSAttributedString *text = _innerText;
    BJLYYTextContainer *container = _innerContainer;
    BJLYYTextVerticalAlignment verticalAlignment = _textVerticalAlignment;
    BJLYYTextDebugOption *debug = _debugOption;
    ...
    __block BJLYYTextLayout *layout = (_state.showingHighlight && _highlightLayout) ? self._highlightLayout : self._innerLayout;
    __block BJLYYTextLayout *shrinkLayout = nil;
    ...
    // create display task
    BJLYYTextAsyncLayerDisplayTask *task = [BJLYYTextAsyncLayerDisplayTask new];
    ...
    task.display = ^(CGContextRef context, CGSize size, BOOL (^isCancelled)(void)) {
        ...
        BJLYYTextLayout *drawLayout = layout;
        if (layoutNeedUpdate) {
            layout = [BJLYYTextLayout layoutWithContainer:container text:text];
            shrinkLayout = [BJLYYLabel _shrinkLayoutWithLayout:layout];
            if (isCancelled()) return;
            layoutUpdated = YES;
        ...
        if (verticalAlignment == BJLYYTextVerticalAlignmentCenter) {
        ...
        else if (verticalAlignment == BJLYYTextVerticalAlignmentBottom) {
            if (drawLayout.container.isVerticalForm) {
        ...
        point = BJLYYTextCGPointPixelRound(point);
    ...
    task.didDisplay = ^(CALayer *layer, BOOL finished) {
        ...
        if (!finished) {
            ...
            for (BJLYYTextAttachment *a in drawLayout.attachments) {
        ...
        __strong BJLYYLabel *view = (BJLYYLabel *)layer.delegate;
        if (!view) return;
...
@interface BJLYYLabel (IBInspectableProperties)
@end

@implementation BJLYYLabel (IBInspectableProperties)
...
- (void)setDebugEnabled_:(BOOL)enabled {
    if (!enabled) {
        ...
        BJLYYTextDebugOption *debugOption = [BJLYYTextDebugOption new];
