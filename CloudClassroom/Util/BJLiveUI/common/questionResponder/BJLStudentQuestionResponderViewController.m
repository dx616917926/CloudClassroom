//
//  BJLStudentQuestionResponderViewController.m
//  BJLiveUI
//
...
static const CGFloat BJLStudentQuestionRespondeButtonIpadWidth = 100.0f;

@interface BJLStudentQuestionResponderViewController ()
...
@implementation BJLStudentQuestionResponderViewController
...
- (void)makeSubviews {
    ...
    self.countDownContainerView = ({
        ...
        view.accessibilityIdentifier = BJLKeypath(self, countDownContainerView);
    ...
    self.countDownLabel = ({
        ...
        label.accessibilityIdentifier = BJLKeypath(self, countDownLabel);
    ...
    self.tipLabel = ({
        ...
        label.textColor = BJLTheme.viewTextColor;
        label.textAlignment = NSTextAlignmentCenter;
        label.text = BJLLocalizedString(@"倒计时结束开始抢答");
        label.accessibilityIdentifier = BJLKeypath(self, tipLabel);
        [self.countDownContainerView addSubview:label];
        bjl_return label;
    });
    ...
    self.responderButton = ({
        ...
        button.accessibilityIdentifier = BJLKeypath(self, responderButton);
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionResponder_normal"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionResponder_highlight"] forState:UIControlStateHighlighted];
        button.hidden = YES;
...
- (void)makeObserving {
    ...
    [self bjl_observe:BJLMakeMethod(self.room.roomVM, didReceiveRevokeQuestionResponder) observer:^BOOL {
        ...
        if (self.errorCallback) {
            self.errorCallback(BJLLocalizedString(@"抢答已取消"));
        }
    ...
    [self bjl_observe:BJLMakeMethod(self.room.roomVM, didReceiveEndQuestionResponderWithWinner:) observer:^BOOL(BJLUser *user) {
        ...
        if (!user) {
            if (self.errorCallback) {
                self.errorCallback(BJLLocalizedString(@"无人抢中"));
            }
            [self hide];
            return NO;
        }
        ...
        if (self.room.roomInfo.roomType == BJLRoomType_1vNClass) {
            if (self.room.roomInfo.newRoomGroupType == BJLRoomNewGroupType_onlinedoubleTeachers
                || self.room.roomInfo.newRoomGroupType == BJLRoomNewGroupType_group) {
                // 可见所有成员时，或者是同组学生
                BOOL notify = self.room.featureConfig.enableShowAllGroupMember || (user.groupID == self.room.loginUser.groupID);
                if (!notify) {
                    if (self.errorCallback) {
                        self.errorCallback(BJLLocalizedString(@"无人抢中"));
                    }
        ...
        if (!self.responderButton.isHidden && [self.room.loginUser.ID isEqualToString:user.ID]) {
            ...
            [self.responderButton setImage:[UIImage bjl_imageNamed:@"bjl_questionResponder_success"] forState:UIControlStateNormal];
            ...
            [self.responderButton setImage:[UIImage bjl_imageNamed:@"bjl_questionResponder_failed"] forState:UIControlStateNormal];
            dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(3 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
                [self hide];
            });
        }
...
- (void)startResponderTimer {
    ...
    bjl_weakify(self);
    self.respoderTimer = [NSTimer scheduledTimerWithTimeInterval:2 repeats:YES block:^(NSTimer *_Nonnull timer) {
        bjl_strongify(self);
...
- (void)updateResponderFrame {
    CGFloat width = BJLStudentQuestionRespondeButtonIpadWidth;
