//
//  BJLQuestionAnswerViewController.m
//  BJLiveUI
//
...
#import "BJLQuestionAnswerViewController+protected.h"
...
#import "BJLQuestionAnswerViewController+statistics.h"
...
@interface BJLQuestionAnswerViewController () <UITextViewDelegate, UITextFieldDelegate, UICollectionViewDelegate, UICollectionViewDataSource, UITableViewDelegate, UITableViewDataSource>
@property (nonatomic) BJLQuestionAnswerWindowLayout layout;
...
@implementation BJLQuestionAnswerViewController
...
- (instancetype)initWithRoom:(BJLRoom *)room
    ...
    if (self) {
        ...
        self.countDownTime = (layout == BJLQuestionAnswerWindowLayout_publish) ? self.answerSheet.duration : 0;
        self.onlineUserList = [NSMutableArray new];
        self.judgeTitleArray = [NSMutableArray arrayWithArray:@[
            @{@"1": BJLLocalizedString(@"对"), @"0": BJLLocalizedString(@"错")},
        ]];
    }
    return self;
}
...
- (void)makeEditContainView {
    ...
    self.editContainerView = ({
        UIView *view = [BJLHitTestView viewWithHitTestBlock:^UIView *_Nullable(UIView *_Nullable hitView, CGPoint point, UIEvent *_Nullable event) {
            bjl_strongify(self);
            //当自定义判断题的题目比较多的时候，table view的长度会超过父容器，导致列表底部的点击事件收不到
            //这里需要特殊处理一下，把点击事件直接转发到table view中
            CGRect tableViewRect = [self.editContainerView convertRect:self.judgeTitleTableView.frame fromView:self.judgeTitleTableView];
            if (!self.judgeTitleTableView.hidden && CGRectContainsPoint(tableViewRect, point)) {
                return self.judgeTitleTableView;
            }
    ...
    self.topChoosenContainView = ({
        ...
        view.accessibilityIdentifier = BJLKeypath(self, topChoosenContainView);
        view.backgroundColor = [UIColor clearColor];
    ...
    UILabel *label = ({
        ...
        label.text = BJLLocalizedString(@"类型:");
    ...
    self.choosenButton = ({
        ...
        [button setTitle:BJLLocalizedString(@"选择题") forState:UIControlStateNormal];
        ...
        [button setBackgroundColor:[BJLTheme brandColor]];
        [button.titleLabel setFont:[UIFont systemFontOfSize:self.isPortrait ? 12.0 : 14.0]];
        button.layer.cornerRadius = BJLAppearance.cornerRadius;
        button.accessibilityIdentifier = BJLKeypath(self, choosenButton);
    ...
    self.judgementButton = ({
        UIButton *button = [UIButton new];
        [button setTitle:BJLLocalizedString(@"判断题") forState:UIControlStateNormal];
        [button setBackgroundColor:[BJLTheme subButtonBackgroundColor]];
        [button setTitleColor:BJLTheme.buttonTextColor forState:UIControlStateSelected];
        [button setTitleColor:BJLTheme.subButtonTextColor forState:UIControlStateNormal];
        [button.titleLabel setFont:[UIFont systemFontOfSize:self.isPortrait ? 12.0 : 14.0]];
        button.layer.cornerRadius = BJLAppearance.cornerRadius;
        button.accessibilityIdentifier = BJLKeypath(self, judgementButton);
    ...
    self.shuoldShowCorrectAnswerButton = ({
        ...
        button.accessibilityIdentifier = BJLKeypath(self, shuoldShowCorrectAnswerButton);
        [button setTitle:BJLLocalizedString(@"答题结束公布正确答案") forState:UIControlStateNormal];
        [button setTitleColor:BJLTheme.viewTextColor forState:UIControlStateSelected];
        [button setTitleColor:BJLTheme.viewTextColor forState:UIControlStateNormal];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionAnswer_notShow"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionAnswer_show"] forState:UIControlStateSelected];
    ...
    [self.topChoosenContainView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.top.equalTo(self.editContainerView).offset(BJLAppearance.userWindowDefaultBarHeight);
    ...
    [self.choosenButton bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.centerY.equalTo(self.topChoosenContainView);
        make.top.bottom.equalTo(self.topChoosenContainView);
        make.left.equalTo(self.isPortrait ? self.topChoosenContainView : label.bjl_right).offset(10);
        make.width.equalTo(@(self.isPortrait ? 64.0 : 80));
    }];
    [self.judgementButton bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.centerY.equalTo(self.choosenButton);
        make.top.bottom.equalTo(self.topChoosenContainView);
    ...
    [self.shuoldShowCorrectAnswerButton bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.centerY.equalTo(self.judgementButton);
        make.top.bottom.equalTo(self.topChoosenContainView);
    ...
    self.judgementContainView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, judgementContainView);
        view.backgroundColor = [UIColor clearColor];
    ...
    self.judgementTitleContainView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, judgementTitleContainView);
        view.backgroundColor = [UIColor clearColor];
    ...
    view.alpha = 0.1;
    view.layer.cornerRadius = BJLAppearance.cornerRadius;
    view.layer.masksToBounds = YES;
    [self.judgementContainView addSubview:view];

    self.judgeTitleButton = ({
        UIButton *button = [UIButton new];
        [button setTitle:BJLLocalizedString(@"对/错") forState:UIControlStateNormal];
        [button.titleLabel setFont:[UIFont systemFontOfSize:12]];
        [button setTitleColor:BJLTheme.viewTextColor forState:UIControlStateNormal];
        button.titleLabel.textAlignment = NSTextAlignmentLeft;
        button.titleEdgeInsets = UIEdgeInsetsMake(0, 10, 0, 30);
        button.contentHorizontalAlignment = UIControlContentHorizontalAlignmentLeft;
        button.accessibilityIdentifier = BJLKeypath(self, judgeTitleButton);
        ...
        self.judgeTitleIconButton = ({
            ...
            [iconButton setImage:[UIImage bjl_imageNamed:@"bjl_questionResponder_open"] forState:UIControlStateNormal];
            [iconButton setImage:[UIImage bjl_imageNamed:@"bjl_questionResponder_close"] forState:UIControlStateSelected];
    ...
    self.judgeTitleTableView = ({
        ...
        tableView.layer.borderColor = BJLTheme.buttonBorderColor.CGColor;
    ...
    self.rightButtonView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, rightButtonView);
        [self.judgementContainView addSubview:view];
    ...
    self.wrongButtonView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, wrongButtonView);
        [self.judgementContainView addSubview:view];
    ...
    self.rightTextField = ({
        ...
        textField.accessibilityIdentifier = BJLKeypath(self, rightTextField);
        ...
        textField.text = BJLLocalizedString(@"对");
    ...
    self.wrongTextField = ({
        BJLTextField *textField = [BJLTextField new];
        textField.accessibilityIdentifier = BJLKeypath(self, wrongTextField);
        textField.textColor = BJLTheme.viewTextColor;
        textField.font = [UIFont systemFontOfSize:12];
        textField.textAlignment = NSTextAlignmentCenter;
        textField.textInsets = textField.editingInsets = UIEdgeInsetsMake(0, 0, 0, 0);
        textField.text = BJLLocalizedString(@"错");
        textField.delegate = self;
        textField.userInteractionEnabled = NO;
        [self.judgementContainView addSubview:textField];
        bjl_return textField;
    ...
    [self.judgementContainView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        ...
        make.height.equalTo(@([self answerCollectionContainerHeight:5 ipad:15]));
    }];
    ...
    [self.rightButtonView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.width.height.equalTo(@(BJLAppearance.questionAnswerOptionButtonHeight));
    ...
    self.optionsContainView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, optionsContainView);
        view.backgroundColor = [UIColor clearColor];
        [self.editContainerView addSubview:view];
        bjl_return view;
    });
    ...
    self.minusOptionButton = ({
        UIButton *button = [UIButton new];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionAnswer_option_reduce"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionAnswer_option_reduce_disable"] forState:UIControlStateDisabled];
        button.accessibilityIdentifier = BJLKeypath(self, minusOptionButton);
    ...
    self.plusOptionButton = ({
        UIButton *button = [UIButton new];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionAnswer_option_add"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_questionAnswer_option_add_disable"] forState:UIControlStateDisabled];
        button.accessibilityIdentifier = BJLKeypath(self, plusOptionButton);
    ...
    if (self.answerSheet.answerType == BJLAnswerSheetType_Choosen) {
        optionsViewHeight = [self answerCollectionViewHeight:10 ipad:25];
    ...
    [self.optionsContainView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        ...
        make.height.equalTo(@([self answerCollectionContainerHeight:10 ipad:15]));
    }];
    ...
    self.commentsContainView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, commentsContainView);
        view.backgroundColor = [UIColor clearColor];
        [self.editContainerView addSubview:view];
        bjl_return view;
    });
    ...
    self.optionTipLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"点击选项可设置为正确答案");
        label.textColor = [BJLTheme viewSubTextColor];
        label.font = [UIFont systemFontOfSize:14];
        label.textAlignment = NSTextAlignmentCenter;
        label.accessibilityIdentifier = BJLKeypath(self, optionTipLabel);
    ...
    self.commentsTextView = ({
        ...
        textView.bjl_placeholderLabel.text = BJLLocalizedString(@"您可以在这里添加说明...");
        textView.bjl_placeholderLabel.textColor = [BJLTheme viewSubTextColor];
        ...
        textView.layer.cornerRadius = BJLAppearance.cornerRadius;
        textView.layer.borderColor = BJLTheme.buttonBorderColor.CGColor;
        textView.textColor = BJLTheme.viewTextColor;
        textView.font = [UIFont systemFontOfSize:12];
        textView.layer.borderWidth = onePixel;
        textView.accessibilityIdentifier = BJLKeypath(self, commentsTextView);
    ...
    self.commentsTextCountLabel = ({
        ...
        label.accessibilityIdentifier = BJLKeypath(self, commentsTextCountLabel);
        [self.commentsContainView addSubview:label];
        bjl_return label;
    });
    ...
    [self.commentsContainView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        ...
        make.right.equalTo(self.editContainerView).offset(-10);
        make.bottom.lessThanOrEqualTo(self.editContainerView).offset(-10);
    }];
    ...
    UILabel *leftLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"计时: ");
        label.textColor = [BJLTheme viewSubTextColor];
        label.font = [UIFont systemFontOfSize:14];
        label.textAlignment = NSTextAlignmentRight;
    ...
    UILabel *rightLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"分后结束答题");
        label.textColor = [BJLTheme viewSubTextColor];
        label.font = [UIFont systemFontOfSize:14];
        label.textAlignment = NSTextAlignmentLeft;
        [self.bottomEditContainerView addSubview:label];
    ...
    self.publishButton = ({
        ...
        button.titleLabel.font = [UIFont systemFontOfSize:self.isPortrait ? 12.0 : 14.0];
    ...
    [self.publishButton bjl_remakeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        ...
        make.width.equalTo(@(self.isPortrait ? 64.0 : 80.0));
    }];
...
- (void)makePublishContainView {
    self.publishContainerView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, publishContainerView);
        view.backgroundColor = [UIColor clearColor];
        [self.view addSubview:view];
        bjl_return view;
    });

    [self.publishContainerView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.edges.equalTo(self.editContainerView);
    }];

    self.countDownTimeLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"结束倒计时: ");
        label.textColor = BJLTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:14];
        label.textAlignment = NSTextAlignmentLeft;
    ...
    self.answerSituationLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"发布：0人，提交：0人 ");
        label.textColor = BJLTheme.viewSubTextColor;
        label.font = [UIFont systemFontOfSize:14];
        label.textAlignment = NSTextAlignmentCenter;
        label.accessibilityIdentifier = BJLKeypath(self, answerSituationLabel);
        [self.publishContainerView addSubview:label];
        bjl_return label;
    });
    ...
    self.publishedCommentsLabel = ({
        ...
        label.accessibilityIdentifier = BJLKeypath(self, publishedCommentsLabel);
        [self.publishContainerView addSubview:label];
        bjl_return label;
    });
    ...
    self.answerOptionsCollectionView = ({
        ...
        layout.sectionInset = UIEdgeInsetsZero;
        layout.scrollDirection = UICollectionViewScrollDirectionVertical;
        layout.itemSize = CGSizeMake(BJLAppearance.questionAnswerOptionButtonWidth, 75);
        layout.minimumInteritemSpacing = isIphone ? 10 : 25;
        // view
        UICollectionView *view = [[UICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        view.backgroundColor = [UIColor clearColor];
        ...
        view.dataSource = self;
        view.delegate = self;
        view.accessibilityIdentifier = BJLKeypath(self, answerOptionsCollectionView);
        [view registerClass:[BJLQuestionAnswerPublishedOptionCollectionViewCell class] forCellWithReuseIdentifier:BJLQuestionAnswerPublishedOptionCollectionViewCell_ChoosenCell];
        [view registerClass:[BJLQuestionAnswerPublishedOptionCollectionViewCell class] forCellWithReuseIdentifier:BJLQuestionAnswerPublishedOptionCollectionViewCell_JudgeCell_right];
        [view registerClass:[BJLQuestionAnswerPublishedOptionCollectionViewCell class] forCellWithReuseIdentifier:BJLQuestionAnswerPublishedOptionCollectionViewCell_JudgeCell_wrong];
        [self.publishContainerView addSubview:view];
        bjl_return view;
    });

    [self.countDownTimeLabel bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.top.equalTo(self.publishContainerView).offset(16.0);
        make.left.equalTo(self.publishContainerView.bjl_left).offset(10);
        make.right.equalTo(self.publishContainerView).offset(-10);
        make.height.equalTo(@(18));
    }];

    CGFloat itemWidth = BJLAppearance.questionAnswerOptionButtonHeight + 15;
    CGFloat optionsViewWidth = (itemWidth * 4 + (isIphone ? 10 : 25) * 3);
    NSInteger row = 2;
    if ([self.answerSheet.options count] <= 4) {
        row = 1;
        optionsViewWidth = (itemWidth * [self.answerSheet.options count]) + (isIphone ? 10 : 25) * (MAX([self.answerSheet.options count], 2) - 1);
    }
    NSInteger optionsViewHeight = 75 * row;
    [self.answerOptionsCollectionView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.center.equalTo(self.publishContainerView);
        make.height.equalTo(@(optionsViewHeight));
        make.width.equalTo(@(optionsViewWidth));
    }];
    ...
    [self.publishedCommentsLabel bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        ...
        make.left.equalTo(self.publishContainerView).offset(10);
        make.right.equalTo(self.publishContainerView).offset(-10);
        make.bottom.lessThanOrEqualTo(self.publishContainerView.bjl_bottom).offset(-10.0);
    }];

    self.bottomPublishContainerView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, bottomPublishContainerView);
        view.backgroundColor = [UIColor clearColor];
        view.hidden = YES;
        [self.bottomBar addSubview:view];
        bjl_return view;
...
- (void)makeEndContainView {
    self.endContainerView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, endContainerView);
        view.backgroundColor = [UIColor clearColor];
        [self.view addSubview:view];
        bjl_return view;
    });
    [self.endContainerView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.edges.equalTo(self.publishContainerView);
    }];
    ...
    self.detailButton = ({
        ...
        button.layer.cornerRadius = BJLAppearance.cornerRadius;
        button.layer.masksToBounds = YES;
        button.accessibilityIdentifier = BJLKeypath(self, detailButton);
        button.titleLabel.font = [UIFont systemFontOfSize:12.0];
        [button setTitle:BJLLocalizedString(@"查看详情") forState:UIControlStateNormal];
        [button setTitleColor:BJLTheme.buttonTextColor forState:UIControlStateNormal];
        [button setBackgroundColor:[BJLTheme buttonBorderColor]];
    ...
    self.answerUseTimeLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"用时：0分0秒 ");
        label.textColor = BJLTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:12];
        label.textAlignment = NSTextAlignmentLeft;
        label.accessibilityIdentifier = BJLKeypath(self, answerUseTimeLabel);
    ...
    self.publishNumberLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"发布：0人 ");
        label.textColor = BJLTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:12];
        label.textAlignment = NSTextAlignmentLeft;
        label.accessibilityIdentifier = BJLKeypath(self, publishNumberLabel);
        [self.endContainerView addSubview:label];
        bjl_return label;
    });
    ...
    self.chartContainView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, chartContainView);
        view.backgroundColor = [UIColor clearColor];
    ...
    self.endCommentsLabel = ({
        ...
        label.textAlignment = NSTextAlignmentCenter;
        label.numberOfLines = 0;
        label.accessibilityIdentifier = BJLKeypath(self, endCommentsLabel);
    ...
    self.statisticsLine = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, statisticsLine);
    ...
    self.participatedNumberLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"答题：0人 ");
        label.textColor = BJLTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:12];
        label.textAlignment = NSTextAlignmentRight;
        label.accessibilityIdentifier = BJLKeypath(self, publishNumberLabel);
    ...
    self.correctRateLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"正确率：0% ");
        label.textColor = BJLTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:12];
        label.textAlignment = NSTextAlignmentLeft;
        label.accessibilityIdentifier = BJLKeypath(self, correctRateLabel);
        [self.chartContainView addSubview:label];
        bjl_return label;
    });

    [self.answerUseTimeLabel bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.top.equalTo(self.endContainerView).offset(BJLAppearance.userWindowDefaultBarHeight);
    ...
    self.bottomEndContainerView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, bottomEndContainerView);
        view.backgroundColor = [UIColor clearColor];
        view.hidden = YES;
        [self.bottomBar addSubview:view];
        bjl_return view;
    ...
    self.backButton = ({
        UIButton *button = [UIButton new];
        button.accessibilityIdentifier = BJLKeypath(self, backButton);
        button.titleLabel.font = [UIFont systemFontOfSize:14.0];
        [button setTitle:BJLLocalizedString(@"返回") forState:UIControlStateNormal];
        [button setTitleColor:BJLTheme.viewTextColor forState:UIControlStateNormal];
    ...
    self.correctAnswerLabel = ({
        ...
        label.textAlignment = NSTextAlignmentLeft;
        label.hidden = YES;
        label.accessibilityIdentifier = BJLKeypath(self, correctAnswerLabel);
        [self.bottomEndContainerView addSubview:label];
        bjl_return label;
    });
    ...
    [self.answerLabel bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.centerY.height.equalTo(self.correctAnswerLabel);
        make.right.equalTo(self.correctAnswerLabel.bjl_left).offset(5.0);
    }];

    self.detailContainerView = ({
        UIView *view = [BJLHitTestView new];
        view.accessibilityIdentifier = BJLKeypath(self, detailContainerView);
        [self.view addSubview:view];
        view;
    });
    [self.detailContainerView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.edges.equalTo(self.endContainerView);
    }];
    ...
    self.detailTableView = ({
        ...
        tableView.tableFooterView = [UIView new];
        tableView.separatorStyle = UITableViewCellSeparatorStyleNone;
        [tableView registerClass:[BJLQuestionAnswerSheetUserDetailTableViewCell class] forCellReuseIdentifier:detailTableViewCellIdentifier];
        [self.detailContainerView addSubview:tableView];
        bjl_return tableView;
    });
    [self.detailTableView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.top.equalTo(self.detailContainerView).offset(BJLAppearance.userWindowDefaultBarHeight);
...
- (void)updateContentViewAndBottomView {
    if (self.layout == BJLQuestionAnswerWindowLayout_normal) {
        [self upadteQuestionTypeButton];
        [self updateEditContainerView];
    }
    else if (self.layout == BJLQuestionAnswerWindowLayout_publish) {
        [self updatePublishContainerView];
    }
    else if (self.layout == BJLQuestionAnswerWindowLayout_end) {
        [self updateEndContainerView];
    }

    self.bottomEditContainerView.hidden = (self.layout != BJLQuestionAnswerWindowLayout_normal);
    self.bottomPublishContainerView.hidden = (self.layout != BJLQuestionAnswerWindowLayout_publish);
    self.bottomEndContainerView.hidden = (self.layout != BJLQuestionAnswerWindowLayout_end);
}

- (void)updatePublishContainerView {
    self.editContainerView.hidden = YES;
    self.publishContainerView.hidden = NO;
    self.endContainerView.hidden = YES;
    self.detailContainerView.hidden = YES;

    self.answerSituationLabel.text = [NSString stringWithFormat:BJLLocalizedString(@"发布：%td人，提交：%td人"), self.answerSheet.userCountParticipate, self.answerSheet.userCountSubmit];
    ...
    self.answerOptionsCollectionView.showsVerticalScrollIndicator = self.answerSheet.options.count > 8;
...
- (void)updateEndContainerView {
    self.editContainerView.hidden = YES;
    self.publishContainerView.hidden = YES;
    self.endContainerView.hidden = NO;
    self.detailContainerView.hidden = YES;
    ...
    self.answerUseTimeLabel.text = [NSString stringWithFormat:BJLLocalizedString(@"用时：%02i:%02i"), minutes, second];
    //发布人数
    self.publishNumberLabel.text = [NSString stringWithFormat:BJLLocalizedString(@"发布：%td人"), self.answerSheet.userCountParticipate];

    //答题人数
    self.participatedNumberLabel.text = [NSString stringWithFormat:BJLLocalizedString(@"答题：%td人"), self.answerSheet.userCountSubmit];
    ...
    self.correctRateLabel.text = [NSString stringWithFormat:BJLLocalizedString(@"正确率：%.1f%%"), correctRate * 100];
}
...
- (void)updateCountDownShowTime {
    ...
    self.countDownTimeLabel.text = [NSString stringWithFormat:BJLLocalizedString(@"结束倒计时：%02i:%02i"), minutes, second];
}

- (void)updateCorrectAnswerTip {
    if (self.answerSheet.answerType == BJLAnswerSheetType_Judgement) {
        self.optionTipLabel.text = BJLLocalizedString(@"点击选项可设置正确答案");
        return;
    }

    BOOL hasSetCorrectAnswer = NO;
    NSMutableString *correctAnswer = [[NSMutableString alloc] initWithString:BJLLocalizedString(@"已设置正确答案为：")];
...
- (void)closeUI {
    // 如果在答题中,关闭时需要撤回答题
    BOOL isPublish = (self.layout == BJLQuestionAnswerWindowLayout_publish);
    if (isPublish) {
...
- (BOOL)keyboardDidShow {
    if ([self.timeTextField isFirstResponder] || [self.commentsTextView isFirstResponder]
        || [self.rightTextField isFirstResponder] || [self.wrongTextField isFirstResponder]) {
        [self hideKeyboardView];
        return NO;
    }
    return YES;
...
- (void)updateEditContainerView {
    ...
    if (self.answerSheet.answerType == BJLAnswerSheetType_Choosen) {
        ...
        optionsViewHeight = [self answerCollectionViewHeight:10 ipad:15];
        optionsViewWidth = [self.answerSheet.options count] <= 4
                               ? (BJLAppearance.questionAnswerOptionButtonHeight * [self.answerSheet.options count] + (isIphone ? 10 : 25) * ([self.answerSheet.options count] - 1))
                               : (BJLAppearance.questionAnswerOptionButtonHeight * 4 + (isIphone ? 10 : 25) * 3);
...
- (void)upadteQuestionTypeButton {
    ...
    [self.choosenButton setBackgroundColor:isChoose ? [BJLTheme brandColor] : [BJLTheme subButtonBackgroundColor]];
    [self.judgementButton setBackgroundColor:!isChoose ? [BJLTheme brandColor] : [BJLTheme subButtonBackgroundColor]];
}
//点击选择题
...
- (void)publishQuestion {
    ...
    if (!hasJudegeTitle && self.answerSheet.answerType == BJLAnswerSheetType_Judgement) {
        if (self.errorCallback) {
            self.errorCallback(BJLLocalizedString(@"判断题选择内容不能为空"));
        }
        return;
    }
    ...
    if (!hasCorrectAnswer && self.answerSheet.answerType == BJLAnswerSheetType_Choosen) {
        if (self.errorCallback) {
            self.errorCallback(BJLLocalizedString(@"请先选择一个正确答案"));
        }
        return;
    }
...
- (void)resetQuestion {
    ...
    [self resetAnswerOptions];
    [self updateEditContainerView];
}

- (void)resetAnswerOptions {
    ...
    if (self.answerSheet.answerType == BJLAnswerSheetType_Choosen) {
        ...
        if (self.rightButton.isSelected) {
            [self updateWithChooseRightButton];
        }
        if (self.wrongButton.isSelected) {
            [self updateWithChooseWrongButton];
        }
    }
}
...
//结束答题
- (void)endQuestion {
    if (self.layout != BJLQuestionAnswerWindowLayout_publish) {
        return;
    }
...
//重新编辑
- (void)reeditQuestion {
    if (self.layout != BJLQuestionAnswerWindowLayout_end) {
        return;
    }

    self.layout = BJLQuestionAnswerWindowLayout_normal;
    [self initialAnswerOptions];
    [self updateContentViewAndBottomView];
}
...
- (void)textFieldDidEndEditing:(UITextField *)textField {
    if (textField == self.timeTextField) {
        ...
        //正确和错误的选项都输入了值，才进行后续逻辑
        if (!(self.rightTextField.text.length > 0 && self.wrongTextField.text.length > 0)) {
            return;
        }
        // 新增判断题title
        NSDictionary *dic = @{
        ...
        //这里把自定义选项收集后，把textfield的交互关闭，避免用户反复输入形成重复选项
        self.wrongTextField.layer.borderWidth = 0;
        self.wrongTextField.userInteractionEnabled = NO;
        self.wrongTextField.layer.borderWidth = 0;
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == self.collectionView) {
        ...
        BJLQuestionAnswerPublishedOptionCollectionViewCell *publishedCell;
        if (self.answerSheet.answerType == BJLAnswerSheetType_Choosen) {
            publishedCell = [collectionView dequeueReusableCellWithReuseIdentifier:BJLQuestionAnswerPublishedOptionCollectionViewCell_ChoosenCell forIndexPath:indexPath];
            [publishedCell updateContentWithOptionKey:option.key isSelected:option.isAnswer selectedTimes:option.choosenTimes];
        }
        else if (self.answerSheet.answerType == BJLAnswerSheetType_Judgement) {
            if (indexPath.row == 0) {
                publishedCell = [collectionView dequeueReusableCellWithReuseIdentifier:BJLQuestionAnswerPublishedOptionCollectionViewCell_JudgeCell_right forIndexPath:indexPath];
            }
            else {
                publishedCell = [collectionView dequeueReusableCellWithReuseIdentifier:BJLQuestionAnswerPublishedOptionCollectionViewCell_JudgeCell_wrong forIndexPath:indexPath];
            }
            [publishedCell updateContentWithSelected:option.isAnswer selectedTimes:option.choosenTimes];
            [publishedCell updateJudgeOptionTitle:option.key]; //更新判断题title
        }
...
- (CGSize)collectionView:(UICollectionView *)collectionView
    ...
    return CGSizeMake(BJLAppearance.questionAnswerOptionButtonHeight + 15, 75);
}
...
- (CGFloat)answerCollectionViewHeight:(CGFloat)iphone ipad:(CGFloat)ipad {
    BOOL isIphone = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone);
    CGFloat optionsViewHeight = 0;
    if ([self.answerSheet.options count] <= 4) {
        optionsViewHeight = BJLAppearance.questionAnswerOptionButtonHeight;
    }
    else if ([self.answerSheet.options count] <= 4 * 2) {
...
- (CGFloat)answerCollectionContainerHeight:(CGFloat)iphone ipad:(CGFloat)ipad {
    BOOL isIphone = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone);
    CGFloat height = 0;
    if ([self.answerSheet.options count] <= 4 * 2) {
        height = BJLAppearance.questionAnswerOptionButtonHeight * 2 + (isIphone ? iphone : ipad);
    }
    else {
        height = BJLAppearance.questionAnswerOptionButtonHeight * 3 + (isIphone ? iphone : ipad) * 2;
    }
    return height;
}
...
- (void)makeObserving {
    ...
    [self bjl_observe:BJLMakeMethod(self.room.roomVM, didReceiveQuestionAnswerSheet:) observer:^BOOL(BJLAnswerSheet *answerSheet) {
        ...
        self.answerSheet = answerSheet;
        self.layout = BJLQuestionAnswerWindowLayout_publish;
    ...
    [self bjl_observe:BJLMakeMethod(self.room.roomVM, didReceiveEndQuestionAnswerWithEndTime:) observer:(BJLMethodObserver) ^ BOOL(NSTimeInterval endTime) {
        ...
        self.layout = BJLQuestionAnswerWindowLayout_end;
    ...
    [self bjl_observe:BJLMakeMethod(self.room.roomVM, didReceiveQuestionAnswerDetailInfo:) observer:^BOOL((NSArray<BJLAnswerSheet *> * answerSheetArray)) {
        ...
        for (BJLAnswerSheetOption *option in self.answerSheet.options) {
            if (option.isAnswer) {
                self.correctAnswerLabel.text = option.key;
                break;
            }
        }
    ...
    BOOL isSheetPresent = iPhone && (UIScreen.mainScreen.bounds.size.height > UIScreen.mainScreen.bounds.size.width);
    [self bjl_kvo:BJLMakeProperty(self, layout)
         observer:^BJLControlObserving(id _Nullable value, id _Nullable oldValue, BJLPropertyChange *_Nullable change) {
             bjl_strongify(self);
             if (([self answerSheetOptionsCount] > 8) && self.layout == BJLQuestionAnswerWindowLayout_publish) {
                 CGSize size = [self presentationSize];
                 if (isSheetPresent) {
                     self.preferredContentSize = CGSizeMake(UIScreen.mainScreen.bounds.size.width, UIScreen.mainScreen.bounds.size.height * 0.5 + 80.0);
                 }
                 else {
                     self.preferredContentSize = CGSizeMake(size.width, size.height + 80.0);
                 }
             }
             else {
                 if (isSheetPresent) {
                     self.preferredContentSize = CGSizeMake(UIScreen.mainScreen.bounds.size.width, UIScreen.mainScreen.bounds.size.height * 0.5);
                 }
                 else {
                     self.preferredContentSize = [self presentationSize];
                 }
             }
             return YES;
         }];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (tableView == self.detailTableView) {
        BJLQuestionAnswerSheetUserDetailTableViewCell *cell = [tableView dequeueReusableCellWithIdentifier:detailTableViewCellIdentifier forIndexPath:indexPath];
        ...
        if (indexPath.row == [[self judgeTitleArray] count]) {
            cell.textLabel.text = BJLLocalizedString(@"自定义");
        }
        else {
        ...
        cell.textLabel.textColor = BJLTheme.viewTextColor;
        return cell;
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (indexPath.row >= [[self judgeTitleArray] count]) {
        [self.judgeTitleButton setTitle:BJLLocalizedString(@"自定义") forState:UIControlStateNormal];
        BJLAnswerSheetOption *YESOption = [self.answerSheet.options firstObject];
        YESOption.key = @"";
        ...
        self.rightTextField.layer.cornerRadius = BJLAppearance.cornerRadius;
        ...
        self.wrongTextField.layer.cornerRadius = BJLAppearance.cornerRadius;
    }
    else {
        NSDictionary *dic = [[self judgeTitleArray] objectAtIndex:indexPath.row];
        NSString *rightString = [dic bjl_stringForKey:@"1"];
