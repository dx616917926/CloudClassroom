//
//  BJLQuestionAnswerViewController+statistics.m
//  BJLiveUI
//
...
@implementation BJLQuestionAnswerViewController (statistics)
...
- (void)updateStatisticsData {
    ...
    UIFont *reateLabelFont = [UIFont boldSystemFontOfSize:12];
    if (count > 4) {
        reateLabelFont = [UIFont systemFontOfSize:10];
    }
    for (NSInteger i = 0; i < count; i++) {
        ...
        [view bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
            ...
            make.height.equalTo(self.chartView).multipliedBy(rate);
            make.bottom.equalTo(self.chartView);
        }];
        UILabel *rateLabel = [UILabel new];
        rateLabel.text = [NSString stringWithFormat:@"%.1f%%", rate * 100];
        rateLabel.textColor = BJLTheme.viewTextColor;
        rateLabel.font = reateLabelFont;
        ...
        label.textColor = option.isAnswer ? [BJLTheme brandColor] : BJLTheme.viewTextColor;
