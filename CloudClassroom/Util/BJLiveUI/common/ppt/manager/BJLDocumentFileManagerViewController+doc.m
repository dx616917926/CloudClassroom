//
//  BJLDocumentFileManagerViewController+doc.m
//  BJLiveUI
//
...
@implementation BJLDocumentFileManagerViewController (doc)
...
- (void)makeDocumentChooseView {
    ...
    self.addDocumentContainerView = ({
        ...
        view.accessibilityIdentifier = BJLKeypath(self, addDocumentContainerView);
        view;
    });

    [self.chooseDocumentLayer addSubview:self.addDocumentContainerView];
    ...
    UILabel *titleLabel = ({
        ...
        label.text = BJLLocalizedString(@"上传文件");
    ...
    UIView *gapline1 = [UIView bjl_createSeparateLine];
    ...
    normalFileImageView.image = [UIImage bjl_imageNamed:@"bjl_document_normalfile"];
    ...
    UILabel *normalFileLabel = ({
        ...
        label.backgroundColor = [UIColor clearColor];
        label.textAlignment = NSTextAlignmentLeft;
        label.text = BJLLocalizedString(@"上传普通文件");
        label.textColor = BJLTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:14.0];
        label;
    });
    ...
    UILabel *normalFileDetailLabel = ({
        ...
        label.backgroundColor = [UIColor clearColor];
        label.textAlignment = NSTextAlignmentLeft;
        label.text = BJLLocalizedString(@"转码时间较短");
        label.textColor = BJLTheme.viewSubTextColor;
        label.font = [UIFont systemFontOfSize:12.0];
        label;
    });
    [self.addDocumentContainerView addSubview:normalFileDetailLabel];
    UIImageView *normalIcon = [[UIImageView alloc] initWithImage:[UIImage bjl_imageNamed:@"bjl_document_upload"] highlightedImage:[UIImage bjl_imageNamed:@"bjl_document_upload_highlight"]];
    ...
    animatedFileImageView.image = [UIImage bjl_imageNamed:@"bjl_document_animatedfile"];
    ...
    UILabel *animatedFileLabel = ({
        ...
        label.text = BJLLocalizedString(@"上传动效文件");
        label.textColor = BJLTheme.viewTextColor;
        label.font = [UIFont systemFontOfSize:14.0];
        label;
    });
    ...
    UILabel *animatedFileDetailLabel = ({
        ...
        label.backgroundColor = [UIColor clearColor];
        label.textAlignment = NSTextAlignmentLeft;
        label.text = BJLLocalizedString(@"转码时间较长");
        label.textColor = BJLTheme.viewSubTextColor;
        label.font = [UIFont systemFontOfSize:12.0];
        label;
    });
    [self.addDocumentContainerView addSubview:animatedFileDetailLabel];
    UIImageView *animateIcon = [[UIImageView alloc] initWithImage:[UIImage bjl_imageNamed:@"bjl_document_upload"] highlightedImage:[UIImage bjl_imageNamed:@"bjl_document_upload_highlight"]];
    ...
    UILabel *noticeLabel = ({
        ...
        label.textColor = BJLTheme.viewSubTextColor;
        label.font = [UIFont systemFontOfSize:14.0];
        label.text = BJLLocalizedString(@"1. 动效文件支持ppt、pptx、zip(H5)格式，上传后还原源文件中的动效\n2. 普通文件支持ppt、pptx、doc、docx、jpg、pdf格式\n3. 文件中不能包含密码\n4. WPS编辑的文档请转换成PDF后上传");
        label;
    });
...
- (void)makeObservingForDoc {
    ...
    [self bjl_observe:BJLMakeMethod(self.room.documentVM, didLoadMediaFiles:)
             observer:^BJLControlObserving(NSArray<BJLMediaFile *> *mediaFiles) {
                 bjl_strongify(self);
                 if (self.room.roomInfo.roomType != BJLRoomType_interactiveClass) {
                     return YES;
                 }
                 self.mutableMediaFileList = [NSMutableArray array];
                 for (BJLMediaFile *mediaFile in mediaFiles) {
                     [self.mutableMediaFileList bjl_addObject:[[BJLDocumentFile alloc] initWithRemoteMediaFile:mediaFile]];
                 }
                 [self reloadTableViewWithLayoutType:BJLDocumentFileLayoutTypeDocument];
                 return YES;
             }];
}
...
- (void)updateTranscodeListWithDocumentFile:(BJLDocumentFile *)documentFile {
    ...
    NSString *documentID = documentFile.remoteDocument.documentID;
    for (BJLDocumentFile *documentFile in [self.mutableTranscodeDocumentFileList copy]) {
        ...
        else if (documentID.length && [documentFile.remoteDocument.documentID isEqualToString:documentID]) {
            changedFile = documentFile;
        }
    }
...
- (void)deleteDocumetFile:(BJLDocumentFile *)file {
    if ([self.mutableAllDocumentFileList containsObject:file]) {
        [self.mutableAllDocumentFileList bjl_removeObject:file];
    }

    if ([self.mutableMediaFileList containsObject:file]) {
        [self.mutableMediaFileList bjl_removeObject:file];
    }
