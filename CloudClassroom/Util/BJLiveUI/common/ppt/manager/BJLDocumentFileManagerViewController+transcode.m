//
//  BJLDocumentFileManagerViewController+transcode.m
//  BJLiveUI
//
...
@implementation BJLDocumentFileManagerViewController (transcode)
...
- (void)startPollTimer {
    ...
    bjl_weakify(self);
    self.pollTimer = [NSTimer scheduledTimerWithTimeInterval:pollDuration repeats:YES block:^(NSTimer *_Nonnull timer) {
        bjl_strongify(self);
        if (!self) {
            [timer invalidate];
            return;
