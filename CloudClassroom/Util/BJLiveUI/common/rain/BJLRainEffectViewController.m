...
- (instancetype)initWithRoom:(BJLRoom *)room envelopeID:(NSInteger)envelopeID duration:(NSInteger)duration {
    if (self = [self initWithRoom:room]) {
        ...
        for (NSInteger i = 1; i <= 14; i++) {
            NSString *imageName = [NSString stringWithFormat:@"bjl_envelope_open%ld", (long)i];
...
- (void)setupRainEffectSize:(CGSize)size rainImageName:(NSString *)imageName rainCount:(NSInteger)count rainSize:(CGSize)rainSize {
    ...
    NSString *imageFilePath = [NSTemporaryDirectory() stringByAppendingPathComponent:@"bjl_envelope_run.png"];
    if (!imageName.length) {
        UIImage *image = [UIImage bjl_imageNamed:@"bjl_envelope_run"];
        NSData *imageData = UIImagePNGRepresentation(image);
        [imageData writeToFile:imageFilePath atomically:YES];
...
- (void)makeSubviewsAndConstraints {
    ...
    UIImageView *envelopeLeftView = [self makeEnvelopeImageViewWithName:@"bjl_envelope_start"];
    UIImageView *envelopeRightView = [self makeEnvelopeImageViewWithName:@"bjl_envelope_start"];
    UIImageView *envelopeCenterView = [self makeEnvelopeImageViewWithName:@"bjl_envelope_start"];
    ...
    self.delayRainTipLabel = ({
        UILabel *label = [UILabel new];
        label.backgroundColor = [UIColor clearColor];
        label.text = BJLLocalizedString(@"抢红包啦");
    ...
    UILabel *cutTipLabel = ({
        ...
        attach.image = [UIImage bjl_imageNamed:@"bjl_envelope_cut"];
        ...
        NSAttributedString *tipString = [[NSAttributedString alloc] initWithString:BJLLocalizedString(@"也可以切红包哦~")
...
- (void)startDelayRainTimer {
    [self stopDelayRainTimer];
    bjl_weakify(self);
    self.delayRainTimer = [NSTimer scheduledTimerWithTimeInterval:1.0
                                                          repeats:YES
...
- (void)makeRainScene {
    ...
    [self.rainScene setRequestOpenEnvelopeScoreCallback:^(BJLOpenEnvelopeScoreCompletion _Nonnull completion) {
        bjl_strongify(self);
        [self.room.roomVM grapEnvelopeWithID:self.envelopeID
                                  completion:^(NSInteger score, BJLError *_Nullable error) {
                                      completion(score);
                                  }];
    }];
...
- (void)removeRainScene {
    ...
    if (rainSceneOnScreen) {
        ...
        if (self.room.loginUser.isStudent) {
            ...
            BJLEnvelopesRainView *envelopesRainView = [BJLEnvelopesRainView resultEnvelopesRainViewIsTeacher:self.room.loginUser.isTeacher];

            UIViewController *presentViewController = [UIViewController new];
            [presentViewController.view addSubview:envelopesRainView];
            [envelopesRainView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
                make.edges.equalTo(presentViewController.view);
            }];

            bjl_weakify(self, presentViewController);
            [envelopesRainView setCloseCallback:^{
                bjl_strongify(self, presentViewController);
                [presentViewController dismissViewControllerAnimated:YES
                                                          completion:^{
                                                              [self hide];
                                                          }];
            }];
            [envelopesRainView setShowResultCallback:^(BJLEnvelopesRainView *_Nonnull resultView) {
                bjl_strongify(self, presentViewController);
                [presentViewController dismissViewControllerAnimated:NO
                                                          completion:^{
            ...
            [envelopesRainView setOnceMoreCallback:^{
                bjl_strongify(self, presentViewController);
                [presentViewController dismissViewControllerAnimated:YES
                                                          completion:^{
            ...
            if (needSheetPresent) {
                BJLSheetPresentationController *sheetPresentationController = [[BJLSheetPresentationController alloc] initWithPresentedViewController:presentViewController presentingViewController:self];
                [sheetPresentationController setTapCallback:^BOOL(UIViewController *_Nullable viewController) {
                    bjl_strongify(presentViewController);
                    [presentViewController dismissViewControllerAnimated:YES
                                                              completion:^{
                                                                  [self hide];
                                                              }];
                ...
                presentViewController.transitioningDelegate = sheetPresentationController;
                presentViewController.preferredContentSize = CGSizeMake(self.view.bounds.size.width, self.view.bounds.size.height * 0.48);
                [self presentViewController:presentViewController animated:YES completion:nil];
            }
            else {
                BJLAlertPresentationController *alertPresentationController = [[BJLAlertPresentationController alloc] initWithPresentedViewController:presentViewController presentingViewController:self];
                ...
                presentViewController.transitioningDelegate = alertPresentationController;
                presentViewController.preferredContentSize = CGSizeMake(280.0, 318.0);
                [self presentViewController:presentViewController animated:YES completion:nil];
            }
        }
    }
...
- (void)makeEnvelopeResultViewAndConstraints:(NSInteger)score {
    ...
    UIImageView *imageView = ({
        UIImageView *imageView = [UIImageView new];
        imageView.image = score > 0 ? [UIImage bjl_imageNamed:@"bjl_envelope"] : [UIImage bjl_imageNamed:@"bjl_envelope_empty"];
        imageView.contentMode = UIViewContentModeScaleAspectFill;
        imageView;
    });
    ...
    UIButton *confirmButton = ({
        ...
        [button setTitle:score > 0 ? BJLLocalizedString(@"完成") : BJLLocalizedString(@"再接再厉") forState:UIControlStateNormal];
    ...
    [self bjl_kvo:BJLMakeProperty(self.rainScene, totalScore)
         observer:^BJLControlObserving(id _Nullable value, id _Nullable oldValue, BJLPropertyChange *_Nullable change) {
             bjl_strongify(self);
             imageView.image = self.rainScene.totalScore > 0 ? [UIImage bjl_imageNamed:@"bjl_envelope"] : [UIImage bjl_imageNamed:@"bjl_envelope_empty"];
             [confirmButton setTitle:self.rainScene.totalScore > 0 ? BJLLocalizedString(@"完成") : BJLLocalizedString(@"再接再厉") forState:UIControlStateNormal];
...
- (void)makeScoreLabelWithImageView:(UIImageView *)imageView score:(NSInteger)score {
    ...
    if (score > 0) {
        UILabel *label = ({
            ...
            NSAttributedString *follow = [[NSAttributedString alloc] initWithString:BJLLocalizedString(@" 学分")
        ...
        UILabel *label = ({
            UILabel *label = [UILabel new];
            label.backgroundColor = [UIColor clearColor];
            label.text = BJLLocalizedString(@"一个都没抢到～");
            label.textColor = [UIColor whiteColor];
        ...
        UIImageView *emoticonView = ({
            UIImageView *imageView = [UIImageView new];
            imageView.image = [UIImage bjl_imageNamed:@"bjl_envelope_emoticon"];
            imageView.contentMode = UIViewContentModeScaleAspectFill;
            imageView;
        });
...
- (void)makeEnvelopeRankResultViewAndConstraints {
    ...
    UIImageView *envelopeResultImageView = ({
        ...
        imageView.image = [UIImage bjl_imageNamed:@"bjl_envelope_result"];
        imageView.contentMode = UIViewContentModeScaleAspectFill;
        imageView;
    });
    ...
    self.emptyRankResultView = ({
        UIView *view = [UIView new];
        view.hidden = YES;
        view.accessibilityIdentifier = BJLKeypath(self, emptyRankResultView);
        ...
        UILabel *label = ({
            UILabel *label = [UILabel new];
            label.backgroundColor = [UIColor clearColor];
            label.text = BJLLocalizedString(@"没人抢到～");
    ...
    self.closeButton = ({
        ...
        [button setImage:[UIImage bjl_imageNamed:@"bjl_envelope_close"] forState:UIControlStateNormal];
...
- (void)reloadRankList {
    ...
    bjl_weakify(self);
    [self.room.roomVM requestRankListWithEnvelopeID:self.envelopeID
                                         completion:^(NSArray<BJLEnvelopeRank *> *_Nullable rankList, BJLError *_Nullable error) {
                                             bjl_strongify(self);
                                             if (!self) {
                                                 return;
                                             }
...
- (UIView *)makeHeaderForRankTableView {
    ...
    UILabel *rankLabel = ({
        ...
        label.text = BJLLocalizedString(@"排名");
        label;
    });
    ...
    UILabel *userNameLabel = ({
        ...
        label.font = [UIFont systemFontOfSize:13.0];
        label.textColor = [UIColor whiteColor];
        label.text = BJLLocalizedString(@"用户");
        label.textAlignment = NSTextAlignmentCenter;
        label;
    });
    ...
    UILabel *scoreLabel = ({
        ...
        label.font = [UIFont systemFontOfSize:13.0];
        label.textColor = [UIColor whiteColor];
        label.text = BJLLocalizedString(@"金币");
