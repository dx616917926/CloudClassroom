//
//  BJLAlertPresentationController.m
//  BJLiveUIBase
//
...
@interface BJLAlertPresentationController () <UIViewControllerAnimatedTransitioning, UIGestureRecognizerDelegate>
@property (nonatomic, strong) UIView *dimmingView;
@property (nonatomic, strong) UIView *presentationWrappingView;
@end

@implementation BJLAlertPresentationController
...
- (void)presentationTransitionWillBegin {
    ...
    // SEE ALSO: The note in AAPLCustomPresentationSecondViewController.m.
    {
        ...
        presentationWrapperView.layer.shadowOffset = CGSizeMake(0.0, -6.f);
        ...
        //CGRect newRect = CGRectMake((self.containerView.bounds.size.width - presentationWrapperView.bounds.size.width) / 2, presentationWrapperView.bounds.origin.y, presentationWrapperView.bounds.size.width, presentationWrapperView.bounds.size.height);
        UIView *presentationRoundedCornerView = [[UIView alloc] initWithFrame:UIEdgeInsetsInsetRect(presentationWrapperView.bounds, UIEdgeInsetsZero)];
        ...
        UIView *presentedViewControllerWrapperView = [[UIView alloc] initWithFrame:UIEdgeInsetsInsetRect(presentationRoundedCornerView.bounds, UIEdgeInsetsZero)];
        ...
        self.presentationWrappingView.bjl_titleBarHeight = 30.0;
        [self.presentationWrappingView bjl_addTitleBarPanGesture];
    }
...
- (void)dimmingViewTapped:(UITapGestureRecognizer *)sender {
    if (self.tapCallback && !self.tapCallback([super presentedViewController])) {
        return;
    }
...
- (void)animateTransition:(id<UIViewControllerContextTransitioning>)transitionContext {
    ...
    if (isPresenting) {
        toViewInitialFrame.origin = CGPointMake(toViewFinalFrame.origin.x, CGRectGetMaxY(containerView.bounds));
        ...
        fromViewFinalFrame = CGRectOffset(fromView.frame, 0.0, UIScreen.mainScreen.bounds.size.height - fromView.frame.origin.y);
    }
...
- (BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch {
    if ([touch.view isDescendantOfView:[super presentedView]]) {
        return NO;
    }
    return YES;
}
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    CGRect oldFrame = self.presentationWrappingView.frame;
    CGFloat relativeY = (UIScreen.mainScreen.bounds.size.height - oldFrame.size.height) / 7.0;
    if (iPhone) {
        relativeY = 0.0;
    }
    [UIView animateWithDuration:1.0
                     animations:^{
                         self.presentationWrappingView.frame = CGRectMake(oldFrame.origin.x, relativeY, oldFrame.size.width, oldFrame.size.height);
                     }];
}

- (void)keyboardWillHide:(NSNotification *)notification {
    CGRect oldFrame = self.presentationWrappingView.frame;
    CGFloat relativeY = (UIScreen.mainScreen.bounds.size.height - oldFrame.size.height) / 2.0;
    [UIView animateWithDuration:1.0
                     animations:^{
                         self.presentationWrappingView.frame = CGRectMake(oldFrame.origin.x, relativeY, oldFrame.size.width, oldFrame.size.height);
                     }];
