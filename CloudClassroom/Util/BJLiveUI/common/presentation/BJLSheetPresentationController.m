//
//  BJLSheetPresentationController.m
//  BJLiveUIBase
//
...
@interface BJLSheetPresentationController () <UIViewControllerAnimatedTransitioning, UIGestureRecognizerDelegate>
@property (nonatomic, strong) UIView *dimmingView;
@property (nonatomic, strong) UIView *presentationWrappingView;
@end

@implementation BJLSheetPresentationController
...
- (instancetype)initWithPresentedViewController:(UIViewController *)presentedViewController presentingViewController:(UIViewController *)presentingViewController {
    ...
    if (self) {
        ...
        if ([presentedViewController isKindOfClass:[BJLViewController class]]) {
            BJLViewController *vc = bjl_as(presentedViewController, BJLViewController);
            vc.bjl_supportedInterfaceOrientations = UIInterfaceOrientationMaskPortrait;
        }
...
- (void)presentationTransitionWillBegin {
    ...
    // SEE ALSO: The note in AAPLCustomPresentationSecondViewController.m.
    {
        ...
        UIView *presentationRoundedCornerView = [[UIView alloc] initWithFrame:UIEdgeInsetsInsetRect(presentationWrapperView.bounds, UIEdgeInsetsMake(0.0, 0.0, -CORNER_RADIUS, 0.0))];
        presentationRoundedCornerView.autoresizingMask = UIViewAutoresizingFlexibleWidth | UIViewAutoresizingFlexibleHeight;
        presentationRoundedCornerView.layer.cornerRadius = 8.0;
        ...
        UIView *presentedViewControllerWrapperView = [[UIView alloc] initWithFrame:UIEdgeInsetsInsetRect(presentationRoundedCornerView.bounds, UIEdgeInsetsMake(0.0, 0.0, CORNER_RADIUS, 0.0))];
...
- (void)keyboardWillShow:(NSNotification *)notification {
    ...
    if (!userInfo || [[super presentedViewController] isKindOfClass:BJLScQuestionViewController.class]) {
        return;
    }
    ...
    CGFloat relativeY = UIScreen.mainScreen.bounds.size.height - oldFrame.size.height - keyboardFrame.size.height;
    relativeY = MAX(0, relativeY);
    if ([[super presentedViewController] isKindOfClass:BJLScQuestionInputViewController.class]) {
        [super presentedViewController].view.hidden = NO;
    }
...
- (void)keyboardWillHide:(NSNotification *)notification {
    CGRect oldFrame = self.presentationWrappingView.frame;
    CGFloat relativeY = UIScreen.mainScreen.bounds.size.height - oldFrame.size.height;
    [UIView animateWithDuration:0.5
                     animations:^{
                         self.presentationWrappingView.frame = CGRectMake(oldFrame.origin.x, relativeY, oldFrame.size.width, oldFrame.size.height);
                     }];
