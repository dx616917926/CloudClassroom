...
@interface BJLDrawStrokeColorSelectView () <UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout>
...
@implementation BJLDrawStrokeColorSelectView
...
- (void)setupSubviews {
    ...
    UIView *line = [UIView bjl_createSeparateLine];
    [self addSubview:line];
    [line bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.top.equalTo(self);
        make.left.right.equalTo(self).inset(BJLAppearance.toolboxDrawSpace).priorityHigh();
    ...
    [self.strokeColorsView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.edges.equalTo(self).insets(UIEdgeInsetsMake(BJLAppearance.toolboxDrawSpace, BJLAppearance.toolboxDrawSpace, BJLAppearance.toolboxDrawSpace, BJLAppearance.toolboxDrawSpace)).priorityHigh();
    }];
}

- (void)layoutSubviews {
    [super layoutSubviews];
    [self.containerView bjl_drawRectCorners:UIRectCornerBottomLeft | UIRectCornerBottomRight cornerRadii:CGSizeMake(BJLAppearance.toolboxCornerRadius, BJLAppearance.toolboxCornerRadius)];
}
...
- (CGFloat)collectionView:(UICollectionView *)collectionView
    ...
    return BJLAppearance.toolboxOffset * 2;
}
...
// 视图需要自适应，一行 4 个颜色
- (CGSize)itemSize {
    CGFloat itemWidth = (self.strokeColorsView.bounds.size.width - BJLAppearance.toolboxOffset * 2 * 3) / 4.0;
    if (itemWidth < 0) {
...
- (NSArray *)strokeColors {
    if (!_strokeColors) {
        _strokeColors = @[@"#FF9500", @"#FFBF2F", @"#F8E71C", @"#EE4844", @"#1795FF", @"#00CAF5", @"#50E3C2", @"#FC9D9A", @"#8B572A", @"#417505", @"#29CF42", @"#9A1BFF", @"#FFFFFF", @"#8E8E93", @"#424242", @"#000000"];
    }
