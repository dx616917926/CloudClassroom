...
- (BOOL)enableAtUser:(BJLMediaUser *)user {
    if (self.room.loginUser.isTeacherOrAssistant) {
        return YES;
    }

    BOOL enableAt = NO;
    BJLStudentWhisperRole role = self.room.featureConfig.studentWhisperRole;
    if (user.isTeacher) {
        enableAt = (role == BJLStudentWhisperRoleTeacher) || (role == BJLStudentWhisperRoleAll);
    }
    else if (user.isAssistant) {
        enableAt = (role == BJLStudentWhisperRoleAssistant) || (role == BJLStudentWhisperRoleAll);
    }
...
- (void)updateChatStatus:(BJLChatStatus)chatStatus withTargetUser:(nullable BJLUser *)targetUser {
    ...
    if (chatStatus == BJLChatStatus_private) {
        ...
        // text
        self.chatStatusLabel.text = [NSString stringWithFormat:BJLLocalizedString(@"正在和 %@ 私聊中..."), self.targetUser.displayName];
    }
    else {
        // reset
        [self.chatStatusView bjl_updateConstraints:^(BJLConstraintMaker *make) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.backgroundColor = ([self.targetUser.ID isEqualToString:user.ID]) ? BJLTheme.separateLineColor : [UIColor clearColor];
...
- (UIButton *)emptyListView {
    if (!_emptyListView) {
        _emptyListView = ({
            ...
            [button setTitle:BJLLocalizedString(@"暂无可私聊对象，点击刷新") forState:UIControlStateNormal];
