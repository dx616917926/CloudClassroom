...
@property (nonatomic) BJLChatQuickReplyWordsViewController *quickReplyWordsCollectionView;
...
- (void)viewDidLoad {
    ...
    [UIMenuController sharedMenuController].menuItems = @[[[UIMenuItem alloc] initWithTitle:BJLLocalizedString(@"换行") action:@selector(insertNewLine:)]];
}
...
- (void)makeSubviews {
    ...
    self.quickReplyWordsCollectionView = [[BJLChatQuickReplyWordsViewController alloc] initWithRoom:self.room];
    [self.view addSubview:self.quickReplyWordsCollectionView.view];
    ...
    self.textView = ({
        ...
        textView.bjl_placeholder = (self.chatStatus == BJLChatStatus_private) ? [NSString stringWithFormat:BJLLocalizedString(@"私聊%@"), self.targetUser.displayName] : BJLLocalizedString(@"输入聊天内容");
    ...
    if (self.room.featureConfig.enableWhisper) {
        self.privateChatButton = ({
            ...
            UILabel *label = ({
                ...
                label.text = BJLLocalizedString(@"私聊");
                label.textAlignment = NSTextAlignmentCenter;
                label.adjustsFontSizeToFitWidth = YES;
                label.font = [UIFont systemFontOfSize:12.0];
                label.layer.masksToBounds = YES;
                label.layer.borderWidth = 0.5;
                label.layer.borderColor = BJLTheme.viewSubTextColor.CGColor;
    ...
    self.emoticonButton = ({
        UIButton *button = [UIButton new];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_sc_emotion"] forState:UIControlStateNormal];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_sc_emotion"] forState:UIControlStateHighlighted];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_sc_keybord"] forState:UIControlStateSelected];
        [button setImage:[UIImage bjl_imageNamed:@"bjl_sc_keybord"] forState:UIControlStateSelected | UIControlStateHighlighted];
        button;
    });
    ...
    if (self.room.loginUser.isTeacherOrAssistant
        ...
        self.imageButton = ({
            UIButton *button = [UIButton new];
            [button setImage:[UIImage bjl_imageNamed:@"bjl_sc_img"] forState:UIControlStateNormal];
            button;
        });
...
- (void)makeConstraints {
    [self.quickReplyWordsCollectionView.view bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        make.top.left.right.equalTo(self.view);
        make.height.equalTo(@50);
    }];

    [self.contentView bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.top.equalTo(self.quickReplyWordsCollectionView.view.bjl_bottom);
    ...
    [self.privateChatButton bjl_makeConstraints:^(BJLConstraintMaker *make) {
        ...
        make.centerY.equalTo(self.emoticonButton);
    }];

    UIView *seperatorLine = [UIView new];
    seperatorLine.backgroundColor = BJLTheme.separateLineColor;
    [self.contentView addSubview:seperatorLine];
    [seperatorLine bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
...
- (void)makeObserving {
        ...
        observer:^BOOL(id _Nullable now, id _Nullable old, BJLPropertyChange *_Nullable change) {
            ...
            if (self.emoticonViewController) {
                if (self.emoticonButton.selected) {
                    ...
                    self.emoticonViewController.popoverPresentationController.backgroundColor = BJLTheme.windowBackgroundColor;
    ...
    [self bjl_kvo:BJLMakeProperty(self.textView, inputView)
         observer:^BOOL(UIView *_Nullable now, UIView *_Nullable old, BJLPropertyChange *_Nullable change) {
             bjl_strongify(self);
             // 光标颜色
             self.textView.tintColor = now ? [UIColor clearColor] : BJLTheme.brandColor;
...
- (void)makeCallbacks {
    ...
    [self.quickReplyWordsCollectionView setDidSelectedWordCallback:^(BJLChatQuickReplyWordsViewController *_Nonnull vc, NSString *_Nonnull word) {
        bjl_strongify(self);
...
- (void)clearInputView {
    self.showWhisperChatUserListWhenExpend = NO;
    self.privateChatButton.selected = NO;
    self.emoticonButton.selected = NO;
...
- (void)updateChatStatus:(BJLChatStatus)chatStatus withTargetUser:(nullable BJLUser *)targetUser {
    ...
    // update content
    self.privateChatLabel.textColor = privateChat ? BJLTheme.brandColor : BJLTheme.viewSubTextColor;
    self.privateChatLabel.layer.borderColor = (privateChat ? BJLTheme.brandColor : BJLTheme.viewSubTextColor).CGColor;
    self.textView.bjl_placeholder = privateChat ? [NSString stringWithFormat:BJLLocalizedString(@"私聊%@"), self.targetUser.displayName] : BJLLocalizedString(@"输入聊天内容");
...
- (BOOL)send:(nullable NSString *)message data:(nullable NSDictionary *)data {
    ...
    BOOL isStudentForbid = self.room.loginUser.isStudent && (self.room.chatVM.forbidAll || self.room.chatVM.forbidMyGroup || self.room.chatVM.forbidMe);
    BOOL isStudentPrivateForbid = (self.chatStatus == BJLChatStatus_private) && self.room.loginUser.isStudent && ((self.room.chatVM.forbidAll && !self.room.featureConfig.enableWhisperToTeacherWhenForbidAll) || self.room.chatVM.forbidMyGroup || self.room.chatVM.forbidMe);
    BOOL forbid = isStudentForbid || isStudentPrivateForbid;
    BOOL secretForbid = self.room.featureConfig.useSecretMsgSendForbid;
    if (forbid && secretForbid) {
        if (self.secretForbidMessageCallback) self.secretForbidMessageCallback(message, targetUser);
        self.textView.text = nil;
        [self textViewDidChange:self.textView];
        return NO;
    }
    else if (forbid && !secretForbid) {
        return NO;
    }

    BJLError *error = (message
                           ? [self.room.chatVM sendMessage:message toUser:targetUser]
                       : data ? [self.room.chatVM sendMessageData:data toUser:targetUser]
                              : nil);
    ...
    /*
     ...
     [onOff addObject:BJLLocalizedString(@"统计日志")];
     }
     if (LPContextInstance.consolePrintAVInfo) {
     [onOff addObject:BJLLocalizedString(@"音视频日志")];
     }
     }
     [self sendMessageAfterBack:[NSString stringWithFormat:BJLLocalizedString(@"%@已%@"),
     [onOff componentsJoinedByString:BJLLocalizedString(@"、")] ?: BJLLocalizedString(@"控制台"),
     LPContextInstance.consolePrintEnabled ? BJLLocalizedString(@"开启") : BJLLocalizedString(@"关闭")]];
     return YES;
     }
     #endif // */
}
...
- (void)photoPicker:(BJLPhotoPicker *)BJLPhotoPicker didFinishLoadImageData:(NSArray<ICLImageFile *> *)data failureItems:(NSArray<NSError *> *)failureItems originResult:(BJLPhotoPickerResult *)result {
    ...
    [BJLPhotoPicker.viewController dismissViewControllerAnimated:YES completion:^{
        bjl_strongify(self) if (self.selectImageFileCallback) self.selectImageFileCallback(data.firstObject, nil);
    }];
}
