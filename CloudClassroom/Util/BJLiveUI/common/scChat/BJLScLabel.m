...
- (instancetype)initWitMinHeadCount:(NSInteger)minHeadCount
    ...
    if (self = [super initWithFrame:CGRectZero]) {
        ...
        self.textColor = textColor ?: [UIColor systemGrayColor];
...
- (void)makeSubviewAndConstraints {
    self.normalLabel = [self makeLabelWithAccessibilityIdentifier:BJLKeypath(self, normalLabel)];
    self.subLabel = [self makeLabelWithAccessibilityIdentifier:BJLKeypath(self, subLabel)];
    self.headLabel = [self makeLabelWithAccessibilityIdentifier:BJLKeypath(self, headLabel)];
    self.midLabel = [self makeLabelWithAccessibilityIdentifier:BJLKeypath(self, midLabel)];
    self.midLabel.textAlignment = NSTextAlignmentCenter;
    self.tailLabel = [self makeLabelWithAccessibilityIdentifier:BJLKeypath(self, tailLabel)];
...
- (void)setMinSizeExpectedLayout:(CGFloat)normalLabelWidth
    ...
    // 布局需要的最小尺寸的视图
    [self.normalLabel bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
        self.normalLabelConstraint = make.width.equalTo(@(normalLabelWidth)).priorityHigh().constraint;
    }];
    [self.headLabel bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
...
- (CGFloat)subLabelWidth {
    CGFloat subLabelWidth = [self bjl_oneRowSizeWithText:self.subLabel.text attributedText:nil fontSize:self.fontSize].width;
    return ceil(subLabelWidth);
}

- (CGFloat)midLabelWidth {
    CGFloat midLabelWidth = [self bjl_oneRowSizeWithText:self.midLabel.text attributedText:nil fontSize:self.fontSize].width;
...
- (CGFloat)omitWidth {
    ...
    dispatch_once(&onceToken, ^{
        omitWidth = [self bjl_oneRowSizeWithText:@"..." attributedText:nil fontSize:self.fontSize].width;
...
- (CGFloat)normalLabelWidth {
    CGFloat normalLabelWidth = [self bjl_oneRowSizeWithText:self.normalLabel.text attributedText:nil fontSize:self.fontSize].width;
    return ceil(normalLabelWidth);
}

- (CGFloat)headLabelWidth {
    CGFloat headLabelWidth = [self bjl_oneRowSizeWithText:self.headLabel.text attributedText:nil fontSize:self.fontSize].width;
    return ceil(headLabelWidth);
}

- (CGFloat)tailLabelWidth {
    CGFloat tailLabelWidth = [self bjl_oneRowSizeWithText:self.tailLabel.text attributedText:nil fontSize:self.fontSize].width;
