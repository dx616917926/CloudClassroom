...
- (instancetype)initWithNeedShowOnlyTeacherOrAssistant:(BOOL)needShowOnlyTeacherOrAssistant
...
- (void)updateButtonConstraints {
    ...
    self.copyingButton = [self createButtonWithTitle:BJLLocalizedString(@"复制")];
    [self.copyingButton addTarget:self action:@selector(copyMessage:) forControlEvents:UIControlEventTouchUpInside];

    self.onlyShowTeacherORAssistantMessageButton = [self createButtonWithTitle:BJLLocalizedString(@"只看老师/助教")];
    ...
    self.recallButton = [self createButtonWithTitle:self.recallType == BJLRecallTypeDelete ? BJLLocalizedString(@"删除") : BJLLocalizedString(@"撤回")];
    [self.recallButton addTarget:self action:@selector(recallMessage:) forControlEvents:UIControlEventTouchUpInside];

    self.stickyMessageButton = [self createButtonWithTitle:self.isSticky ? BJLLocalizedString(@"取消置顶") : BJLLocalizedString(@"置顶")];
