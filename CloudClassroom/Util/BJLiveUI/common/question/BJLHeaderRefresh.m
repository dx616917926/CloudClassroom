...
static NSString *BJL_Refresh_normal_title = BJLConstantString(@"↓ 下拉刷新");
static NSString *BJL_Refresh_pulling_title = BJLConstantString(@"↑ 释放刷新");
static NSString *BJL_Refresh_Refreshing_title = BJLConstantString(@"正在刷新");
...
@property (nonatomic, strong, nullable) id<BJLObservation> offsetObserver;
...
- (void)setupUI {
    ...
    self.label.text = BJLLocalizedString(BJL_Refresh_normal_title);
...
- (void)willMoveToSuperview:(UIView *)newSuperview {
    ...
    if ([newSuperview isKindOfClass:[UIScrollView class]]) {
        ...
        if (self.offsetObserver) {
            [self.offsetObserver stopObserving];
            self.offsetObserver = nil;
        }
        bjl_weakify(self);
        self.offsetObserver = [self bjl_kvo:BJLMakeProperty(self.superScrollView, contentOffset) observer:^BJLControlObserving(id _Nullable value, id _Nullable oldValue, BJLPropertyChange *_Nullable change) {
            bjl_strongify(self);
            [self observerAction];
            return YES;
        }];
...
- (void)updateState:(BJLRefreshState)refreshState {
    ...
    switch (refreshState) {
        ...
        case BJLRefreshStatePulling:
            self.label.text = BJLLocalizedString(BJL_Refresh_pulling_title);
            [self.label sizeToFit];
            break;
