...
- (void)firstRequestRefreshPage {
    if (self.room.loginUser.isTeacherOrAssistant) {
        [self.unreplySource requestQuestionHistory];
        [self.unpublishSource requestQuestionHistory];
        [self.publishSource requestQuestionHistory];
    }
    else {
        [self.myQuestionSouece requestQuestionHistory];
        [self.publishSource requestQuestionHistory];
    }
}
...
- (void)makeObserving {
             ...
             observer:(BJLMethodFilter) ^ BOOL(BJLUser * user, BOOL forbid) {
                 ...
                 if (self.room.loginUser.isTeacherOrAssistant) {
                     ...
                     if ([self.room.loginUser.number isEqualToString:user.number]) {
                         ...
                         self.questionButton.backgroundColor = forbid ? [UIColor bjl_colorWithHexString:@"#EEEEEE"] : [UIColor bjl_colorWithHexString:@"#9FA8B5" alpha:0.1];
                     }
                 }
    ...
    [self bjl_kvo:BJLMakeProperty(self.room.roomVM, forbidQuestionList)
         observer:^BOOL(id _Nullable value, id _Nullable oldValue, BJLPropertyChange *_Nullable change) {
             ...
             self.questionButton.backgroundColor = forbidMe ? [UIColor bjl_colorWithHexString:@"#EEEEEE"] : [UIColor bjl_colorWithHexString:@"#9FA8B5" alpha:0.1];
             return YES;
         }];
}
...
- (void)updateListWithSegmentIndex:(NSInteger)segmentIndex {
    ...
    // 学生
        ...
        switch (segmentIndex) {
            case 0:
                self.emptyLabel.text = BJLLocalizedString(@"没有提问哦~");
                self.currentQuestionList = self.myQuestionSouece.questionList;
                break;

            case 1:
                self.emptyLabel.text = BJLLocalizedString(@"没有公布的问答哦~");
                self.currentQuestionList = self.publishSource.questionList;
                break;
    ...
    if (hidden) {
        ...
        if (!self.emptyView || !self.emptyLabel) {
            self.emptyView = ({
                ...
                imageView.image = [UIImage bjl_imageNamed:@"bjl_sc_notice_noPersonSpeak"];
            ...
            [self.emptyView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
                ...
                make.width.equalTo(self.containerView).multipliedBy(isPortrait ? 0.65 : 0.4);
                make.height.equalTo(isPortrait ? @150.0 : @120.0);
            }];
            ...
            if (self.room.loginUser.isTeacherOrAssistant) {
                self.emptyLabel.text = BJLLocalizedString(@"还没有人提问哦~");
            }
            else {
                if (0 == self.segment.selectedIndex) {
                    self.emptyLabel.text = BJLLocalizedString(@"没有提问哦~");
