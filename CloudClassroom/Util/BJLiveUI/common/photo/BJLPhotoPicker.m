...
UIImage *_Nullable loadAndDownsampleImageFromURL(NSURL *url, NSError **error);

@interface BJLPHPickerResultLoadTaskConfiguration: NSObject
@end
@implementation BJLPHPickerResultLoadTaskConfiguration
@end
...
UIImage *_Nullable loadAndDownsampleImageFromURL(NSURL *url, NSError **error) {
    UIImage *image = nil;
    NSString *erroDomain = @"com.bjy.loadAndDownsampleImageFromURL";
    if (url == nil) {
        if (error) {
            *error = [NSError errorWithDomain:erroDomain code:-1 userInfo:@{NSLocalizedDescriptionKey: @"url is null"}];
        }
        return image;
    }

    CGImageSourceRef imgSourceRef = NULL;
    CGImageRef sampleDownImageRef = NULL;
    CGImageDestinationRef imageDestRef = NULL;

    NSDictionary *imgSourceOpt = @{(NSString *)kCGImageSourceShouldCache: @(false)};
    imgSourceRef = CGImageSourceCreateWithURL((CFURLRef)url, (__bridge CFDictionaryRef)imgSourceOpt);
    if (imgSourceRef != NULL) {
        ...
        sampleDownImageRef = CGImageSourceCreateThumbnailAtIndex(imgSourceRef, 0, (__bridge CFDictionaryRef)sampleDownImageOpt);
        if (sampleDownImageRef != NULL) {
            NSMutableData *imageData = [NSMutableData data];
            imageDestRef = CGImageDestinationCreateWithData((__bridge CFMutableDataRef)imageData, kUTTypeJPEG, 1, NULL);
            if (imageDestRef != NULL) {
                ...
                if (error) {
                    *error = [NSError errorWithDomain:erroDomain code:-4 userInfo:@{NSLocalizedDescriptionKey: @"CGImageDestinationCreateWithData failed."}];
                }
            }
        }
        else {
            if (error) {
                *error = [NSError errorWithDomain:erroDomain code:-3 userInfo:@{NSLocalizedDescriptionKey: @"CGImageSourceCreateThumbnailAtIndex failed."}];
            }
        }
    }
    else {
        if (error) {
            *error = [NSError errorWithDomain:erroDomain code:-2 userInfo:@{NSLocalizedDescriptionKey: @"CGImageSourceCreateWithURL failed."}];
        }
    }
...
NSURL *copyImageFromSystemTempPathToSandboxTempPath(NSURL *from, NSError **error) {
    ...
    NSFileHandle *handle = [NSFileHandle fileHandleForReadingAtPath:from.path];
    NSData *imageData = [handle readDataOfLength:12]; // 有些magic number >= 12 byte
    [handle closeFile];
    NSString *suffix = [imageData bjl_imageFormatString];
