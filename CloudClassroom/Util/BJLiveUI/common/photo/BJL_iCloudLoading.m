...
+ (nullable instancetype)imageFileWithPickerResult:(PHPickerResult *)pickerResult thumbnail:(nullable UIImage *)thumbnail filePath:(NSString *)filePath error:(NSError *__autoreleasing _Nullable *)error {
    ...
    imageFile.thumbnail = thumbnail;
    imageFile.filePath = filePath;
    imageFile.imageSize = thumbnail.size;
    return imageFile;
}
...
- (NSString *)description {
    return [NSString stringWithFormat:@"<%@: %p> %@ (%@), %@", // @"<%@: %p> %@ (%@ - %td), %@"
                     ...
                     self,
                     self.filePath,
                     self.mediaType /* , self.orientation */,
                     self.asset];
}
...
- (void)_icl_loadImageFilesWithAssets:(NSArray<PHAsset *> *)assets
    ...
    void (^__block resultHandler)(UIImage *_Nullable image, NSDictionary *_Nullable info) = ^(UIImage *_Nullable image, NSDictionary *_Nullable info) {
        ...
        // resultHandler: "for asynchronous requests, always called on main thread"
        thumbnailRequestID = [thumbnailManager requestImageForAsset:asset
                                                         targetSize:thumbnailSize
                                                        contentMode:thumbnailContentMode
                                                            options:thumbnailOptions
                                                      resultHandler:^(UIImage *_Nullable thumbnail, NSDictionary *_Nullable info) {
                                                          ...
                                                          if ([self.delegate respondsToSelector:@selector(icl_imagePickerController:didFinishLoadingImageFile:)]) {
                                                              [(id<QBImagePickerControllerDelegate_iCloudLoading>)self.delegate
                                                                  icl_imagePickerController:self
                                                                  didFinishLoadingImageFile:imageFile];
                                                          }
                                                          ...
                                                          if (!asset) {
                                                              [self icl_stopLoadingWithAlert:loadingAlert completion:^{
                                                                  if ([self.delegate respondsToSelector:@selector(icl_imagePickerController:didFinishLoadingImageFiles:)]) {
                                                                      [(id<QBImagePickerControllerDelegate_iCloudLoading>)self.delegate
                                                                           icl_imagePickerController:self
                                                                          didFinishLoadingImageFiles:imageFiles];
                                                                  }
                                                              }];
                                                              resultHandler = nil;
                                                              return;
                                                          ...
                                                          imageRequestID = [imageManager requestImageForAsset:asset
                                                                                                   targetSize:targetSize
                                                                                                  contentMode:contentMode
...
- (void)icl_alertError {
    UIAlertController *alert = [UIAlertController
        alertControllerWithTitle:BJLLocalizedString(@"无法下载照片")
                         message:BJLLocalizedString(@"从“iCloud 照片图库”下载这张照片时出错。请稍后再试。")
                  preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction
                         actionWithTitle:BJLLocalizedString(@"好")
...
- (UIAlertController *)icl_loadingAlertWithProgressView:(UIView *)progressView
    ...
    UIAlertController *alert = [UIAlertController
        alertControllerWithTitle:BJLLocalizedString(@"正在从 iCloud 加载...")
                         message:@"\n\n\n\n\n"
                  preferredStyle:UIAlertControllerStyleAlert];
    [alert addAction:[UIAlertAction
                         actionWithTitle:BJLLocalizedString(@"取消")
                                   style:UIAlertActionStyleCancel
                                 handler:handler]];
