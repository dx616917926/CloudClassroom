...
- (void)makeSubviews {
    ...
    self.headerContainerView = ({
        ...
        [line bjl_makeConstraints:^(BJLConstraintMaker *make) {
            make.left.equalTo(view).with.offset(viewSpaceL);
            make.right.greaterThanOrEqualTo(view); // fix warning if view.width == 0.0
            make.bottom.equalTo(view);
            make.height.equalTo(@(BJLScOnePixel));
        }];
        view.accessibilityIdentifier = BJLKeypath(self, headerContainerView);
    ...
    self.footerContainerView = ({
        UIView *view = [UIView new];
        view.backgroundColor = [UIColor whiteColor];
        view.accessibilityIdentifier = BJLKeypath(self, footerContainerView);
        [self.view addSubview:view];
        view;
    });
    ...
    self.contentContainerView = ({
        ...
        view.accessibilityIdentifier = BJLKeypath(self, contentContainerView);
        view;
    });
    ...
    self.buttonsStackView = ({
        ...
        view.accessibilityIdentifier = BJLKeypath(self, buttonsStackView);
...
- (void)makeConstraints {
    ...
    [self.titleLabel bjl_makeConstraints:^(BJLConstraintMaker *make) {
        make.left.equalTo(self.headerContainerView.bjl_safeAreaLayoutGuide ?: self.headerContainerView).with.offset(viewSpaceL);
    ...
    [self.buttonsStackView bjl_makeConstraints:^(BJLConstraintMaker *make) {
        ...
        make.left.greaterThanOrEqualTo(self.titleLabel.bjl_right).with.offset(viewSpaceL);
...
- (void)updateHeaderViewConstraints {
    ...
    CGFloat headerHeight = shown ? controlSize : 0.0;
...
- (void)updateRightButtons:(nullable NSArray<UIButton *> *)rightButtons {
    ...
    for (UIButton *button in self.rightButtons) {
        ...
        [button bjl_makeConstraints:^(BJLConstraintMaker *make) {
            make.right.equalTo(last.bjl_left ?: self.buttonsStackView).with.offset(-(last ? viewSpaceM : viewSpaceL));
