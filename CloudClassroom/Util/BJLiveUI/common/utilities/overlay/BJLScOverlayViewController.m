...
- (void)removeContentViewController {
    ...
    UIViewController *lastChild = self.childViewControllers.lastObject;
    if (lastChild) {
        self.viewController = lastChild;
    }
    else {
...
- (void)handleTapGesture:(UITapGestureRecognizer *)tapGesture {
    if (self.tapToHide) {
        if (self.tapCallback) {
            if (self.tapCallback(self.viewController)) {
                [self hide];
            }
        }
        else {
        ...
        if (self.tapCallback) {
            self.tapCallback(self.viewController);
        }
    }
}
