//
//  BJLWindowViewController+protected.m
//  BJLiveUI
//
...
@implementation BJLWindowViewController (protected)
...
- (CGFloat)relativeWidthWithRelativeHeight:(CGFloat)relativeHeight aspectRatio:(CGFloat)aspectRatio {
    CGFloat relativeWidth = relativeHeight / BJLAppearance.blackboardAspectRatio;
...
- (void)addHandlers {
    ...
    [self.topBar.maximizeButton bjl_addHandler:^(__kindof UIControl *_Nullable sender) {
        ...
        [self didSelectTopBarButton:self.topBar.maximizeButton];
    ...
    [self.topBar.fullscreenButton bjl_addHandler:^(__kindof UIControl *_Nullable sender) {
        ...
        [self didSelectTopBarButton:self.topBar.fullscreenButton];
    ...
    [self.topBar.closeButton bjl_addHandler:^(__kindof UIControl *_Nullable sender) {
        ...
        [self didSelectTopBarButton:self.topBar.closeButton];
        [self close];
    }];
...
#pragma mark - user change state

- (void)didSelectTopBarButton:(UIButton *)button {
    BJLWindowState state = self.state;
    if (button == self.topBar.maximizeButton) {
        state = button.isSelected ? BJLWindowState_maximized : BJLWindowState_windowed;
    }
    if (button == self.topBar.fullscreenButton) {
        state = button.isSelected                       ? BJLWindowState_fullscreen
                : self.topBar.maximizeButton.isSelected ? BJLWindowState_maximized
                                                        : BJLWindowState_windowed;
    }
    if (button == self.topBar.closeButton) {
        state = BJLWindowState_closed;
    }
    if (self.didWindowStateUserChanged) {
        self.didWindowStateUserChanged(state);
    }
}
...
- (void)makeObserveringForContent {
    ...
    [self bjl_kvo:BJLMakeProperty(self, topBarHidden)
        ...
        observer:^BOOL(NSNumber *_Nullable now, NSNumber *_Nullable old, BJLPropertyChange *_Nullable change) {
            bjl_strongify(self);
            self.topBar.hidden = now.boolValue || !self.windowGesturesEnabled;
            return YES;
        }];
    ...
    [self bjl_kvo:BJLMakeProperty(self, bottomBarHidden)
        filter:^BOOL(NSNumber *_Nullable now, NSNumber *_Nullable old, BJLPropertyChange *_Nullable change) {
            // bjl_strongify(self);
            return now != old && now.boolValue != old.boolValue;
        }
        observer:^BOOL(NSNumber *_Nullable now, NSNumber *_Nullable old, BJLPropertyChange *_Nullable change) {
            bjl_strongify(self);
            self.bottomBar.hidden = now.boolValue || !self.windowGesturesEnabled;
            return YES;
        }];
    ...
    [self bjl_kvo:BJLMakeProperty(self, windowGesturesEnabled)
        filter:^BOOL(NSNumber *_Nullable now, NSNumber *_Nullable old, BJLPropertyChange *_Nullable change) {
            // bjl_strongify(self);
            return now != old && now.integerValue != old.integerValue;
        }
        observer:^BOOL(NSNumber *_Nullable now, NSNumber *_Nullable old, BJLPropertyChange *_Nullable change) {
            ...
            for (UIGestureRecognizer *gesture in self.windowGestures) {
                gesture.enabled = self.windowInterfaceEnabled && self.windowGesturesEnabled;
            }
            // 对于 forgroundView 的可交互性，不受窗口拖动权限影响，当开启画笔，开启可操作PPT的状态时，都将设置为 NO 来穿透这个视图，传到底层视图
            self.forgroundView.userInteractionEnabled = self.windowGesturesEnabled;
            // 当需要传递事件给底层内容视图时，隐藏 topBar 和 bottomBar
            self.topBar.hidden = self.topBarHidden || !self.windowGesturesEnabled;
            self.bottomBar.hidden = self.bottomBarHidden || !self.windowGesturesEnabled;
            return YES;
        }];
}
