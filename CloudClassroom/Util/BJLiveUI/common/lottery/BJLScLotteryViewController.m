...
- (instancetype)initWithRoom:(BJLRoom *)room lottery:(BJLLottery *)lottery {
    self = [super init];
    if (self) {
        self.room = room;
...
- (void)submitAction {
    ...
    if (!name.length) {
        [self.submitView.nameTextField updateTip:BJLLocalizedString(@"请输入名字")];
    }
    if (!mobile.length) {
        [self.submitView.phoneTextField updateTip:BJLLocalizedString(@"请输入手机号")];
        return;
    }
    ...
    if (![phoneTest evaluateWithObject:mobile]) {
        [self.submitView.phoneTextField updateTip:BJLLocalizedString(@"输入有误")];
        return;
    }
...
- (void)closeConfirm {
    ...
    UIAlertController *alertVC = [UIAlertController alertControllerWithTitle:BJLLocalizedString(@"提示") message:BJLLocalizedString(@"你还没有提交信息，确认关闭吗") preferredStyle:UIAlertControllerStyleAlert];
    bjl_weakify(self);
    UIAlertAction *action1 = [UIAlertAction actionWithTitle:BJLLocalizedString(@"确认") style:UIAlertActionStyleDestructive handler:^(UIAlertAction *_Nonnull action) {
        bjl_strongify(self);
        [self hideLotteryViews];
    }];
