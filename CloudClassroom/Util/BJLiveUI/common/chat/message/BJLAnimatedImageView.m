...
@protocol BJLAnimatedImageViewDebugDelegate <NSObject>
@optional
- (void)debug_animatedImageView:(BJLAnimatedImageView *)animatedImageView waitingForFrame:(NSUInteger)index duration:(NSTimeInterval)duration;
@end
#endif

@interface BJLAnimatedImageView ()
...
#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<BJLAnimatedImageViewDebugDelegate> debug_delegate;
#endif

@end

@implementation BJLAnimatedImageView
...
- (void)setAnimatedImage:(BJLAnimatedImage *)animatedImage {
    if (![_animatedImage isEqual:animatedImage]) {
        if (animatedImage) {
            if (super.image) {
                ...
                // See more here: https://github.com/Flipboard/BJLAnimatedImage/issues/100
...
- (NSTimeInterval)frameDelayGreatestCommonDivisor {
    // Presision is set to half of the `kBJLAnimatedImageDelayTimeIntervalMinimum` in order to minimize frame dropping.
    const NSTimeInterval kGreatestCommonDivisorPrecision = 2.0 / kBJLAnimatedImageDelayTimeIntervalMinimum;
...
- (void)startAnimating {
    if (self.animatedImage) {
        ...
        if (!self.displayLink) {
            ...
            BJLWeakProxy *weakProxy = [BJLWeakProxy weakProxyForObject:self];
...
- (void)displayDidRefresh:(CADisplayLink *)displayLink {
    ...
    // Early return!
    if (!self.shouldAnimate) {
        BJLLog(BJLLogLevelWarn, @"Trying to animate image when we shouldn't: %@", self);
        return;
    }
    ...
    if (delayTimeNumber != nil) {
        ...
        if (image) {
            BJLLog(BJLLogLevelVerbose, @"Showing frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
...
#pragma clang diagnostic pop
            ...
            BJLLog(BJLLogLevelDebug, @"Waiting for frame %lu for animated image: %@", (unsigned long)self.currentFrameIndex, self.animatedImage);
#if defined(DEBUG) && DEBUG
