...
const NSTimeInterval kBJLAnimatedImageDelayTimeIntervalMinimum = 0.02;

// An animated image's data size (dimensions * frameCount) category; its value is the max allowed memory (in MB).
// E.g.: A 100x200px GIF with 30 frames is ~2.3MB in our pixel format and would fall into the `BJLAnimatedImageDataSizeCategoryAll` category.
typedef NS_ENUM(NSUInteger, BJLAnimatedImageDataSizeCategory) {
    BJLAnimatedImageDataSizeCategoryAll = 10, // All frames permanently in memory (be nice to the CPU)
    BJLAnimatedImageDataSizeCategoryDefault = 75, // A frame cache of default size in memory (usually real-time performance and keeping low memory profile)
    BJLAnimatedImageDataSizeCategoryOnDemand = 250, // Only keep one frame at the time in memory (easier on memory, slowest performance)
    BJLAnimatedImageDataSizeCategoryUnsupported // Even for one frame too large, computer says no.
};

typedef NS_ENUM(NSUInteger, BJLAnimatedImageFrameCacheSize) {
    BJLAnimatedImageFrameCacheSizeNoLimit = 0, // 0 means no specific limit
    BJLAnimatedImageFrameCacheSizeLowMemory = 1, // The minimum frame cache size; this will produce frames on-demand.
    BJLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning = 2, // If we can produce the frames faster than we consume, one frame ahead will already result in a stutter-free playback.
    BJLAnimatedImageFrameCacheSizeDefault = 5 // Build up a comfy buffer window to cope with CPU hiccups etc.
};

#if defined(DEBUG) && DEBUG
@protocol BJLAnimatedImageDebugDelegate <NSObject>
@optional
- (void)debug_animatedImage:(BJLAnimatedImage *)animatedImage didUpdateCachedFrames:(NSIndexSet *)indexesOfFramesInCache;
- (void)debug_animatedImage:(BJLAnimatedImage *)animatedImage didRequestCachedFrame:(NSUInteger)index;
- (CGFloat)debug_animatedImagePredrawingSlowdownFactor:(BJLAnimatedImage *)animatedImage;
@end
#endif

@interface BJLAnimatedImage ()
...
// The actual type of the object is `BJLWeakProxy`.
@property (nonatomic, strong, readonly) BJLAnimatedImage *weakProxy;

#if defined(DEBUG) && DEBUG
@property (nonatomic, weak) id<BJLAnimatedImageDebugDelegate> debug_delegate;
#endif
...
@implementation BJLAnimatedImage
...
- (NSUInteger)frameCacheSizeCurrent {
    ...
    if (self.frameCacheSizeMax > BJLAnimatedImageFrameCacheSizeNoLimit) {
        frameCacheSizeCurrent = MIN(frameCacheSizeCurrent, self.frameCacheSizeMax);
    }

    if (self.frameCacheSizeMaxInternal > BJLAnimatedImageFrameCacheSizeNoLimit) {
...
+ (void)initialize {
    if (self == [BJLAnimatedImage class]) {
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIApplicationDidReceiveMemoryWarningNotification object:nil queue:nil usingBlock:^(NSNotification *note) {
            ...
            // Note that BJLAnimatedImages can be created on any thread, so the hash table must be locked.
            NSArray *images = nil;
...
- (instancetype)init {
    BJLAnimatedImage *animatedImage = [self initWithAnimatedGIFData:nil];
    if (!animatedImage) {
        BJLLog(BJLLogLevelError, @"Use `-initWithAnimatedGIFData:` and supply the animated GIF data as an argument to initialize an object of type `BJLAnimatedImage`.");
    }
...
- (instancetype)initWithAnimatedGIFData:(NSData *)data optimalFrameCacheSize:(NSUInteger)optimalFrameCacheSize predrawingEnabled:(BOOL)isPredrawingEnabled {
    ...
    if (!hasData) {
        BJLLog(BJLLogLevelError, @"No animated GIF data supplied.");
        return nil;
    }
    ...
    if (self) {
        ...
        if (!_imageSource) {
            BJLLog(BJLLogLevelError, @"Failed to `CGImageSourceCreateWithData` for animated GIF data %@", data);
            return nil;
        }
        ...
        if (!isGIFData) {
            BJLLog(BJLLogLevelError, @"Supplied data is of type %@ and doesn't seem to be GIF data %@", imageSourceContainerType, data);
            return nil;
        }
            ...
            @autoreleasepool {
                ...
                if (frameImageRef) {
                    ...
                    if (frameImage) {
                        ...
                        if (delayTime == nil) {
                            if (i == 0) {
                                BJLLog(BJLLogLevelInfo, @"Falling back to default delay time for first frame %@ because none found in GIF properties %@", frameImage, frameProperties);
                                delayTime = @(kDelayTimeIntervalDefault);
                            }
                            else {
                                BJLLog(BJLLogLevelInfo, @"Falling back to preceding delay time for frame %zu %@ because none found in GIF properties %@", i, frameImage, frameProperties);
                                delayTime = delayTimesForIndexesMutable[@(i - 1)];
                            }
                        }
                        // Support frame delays as low as `kBJLAnimatedImageDelayTimeIntervalMinimum`, with anything below being rounded up to `kDelayTimeIntervalDefault` for legacy compatibility.
                        ...
                        if ([delayTime floatValue] < ((float)kBJLAnimatedImageDelayTimeIntervalMinimum - FLT_EPSILON)) {
                            BJLLog(BJLLogLevelInfo, @"Rounding frame %zu's `delayTime` from %f up to default %f (minimum supported: %f).", i, [delayTime floatValue], kDelayTimeIntervalDefault, kBJLAnimatedImageDelayTimeIntervalMinimum);
                            delayTime = @(kDelayTimeIntervalDefault);
                        }
                        ...
                        skippedFrameCount++;
                        BJLLog(BJLLogLevelInfo, @"Dropping frame %zu because valid `CGImageRef` %@ did result in `nil`-`UIImage`.", i, frameImageRef);
                    }
                    CFRelease(frameImageRef);
                }
                else {
                    skippedFrameCount++;
                    BJLLog(BJLLogLevelInfo, @"Dropping frame %zu because failed to `CGImageSourceCreateImageAtIndex` with image source %@", i, self->_imageSource);
                }
            }
        }
        ...
        if (self.frameCount == 0) {
            BJLLog(BJLLogLevelInfo, @"Failed to create any valid frames for GIF with properties %@", imageProperties);
            return nil;
        }
        else if (self.frameCount == 1) {
            // Warn when we only have a single frame but return a valid GIF.
            BJLLog(BJLLogLevelInfo, @"Created valid GIF but with only a single frame. Image properties: %@", imageProperties);
        }
        else {
        ...
        if (optimalFrameCacheSize == 0) {
            ...
            if (animatedImageDataSize <= BJLAnimatedImageDataSizeCategoryAll) {
                _frameCacheSizeOptimal = self.frameCount;
            }
            else if (animatedImageDataSize <= BJLAnimatedImageDataSizeCategoryDefault) {
                ...
                _frameCacheSizeOptimal = BJLAnimatedImageFrameCacheSizeDefault;
            }
            else {
                // The predicted size exceeds the limits to build up a cache and we go into low memory mode from the beginning.
                _frameCacheSizeOptimal = BJLAnimatedImageFrameCacheSizeLowMemory;
            }
        }
        else {
        ...
        _weakProxy = (id)[BJLWeakProxy weakProxyForObject:self];
...
+ (instancetype)animatedImageWithGIFData:(NSData *)data {
    BJLAnimatedImage *animatedImage = [[BJLAnimatedImage alloc] initWithAnimatedGIFData:data];
    return animatedImage;
}

+ (instancetype)animatedImageWithGIFData:(NSData *)data loopCount:(NSUInteger)loopCount {
    BJLAnimatedImage *animatedImage = [[BJLAnimatedImage alloc] initWithAnimatedGIFData:data];
    if (loopCount >= 0 && animatedImage) {
        animatedImage->_loopCount = loopCount;
    }
    return animatedImage;
}
...
- (UIImage *)imageLazilyCachedAtIndex:(NSUInteger)index {
    ...
    if (index >= self.frameCount) {
        BJLLog(BJLLogLevelWarn, @"Skipping requested frame %lu beyond bounds (total frame count: %lu) for animated image: %@", (unsigned long)index, (unsigned long)self.frameCount, self);
        return nil;
    }
...
- (void)addFrameIndexesToCache:(NSIndexSet *)frameIndexesToAddToCache {
    ...
    if (firstRange.length + secondRange.length != self.frameCount) {
        BJLLog(BJLLogLevelWarn, @"Two-part frame cache range doesn't equal full range.");
    }
    ...
    BJLAnimatedImage *__weak weakSelf = self;
    dispatch_async(self.serialQueue, ^{
        ...
        void (^frameRangeBlock)(NSRange, BOOL *) = ^(NSRange range, BOOL *stop) {
            ...
            for (NSUInteger i = range.location; i < NSMaxRange(range); i++) {
                ...
                BJLLog(BJLLogLevelVerbose, @"Predrew frame %lu in %f ms for animated image: %@", (unsigned long)i, (predrawDuration + slowdownDuration) * 1000, self);
#endif
...
+ (CGSize)sizeForImage:(id)image {
    ...
    else if ([image isKindOfClass:[BJLAnimatedImage class]]) {
        BJLAnimatedImage *animatedImage = (BJLAnimatedImage *)image;
        imageSize = animatedImage.size;
    }
    else {
        // Bear trap to capture bad images; we have seen crashers cropping up on iOS 7.
        BJLLog(BJLLogLevelError, @"`image` isn't of expected types `UIImage` or `BJLAnimatedImage`: %@", image);
    }
...
- (NSMutableIndexSet *)frameIndexesToCache {
    ...
    if (self.frameCacheSizeCurrent == self.frameCount) {
        ...
        if ([indexesToCache count] != self.frameCacheSizeCurrent) {
            BJLLog(BJLLogLevelWarn, @"Number of frames to cache doesn't equal expected cache size.");
        }
...
- (void)growFrameCacheSizeAfterMemoryWarning:(NSNumber *)frameCacheSize {
    ...
    BJLLog(BJLLogLevelDebug, @"Grew frame cache size max to %lu after memory warning for animated image: %@", (unsigned long)self.frameCacheSizeMaxInternal, self);
...
- (void)resetFrameCacheSizeMaxInternal {
    self.frameCacheSizeMaxInternal = BJLAnimatedImageFrameCacheSizeNoLimit;
    BJLLog(BJLLogLevelDebug, @"Reset frame cache size max (current frame cache size: %lu) for animated image: %@", (unsigned long)self.frameCacheSizeCurrent, self);
}
...
- (void)didReceiveMemoryWarning:(NSNotification *)notification {
    ...
    [NSObject cancelPreviousPerformRequestsWithTarget:self.weakProxy selector:@selector(growFrameCacheSizeAfterMemoryWarning:) object:@(BJLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning)];
    ...
    BJLLog(BJLLogLevelDebug, @"Attempt setting frame cache size max to %lu (previous was %lu) after memory warning #%lu for animated image: %@", (unsigned long)BJLAnimatedImageFrameCacheSizeLowMemory, (unsigned long)self.frameCacheSizeMaxInternal, (unsigned long)self.memoryWarningCount, self);
    self.frameCacheSizeMaxInternal = BJLAnimatedImageFrameCacheSizeLowMemory;
    ...
    if ((self.memoryWarningCount - 1) <= kGrowAttemptsMax) {
        [self.weakProxy performSelector:@selector(growFrameCacheSizeAfterMemoryWarning:) withObject:@(BJLAnimatedImageFrameCacheSizeGrowAfterMemoryWarning) afterDelay:kGrowDelay];
    }
...
+ (UIImage *)predrawnImageFromImage:(UIImage *)imageToPredraw {
    ...
    if (!colorSpaceDeviceRGBRef) {
        BJLLog(BJLLogLevelError, @"Failed to `CGColorSpaceCreateDeviceRGB` for image %@", imageToPredraw);
    ...
    // Early return on failure!
    if (!bitmapContextRef) {
        BJLLog(BJLLogLevelError, @"Failed to `CGBitmapContextCreate` with color space %@ and parameters (width: %zu height: %zu bitsPerComponent: %zu bytesPerRow: %zu) for image %@", colorSpaceDeviceRGBRef, width, height, bitsPerComponent, bytesPerRow, imageToPredraw);
        return imageToPredraw;
    }
    ...
    // Early return on failure!
    if (!predrawnImage) {
        BJLLog(BJLLogLevelError, @"Failed to `imageWithCGImage:scale:orientation:` with image ref %@ created with color space %@ and bitmap context %@ and properties and properties (scale: %f orientation: %ld) for image %@", predrawnImageRef, colorSpaceDeviceRGBRef, bitmapContextRef, imageToPredraw.scale, (long)imageToPredraw.imageOrientation, imageToPredraw);
        return imageToPredraw;
    }
...
@implementation BJLAnimatedImage (Logging)

static void (^_logBlock)(NSString *logString, BJLLogLevel logLevel) = nil;
static BJLLogLevel _logLevel;

+ (void)setLogBlock:(void (^)(NSString *logString, BJLLogLevel logLevel))logBlock logLevel:(BJLLogLevel)logLevel {
    _logBlock = logBlock;
    _logLevel = logLevel;
}

+ (void)logStringFromBlock:(NSString * (^)(void))stringBlock withLevel:(BJLLogLevel)level {
...
#pragma mark - BJLWeakProxy

@interface BJLWeakProxy ()

@property (nonatomic, weak) id target;

@end

@implementation BJLWeakProxy

#pragma mark Life Cycle

// This is the designated creation method of an `BJLWeakProxy` and
// as a subclass of `NSProxy` it doesn't respond to or need `-init`.
+ (instancetype)weakProxyForObject:(id)targetObject {
    BJLWeakProxy *weakProxy = [BJLWeakProxy alloc];
