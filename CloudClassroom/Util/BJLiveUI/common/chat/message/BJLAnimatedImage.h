...
extern const NSTimeInterval kBJLAnimatedImageDelayTimeIntervalMinimum;
...
//
@interface BJLAnimatedImage: NSObject
...
// define with loopCount. 0 means repeating the animation forever.
+ (instancetype)animatedImageWithGIFData:(NSData *)data loopCount:(NSUInteger)loopCount;

@end

typedef NS_ENUM(NSUInteger, BJLLogLevel) {
    BJLLogLevelNone = 0,
    BJLLogLevelError,
    BJLLogLevelWarn,
    BJLLogLevelInfo,
...
@interface BJLAnimatedImage (Logging)
...
#define BJLLog(logLevel, format, ...) [BJLAnimatedImage logStringFromBlock:^NSString * { return [NSString stringWithFormat:(format), ##__VA_ARGS__]; } withLevel:(logLevel)]

@interface BJLWeakProxy: NSProxy
