...
- (void)setUpContentView {
    ...
    self.numberLabel = ({
        ...
        label.backgroundColor = BJLTheme.subButtonBackgroundColor;
...
- (void)updateContentWithSlidePage:(BJLSlidePage *)slidePage
                   whiteboardCount:(NSInteger)whiteboardCount
         whiteboardBackgroundImage:(nullable UIImage *)whiteboardBackgroundImage
                         imageSize:(CGSize)imageSize
                    isSelectedCell:(BOOL)isSelectedCell {
    self.slidePage = slidePage;
    // background
    self.pptView.layer.borderWidth = isSelectedCell ? 2.0 : BJLScOnePixel;
    self.pptView.layer.borderColor = isSelectedCell ? BJLTheme.brandColor.CGColor : BJLTheme.separateLineColor.CGColor;
    // ppt
    ...
    if ([slidePage.documentID isEqualToString:BJLBlackboardID] && whiteboardBackgroundImage) {
        [self.pptView setImage:whiteboardBackgroundImage];
    }
    else {
    ...
    if ([slidePage.documentID isEqualToString:BJLBlackboardID]) {
        numberText = whiteboardCount > 1 ? [NSString stringWithFormat:BJLLocalizedString(@"白板%td"), slidePage.documentPageIndex] : BJLLocalizedString(@"白板");
    }
    else {
