...
- (void)viewDidLoad {
    ...
    self.view.layer.borderColor = BJLTheme.separateLineColor.CGColor;
...
- (void)deleteWhiteboardPageIndex:(NSInteger)pageIndex {
    UIAlertController *alert = [UIAlertController
        alertControllerWithTitle:BJLLocalizedString(@"确定删除？")
                         message:nil
    ...
    [alert bjl_addActionWithTitle:BJLLocalizedString(@"取消")
                          ...
                          handler:nil];
    bjl_weakify(self);
    [alert bjl_addActionWithTitle:BJLLocalizedString(@"确定")
...
- (void)setUpCollectionView {
    self.addWhiteboardButton = ({
        ...
        button.accessibilityIdentifier = BJLKeypath(self, addWhiteboardButton);
    ...
    self.whiteboardCollectionView = ({
        ...
        collectionView.accessibilityIdentifier = BJLKeypath(self, whiteboardCollectionView);
    ...
    self.pptContainerView = ({
        ...
        view.accessibilityIdentifier = BJLKeypath(self, pptContainerView);
    ...
    UIView *pullView = ({
        UIView *view = [UIView new];
        view.accessibilityIdentifier = @"pullView";
    ...
    self.pullImageView = ({
        ...
        imageView.accessibilityIdentifier = BJLKeypath(self, pullImageView);
    ...
    self.pptPreviewCollectionView = ({
        ...
        UICollectionView *collectionView = [[UICollectionView alloc] initWithFrame:CGRectZero collectionViewLayout:layout];
        collectionView.allowsSelection = NO;
        collectionView.accessibilityIdentifier = BJLKeypath(self, pptPreviewCollectionView);
        collectionView.backgroundColor = BJLTheme.windowBackgroundColor;
        collectionView.showsHorizontalScrollIndicator = NO;
        collectionView.bounces = YES;
        collectionView.alwaysBounceHorizontal = YES;
    ...
    UIImageView *promptImageView = ({
        UIImageView *imageView = [UIImageView new];
        imageView.image = [UIImage bjlsc_imageNamed:@"bjl_sc_ppt_prompt"];
        imageView;
    });

    UILabel *promptLabel = ({
        UILabel *label = [UILabel new];
        label.text = BJLLocalizedString(@"老师浏览过的课件分页在这里同步展示");
    ...
    if (self.room.loginUser.isStudent) {
        [self.pptPreviewCollectionView addSubview:promptImageView];
        [self.pptPreviewCollectionView addSubview:promptLabel];

        [promptImageView bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
            make.top.equalTo(self.pptPreviewCollectionView).offset(5);
            make.left.equalTo(self.pptPreviewCollectionView).offset(5);
        }];

        [promptLabel bjl_makeConstraints:^(BJLConstraintMaker *_Nonnull make) {
            make.top.equalTo(promptImageView);
            make.left.equalTo(promptImageView.bjl_right).offset(2);
        }];
    }
    ...
    self.finishEditWhiteboardButton = ({
        ...
        [button bjl_setTitle:BJLLocalizedString(@"完成") forState:UIControlStateNormal possibleStates:UIControlStateHighlighted];
...
- (__kindof UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (collectionView == self.pptPreviewCollectionView) {
        ...
        BOOL selected = self.loaclPageIsWhiteboard ? NO : (indexPath.row == self.localPPTPreviewIndex);
        [cell updateContentWithSlidePage:slidePage
                         whiteboardCount:whiteboardCount
               whiteboardBackgroundImage:self.room.slideshowViewController.whiteboardBackgroundImage
                               imageSize:pptSize
        ...
        BOOL selected = self.loaclPageIsWhiteboard ? (indexPath.row == self.localWhiteboardIndex) : NO;
        [cell updateContentWithSlidePage:slidePage
                         whiteboardCount:whiteboardCount
               whiteboardBackgroundImage:self.room.slideshowViewController.whiteboardBackgroundImage
                               imageSize:pptSize
