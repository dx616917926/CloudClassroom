...
#import "BJLDrawStrokeColorSelectView.h"
#import "BJLDrawStrokeWidthSelectView.h"
...
@property (nonatomic) BJLDrawStrokeWidthSelectView *strokeWidthSelectView; // 画笔宽度选择
@property (nonatomic) BJLDrawMarkStrokeWidthSelectView *markStrokeWidthSelectView; // 马克笔宽度选择
@property (nonatomic) BJLDrawShapeSelectView *shapeSelectView; // 形状选择
@property (nonatomic) BJLDrawTextOptionView *textOptionView; // 文字选择
@property (nonatomic) BJLDrawStrokeColorSelectView *strokeColorSelectView; // 调色盘
@property (nonatomic) BJLLaserPointView *laserPointView; // 激光笔
@property (nonatomic) BJLTeachingAidSelectView *teachingAidSelectView; // 教具选择
...
- (void)makeSubviews {
    self.laserPointView = [[BJLLaserPointView alloc] initWithRoom:self.room];
    self.laserPointView.accessibilityIdentifier = BJLKeypath(self, laserPointView);
    ...
    self.toolView.accessibilityIdentifier = BJLKeypath(self, toolView);
    [self.view addSubview:self.toolView];
    [self setupGesture];

    // 线宽选择
    self.strokeWidthSelectView = [[BJLDrawStrokeWidthSelectView alloc] initWithRoom:self.room];
    self.strokeWidthSelectView.hidden = YES;
    self.strokeWidthSelectView.accessibilityIdentifier = BJLKeypath(self, strokeWidthSelectView);

    // 马克笔线宽选择
    self.markStrokeWidthSelectView = [[BJLDrawMarkStrokeWidthSelectView alloc] initWithRoom:self.room];
    self.markStrokeWidthSelectView.hidden = YES;
    self.markStrokeWidthSelectView.accessibilityIdentifier = BJLKeypath(self, markStrokeWidthSelectView);

    // 图形选择
    self.shapeSelectView = [[BJLDrawShapeSelectView alloc] initWithRoom:self.room];
    self.shapeSelectView.hidden = YES;
    self.shapeSelectView.accessibilityIdentifier = BJLKeypath(self, shapeSelectView);

    // 字体选择
    self.textOptionView = [[BJLDrawTextOptionView alloc] initWithRoom:self.room];
    self.textOptionView.hidden = YES;
    self.textOptionView.accessibilityIdentifier = BJLKeypath(self, textOptionView);

    // 颜色选择
    self.strokeColorSelectView = [[BJLDrawStrokeColorSelectView alloc] initWithRoom:self.room];
    self.strokeColorSelectView.hidden = YES;
    self.strokeColorSelectView.accessibilityIdentifier = BJLKeypath(self, strokeColorSelectView);

    // 教具选择
    [self teachingAidSelectView];
    self.teachingAidSelectView.hidden = YES;
    self.teachingAidSelectView.accessibilityIdentifier = BJLKeypath(self, teachingAidSelectView);
}
...
- (BOOL)pptButtonIsSelect {
    return [self.toolView pptButtonIsSelect];
}

- (void)showRollCallBadgePoint:(BOOL)show {
    [self.teachingAidSelectView showRollCallBadge:show];
}
...
- (void)makeCallback {
    ...
    [self.toolView setToolButtonClickCallback:^(BJLScToolViewButtonType type, BOOL isSelected, BOOL needNotShowSubWindow) {
        bjl_strongify(self);
        [self hideSelectViewAndOptionView];
        ...
        else if (type == BJLScToolViewButtonType_ppt) {
            if (self.pptButtonClickCallback) {
                self.pptButtonClickCallback(isSelected);
            }
        }
        else if (type == BJLScToolViewButtonType_teachingAid) {
            if (isSelected) {
                [self showTeachingAidView];
            }
        }
        else {
            if (isSelected && !needNotShowSubWindow) {
...
- (void)showTeachingAidView {
    if (!self.teachingAidSelectView.superview || self.teachingAidSelectView.superview != self.view) {
        [self.view addSubview:self.teachingAidSelectView];
    }
    self.teachingAidSelectView.hidden = NO;
    ...
    BOOL iPhone = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPhone);
    UIButton *button = self.toolView.teachingAidButton;
...
- (void)makeObserving {
    ...
    [self bjl_kvo:BJLMakeProperty(self.room.drawingVM, drawingShapeType)
        filter:^BOOL(NSNumber *_Nullable now, NSNumber *_Nullable old, BJLPropertyChange *_Nullable change) {
            ...
            self.toolView.shapeFill = !!self.room.drawingVM.fillColor;
...
- (void)cancelSelectTeachingAidSelectView {
    if (self.toolView.teachingAidButton.isSelected) {
        [self.toolView.teachingAidButton sendActionsForControlEvents:UIControlEventTouchUpInside];
    }
    [self hideSelectViewAndOptionView];
}
...
- (void)hideSelectViewAndOptionView {
    ...
    self.teachingAidSelectView.hidden = YES;
    [self.teachingAidSelectView removeFromSuperview];
}
...
- (BJLTeachingAidSelectView *)teachingAidSelectView {
    if (!_teachingAidSelectView) {
        _teachingAidSelectView = [[BJLTeachingAidSelectView alloc] initWithRoom:self.room fullScreenWidth:NO];
        _teachingAidSelectView.hidden = YES;
        bjl_weakify(self)
            _teachingAidSelectView.countDownCallback = ^() {
            bjl_strongify(self);
            [self cancelSelectTeachingAidSelectView];
            if (self.countDownTimerCallback) {
                self.countDownTimerCallback(self);
            }
        };

        _teachingAidSelectView.questionAnswerCallback = ^() {
            bjl_strongify(self);
            [self cancelSelectTeachingAidSelectView];
            if (self.questionAnswerCallback) {
                self.questionAnswerCallback(self);
            }
        };

        _teachingAidSelectView.envelopeRainCallback = ^() {
            bjl_strongify(self);
            [self cancelSelectTeachingAidSelectView];
            if (self.envelopeRainCallback) {
                self.envelopeRainCallback(self);
            }
        };

        _teachingAidSelectView.rollCallCallback = ^() {
            bjl_strongify(self);
            [self cancelSelectTeachingAidSelectView];
            if (self.rollCallCallback) {
                self.rollCallCallback(self);
            }
        };

        _teachingAidSelectView.badgeStateDidChangeCallback = ^(BJLTeachingAidSelectView *_Nonnull view) {
            bjl_strongify(self);
            BOOL show = NO;
            if (view.rollCallBadgeDidShown) {
                show = YES;
            }
            [self.toolView showTeachingAidButtonBadgePoint:show];
        };

        _teachingAidSelectView.questionResponderCallback = ^{
            bjl_strongify(self);
            [self cancelSelectTeachingAidSelectView];
            if (self.questionResponderCallback) {
                self.questionResponderCallback(self);
            }
        };
    }
    return _teachingAidSelectView;
}
@end
