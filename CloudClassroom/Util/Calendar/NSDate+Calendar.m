...
- (NSInteger) weekInMonth {
    NSDateComponents *components = [[self __calendar] components:(NSCalendarUnitWeekdayOrdinal) fromDate:self];
    NSInteger result = ([components weekdayOrdinal]);
    return result;
}
...
- (NSDate *) preMonth {
    ...
    NSDate *newDate = [[self __calendar] dateByAddingComponents:dateComponents toDate:[self firstDateInThisMonth] options:0];
    return newDate;
}
