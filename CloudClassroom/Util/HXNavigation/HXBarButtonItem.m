...
- (instancetype)initWithTitle:(NSString *)title style:(HXBarButtonItemStyle)style handler:(void (^)(id sender))action {
    ...
    if (self = [self init]) {
        ...
        [button setTitleColor:COLOR_WITH_ALPHA(0xFFFFFF, 1) forState:UIControlStateNormal];
        [button sizeToFit];
        button.height = 44;
        button.width += 30;
...
- (void)setBadge:(NSString *)badge {
    ...
    if (!self.badgeLabel && badge) {
        ...
        self.badgeLabel.backgroundColor = [UIColor colorWithRed:0.902 green:0.200 blue:0.216 alpha:1.000];
