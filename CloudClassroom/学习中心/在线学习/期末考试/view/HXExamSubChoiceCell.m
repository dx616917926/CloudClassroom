...
-(void)setExamPaperSubQuestionModel:(HXExamPaperSubQuestionModel *)examPaperSubQuestionModel{
    ...
    if (examPaperSubQuestionModel.isWenDa) {//问答题
        ...
        //处理附件图片
        if (examPaperSubQuestionModel.attach.count==0) {//初始化
            examPaperSubQuestionModel.attach = [NSMutableArray array];
        }
        //处理答题和查看答卷时，图片布局
        self.deletePhotoBtn.hidden = !examPaperSubQuestionModel.isContinuerExam;
...
-(void)addPhoto:(UIButton *)button{
    [self endEditing:YES];
    if (self.examPaperSubQuestionModel.attach.count>=5) {
...
-(void)uploadImageWithData:(NSData *)fileData image:(UIImage *)image
{
    if (fileData) {
        ...
        NSString *url =[NSString stringWithFormat:@"%@%@",self.examPaperSubQuestionModel.domain,HXPOST_Answer_FILE];
        ...
        progress:^(NSProgress * _Nonnull uploadProgress) {
            ...
            if ([dic boolValueForKey:@"success"]){
                ...
                [self.examPaperSubQuestionModel.attach addObject:tmpFileName];
                //刷新布局
                [self refreshPhotosContainerViewLayout];
                NSLog(@"%@",tmpFileName);
...
-(void)deletePhoto:(UIButton *)sender{
    ...
    [self.examPaperSubQuestionModel.attach removeObjectAtIndex:self.currentIndex];
    //重新布局照片
    [self refreshPhotosContainerViewLayout];
}
...
-(void)refreshPhotosContainerViewLayout{
    ...
    if (!self.examPaperSubQuestionModel.isContinuerExam) {//查看答卷
        //答案附件,格式: fileId/fileName 多个用逗号分割;
        NSArray *files = [self.examPaperSubQuestionModel.answerModel.file componentsSeparatedByString:@","];
        [files enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *file = obj;
            if (![HXCommonUtil isNull:file]) {
                NSArray *tempArray = [file componentsSeparatedByString:@"/"];
                NSString *imageStr = [NSString stringWithFormat:@"%@/exam/student/exam/question/attaches/upload/file/%@/filePath?inline&__id=%@&__name=%@&__userExamId=%@",self.examPaperSubQuestionModel.domain,self.examPaperSubQuestionModel.answerModel.pqt_id,tempArray.firstObject,tempArray.lastObject,self.examPaperSubQuestionModel.userExamId];
                [imageUrls addObject:imageStr];
            }
        }];
    }else{
        //答案附件,格式: fileId/fileName 多个用逗号分割;
        [self.examPaperSubQuestionModel.attach enumerateObjectsUsingBlock:^(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
            NSString *file = obj;
            if (![HXCommonUtil isNull:file]) {
                NSArray *tempArray = [file componentsSeparatedByString:@"/"];
                //问题id截掉"q_"
                NSString *psqId = HXSafeString([self.examPaperSubQuestionModel.sub_id substringFromIndex:2]);
                NSString *imageStr = [NSString stringWithFormat:@"%@/exam/student/exam/question/attaches/upload/file/%@/filePath?inline&__id=%@&__name=%@&__userExamId=%@",self.examPaperSubQuestionModel.domain,psqId,tempArray.firstObject,tempArray.lastObject,self.examPaperSubQuestionModel.userExamId];
                [imageUrls addObject:imageStr];
            }
        }];
    }
